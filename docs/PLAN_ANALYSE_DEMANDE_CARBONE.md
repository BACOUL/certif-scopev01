...
| 2026-08-17 | `feat/fr-request-analyzer` | Suppression de l'ecart sous le Header FR et fiabilisation de l'ouverture de l'analyseur | Incident reproduit sur le deploiement signale ; 48 tests passes ; `tsc --noEmit` et `next build` reussis ; recette sombre en 360, 390, 768 et 1280 px ; navigation complete et analyse compatible executees en navigateur | Ecart entre Header et contenu inferieur a 1 px aux quatre formats, aucun debordement, analyseur charge sans exception et resultat/CTA coherents | Publier puis verifier la nouvelle preview Vercel |
