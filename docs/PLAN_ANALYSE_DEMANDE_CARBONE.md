...
| 2026-08-17 | `feat/fr-request-analyzer` | Correction de la lisibilite du Header FR sur mobile en mode sombre | Capture Android fournie ; 48 tests passes ; `tsc --noEmit` et `next build` reussis ; rendu sombre controle en 360, 390, 768 et 1280 px | Fond calcule `rgba(255, 255, 255, 0.95)` aux quatre formats, logo lisible et aucun debordement horizontal ; Hero, Stripe, calcul carbone, PDF et pages allemandes inchanges | Publier puis verifier la preview Vercel |
