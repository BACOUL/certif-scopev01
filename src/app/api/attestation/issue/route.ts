export const runtime = "nodejs";

import Stripe from "stripe";
import QRCode from "qrcode";
import { signCanonicalPayload, makeAttestationId } from "@/lib/sign";
import {
  ATTESTATION_I18N,
  AttestationLocale,
  DEFAULT_ATTESTATION_LOCALE,
} from "@/lib/attestation-i18n/index";

const stripe = new Stripe(process.env.STRIPE_SECRET_KEY!);

/**
 * Logo Certif-Scope en base64.
 *
 * IMPORTANT :
 * - Tu peux coller ton base64 complet ici.
 * - Tu peux le coller sur plusieurs lignes.
 * - Ne mets PAS "data:image/png;base64,".
 * - Le .replace(/\s+/g, "") supprime automatiquement les retours à la ligne.
 */
const RAW_CERTIF_SCOPE_LOGO_BASE64 = `

`.replace(/\s+/g, "");

const CERTIF_SCOPE_LOGO_BASE64 = RAW_CERTIF_SCOPE_LOGO_BASE64;

function escapeHtml(input: string) {
  return input
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
    .replace(/"/g, "&quot;")
    .replace(/'/g, "&#39;");
}

function toBase64Url(input: string) {
  return Buffer.from(input, "utf8")
    .toString("base64")
    .replace(/\+/g, "-")
    .replace(/\//g, "_")
    .replace(/=+$/g, "");
}

function addMonthsToISODate(date: string, months: number) {
  const parsed = new Date(`${date}T00:00:00.000Z`);

  if (Number.isNaN(parsed.getTime())) {
    return "";
  }

  parsed.setUTCMonth(parsed.getUTCMonth() + months);

  return parsed.toISOString().slice(0, 10);
}

function formatCO2e(value: number, locale: AttestationLocale) {
  const fixed = Number.isInteger(value) ? value.toFixed(0) : value.toFixed(2);

  if (locale === "fr" || locale === "de") {
    return fixed.replace(".", ",");
  }

  return fixed;
}

function normalizeSectionTitle(input: string) {
  const raw = String(input || "").replace(/\s+/g, " ").trim();

  return raw
    .replace(/^(\d+)\.\s*\1\.\s*/u, "$1. ")
    .replace(/^(\d+)\.\s*(\d+)\.\s*/u, (_, a, b) =>
      a === b ? `${a}. ` : `${a}. ${b}. `
    );
}

function stripLeadingNumber(input: string) {
  return normalizeSectionTitle(input).replace(/^\d+\.\s*/u, "").trim();
}

function getFactorVersionLabel(locale: AttestationLocale) {
  if (locale === "fr") return "Version des facteurs";
  if (locale === "de") return "Faktorenversion";
  return "Factor version";
}

function getFoundationTitle(locale: AttestationLocale) {
  if (locale === "fr") return "Fondement méthodologique";
  if (locale === "de") return "Methodische Grundlage";
  return "Methodological basis";
}

function getUseCaseTitle(locale: AttestationLocale) {
  if (locale === "fr") return "Usage prévu";
  if (locale === "de") return "Vorgesehene Verwendung";
  return "Intended use";
}

function getFormulaLine(locale: AttestationLocale) {
  if (locale === "fr") {
    return "Dépenses agrégées déclarées × facteurs d’émission monétaires = estimation CO₂e indicative";
  }

  if (locale === "de") {
    return "Aggregierte deklarierte Ausgaben × monetäre Emissionsfaktoren = indikative CO₂e-Schätzung";
  }

  return "Declared aggregated expenses × monetary emission factors = indicative CO₂e estimate";
}

function getLocaleCopy(locale: AttestationLocale) {
  if (locale === "fr") {
    return {
      pageTwoTitle: "Méthodologie, références et vérification",
      confidentialityTitle: "Confidentialité",
      authenticityTitle: "Authenticité",
      validityTitle: "Validité",
      formulaTitle: "Formule d’estimation indicative",
      verificationSimpleTitle: "Vérification simple",
      verifiableObjectTitle: "Objet vérifiable",
      technicalElementsTitle: "Éléments techniques de vérification",
      perimeterLimitsTitle: "Périmètre et limites",
      referenceLabelShort: "Référence",
      issuerLabelShort: "Émetteur",
      pageVerificationLabel: "Page de vérification documentaire",
      useQrOrReferencePrefix:
        "Utiliser le QR code ou la référence d’attestation :",
      footerPageLabel: "Page",
      headerBanner: "Document CO₂e indicatif · standardisé · vérifiable",
      pageTwoIntro:
        "Cette page réunit la méthodologie utilisée, les références de cohérence et les éléments de vérification documentaire.",
      languageNotice:
        "Langue du document : en cas de divergence entre les versions linguistiques, la version anglaise prévaut.",
      privacyBullets: [
        "Aucune conservation des données financières détaillées.",
        "Résultat CO₂e agrégé uniquement.",
        "Attestation vérifiable indépendamment.",
      ],
    };
  }

  if (locale === "de") {
    return {
      pageTwoTitle: "Methodik, Referenzen und Verifikation",
      confidentialityTitle: "Vertraulichkeit",
      authenticityTitle: "Authentizität",
      validityTitle: "Gültigkeit",
      formulaTitle: "Indikative Schätzformel",
      verificationSimpleTitle: "Einfache Verifikation",
      verifiableObjectTitle: "Verifizierbares Dokument",
      technicalElementsTitle: "Technische Verifikationselemente",
      perimeterLimitsTitle: "Umfang und Grenzen",
      referenceLabelShort: "Referenz",
      issuerLabelShort: "Aussteller",
      pageVerificationLabel: "Dokumentarische Verifizierungsseite",
      useQrOrReferencePrefix:
        "QR-Code oder Attestierungsreferenz verwenden:",
      footerPageLabel: "Seite",
      headerBanner:
        "Indikatives CO₂e-Dokument · standardisiert · verifizierbar",
      pageTwoIntro:
        "Diese Seite fasst die verwendete Methodik, Kohärenzreferenzen und dokumentarische Verifikationselemente zusammen.",
      languageNotice:
        "Dokumentsprache: Bei Abweichungen zwischen den Sprachversionen hat die englische Version Vorrang.",
      privacyBullets: [
        "Keine Speicherung detaillierter Finanzdaten.",
        "Nur aggregiertes CO₂e-Ergebnis.",
        "Unabhängig verifizierbare Attestierung.",
      ],
    };
  }

  return {
    pageTwoTitle: "Methodology, references and verification",
    confidentialityTitle: "Confidentiality",
    authenticityTitle: "Authenticity",
    validityTitle: "Validity",
    formulaTitle: "Indicative estimation formula",
    verificationSimpleTitle: "Simple verification",
    verifiableObjectTitle: "Verifiable object",
    technicalElementsTitle: "Technical verification elements",
    perimeterLimitsTitle: "Scope and limitations",
    referenceLabelShort: "Reference",
    issuerLabelShort: "Issuer",
    pageVerificationLabel: "Document verification page",
    useQrOrReferencePrefix:
      "Use the QR code or the attestation reference:",
    footerPageLabel: "Page",
    headerBanner: "Indicative CO₂e document · standardised · verifiable",
    pageTwoIntro:
      "This page consolidates the applied methodology, consistency references and documentary verification elements.",
    languageNotice:
      "Document language: in case of divergence between language versions, the English version shall prevail.",
    privacyBullets: [
      "No storage of detailed financial data.",
      "Aggregated CO₂e result only.",
      "Independently verifiable attestation.",
    ],
  };
}

export async function GET(req: Request) {
  try {
    if (!process.env.PDFSHIFT_API_KEY) {
      return new Response("PDFSHIFT_API_KEY missing", { status: 500 });
    }

    const { searchParams } = new URL(req.url);
    const sessionId = searchParams.get("session_id");

    if (!sessionId) {
      return new Response("Missing session_id", { status: 400 });
    }

    let metadataRaw: Record<string, any> = {};

    if (sessionId.startsWith("key_")) {
      metadataRaw = Object.fromEntries(searchParams.entries());
    } else {
      const session = await stripe.checkout.sessions.retrieve(sessionId);

      if (session.payment_status !== "paid") {
        return new Response("Payment not completed", { status: 403 });
      }

      metadataRaw = session.metadata || {};
    }

    const locale =
      (metadataRaw.attestationLocale as AttestationLocale) ||
      DEFAULT_ATTESTATION_LOCALE;

    const i18n = ATTESTATION_I18N[locale] || ATTESTATION_I18N.en;
    const copy = getLocaleCopy(locale);

    const required = ["companyName", "totalCO2e", "year"];
    const missing = required.filter((k) => {
      const v = metadataRaw[k];
      return v === undefined || v === null || String(v).trim() === "";
    });

    if (missing.length) {
      return new Response(`Missing metadata: ${missing.join(", ")}`, {
        status: 400,
      });
    }

    const totalCO2eNum = Number(String(metadataRaw.totalCO2e).replace(",", "."));

    if (Number.isNaN(totalCO2eNum)) {
      return new Response("Invalid metadata: totalCO2e must be a number", {
        status: 400,
      });
    }

    const issuedDate = new Date().toISOString();

    const canonicalPayload = {
      issuer: "Certif-Scope" as const,
      standard: "CS-SB-v1" as const,
      attestationId: "",
      companyName: String(metadataRaw.companyName),
      country: String(metadataRaw.country || "—"),
      year: String(metadataRaw.year),
      totalCO2e: String(totalCO2eNum),
      issuedDate,
    };

    const tempSignature = signCanonicalPayload({
      ...canonicalPayload,
      attestationId: "TEMP",
    });

    const attestationId = makeAttestationId(
      canonicalPayload.year,
      tempSignature.hashHex
    );

    const signatureResult = signCanonicalPayload({
      ...canonicalPayload,
      attestationId,
    });

    const issuedDateForDisplay = issuedDate.slice(0, 10);
    const validityMonths = String(metadataRaw.validityMonths || "12");
    const validUntil =
      String(metadataRaw.validUntil || "").trim() ||
      addMonthsToISODate(issuedDateForDisplay, Number(validityMonths) || 12);

    const factorVersion = String(
      metadataRaw.factorVersion ||
        metadataRaw.emissionFactorVersion ||
        "Certif-Scope factors v1"
    );

    const metadata = {
      attestationId,
      issuerName: escapeHtml(String(metadataRaw.issuerName || "Certif-Scope")),
      issuerSite: escapeHtml(
        String(metadataRaw.issuerSite || "https://www.certif-scope.com")
      ),
      companyName: escapeHtml(String(metadataRaw.companyName)),
      companySector: escapeHtml(String(metadataRaw.companySector || "—")),
      entityIdentifier: escapeHtml(String(metadataRaw.entityIdentifier || "—")),
      country: escapeHtml(String(metadataRaw.country || "—")),
      year: escapeHtml(String(metadataRaw.year)),
      totalCO2e: escapeHtml(formatCO2e(totalCO2eNum, locale)),
      methodology: escapeHtml(
        String(
          metadataRaw.methodology ||
            "Certif-Scope deterministic spend-based methodology v1.0"
        )
      ),
      issuedDate: escapeHtml(issuedDateForDisplay),
      validUntil: escapeHtml(validUntil),
      validityMonths: escapeHtml(validityMonths),
      standardRef: escapeHtml(
        String(metadataRaw.standardRef || "Certif-Scope CS-SB-v1")
      ),
      signature: signatureResult.signatureBase64,
      hash: signatureResult.hashHex,
      algorithm: signatureResult.algorithm,
      factorVersion: escapeHtml(factorVersion),
    };

    const verificationPayload = {
      certificateId: metadata.attestationId,
      issuer: "Certif-Scope",
      issuedAt: metadata.issuedDate,
      validUntil: metadata.validUntil,
      methodVersion: "CS-SB-v1",
      factorVersion,
      algorithm: metadata.algorithm,
      hash: metadata.hash,
      signature: metadata.signature,
    };

    const verificationToken = toBase64Url(JSON.stringify(verificationPayload));

    const verifyUrl =
      locale === "fr"
        ? `https://www.certif-scope.com/fr/verify/?v=${verificationToken}`
        : `https://www.certif-scope.com/verify/?v=${verificationToken}`;

    const verificationDisplayUrl =
      locale === "fr"
        ? "https://www.certif-scope.com/fr/verify"
        : "https://www.certif-scope.com/verify";

    const qrDataUrl = await QRCode.toDataURL(verifyUrl, {
      width: 160,
      margin: 1,
    });

    const entitySectionTitle = escapeHtml(
      stripLeadingNumber(i18n.entitySectionTitle || "")
    );
    const documentNatureSectionTitle = escapeHtml(
      stripLeadingNumber(i18n.documentNatureSectionTitle || "")
    );
    const scopeSectionTitle = escapeHtml(
      stripLeadingNumber(i18n.scopeSectionTitle || "")
    );
    const referencesSectionTitle = escapeHtml(
      stripLeadingNumber(i18n.referencesSectionTitle || "")
    );
    const methodologySectionTitle = escapeHtml(
      stripLeadingNumber(i18n.methodologySectionTitle || "")
    );
    const verificationSectionTitle = escapeHtml(
      stripLeadingNumber(i18n.verificationSectionTitle || "")
    );

    const html = `
<!doctype html>
<html lang="${locale}">
<head>
<meta charset="utf-8"/>
<title>${metadata.issuerName} — Attestation</title>
<style>
  @page {
    size: A4;
    margin: 10mm 12mm 10mm 12mm;
  }

  :root {
    --ink: #10263b;
    --accent: #0B3A63;
    --accent-2: #1FB6C1;
    --muted: #667281;
    --muted-2: #8a97a6;
    --line: #d7e0e8;
    --line-strong: #c4d1dd;
    --bg-soft: #f8fafc;
    --bg-tint: #eef8f9;
    --white: #ffffff;
    --serif: "Times New Roman", Georgia, serif;
  }

  * {
    box-sizing: border-box;
  }

  html,
  body {
    margin: 0;
    padding: 0;
    background: #fff;
  }

  body {
    font-family: Inter, "Helvetica Neue", Arial, Helvetica, sans-serif;
    font-size: 9.8px;
    line-height: 1.43;
    color: var(--ink);
    -webkit-font-smoothing: antialiased;
  }

  .page {
    height: 277mm;
    display: flex;
    flex-direction: column;
    overflow: hidden;
  }

  .page.page-one {
    page-break-after: always;
  }

  .page-main {
    flex: 1;
    display: flex;
    flex-direction: column;
  }

  .footer-static {
    margin-top: 7px;
    padding-top: 5px;
    border-top: 1px solid var(--line);
    display: flex;
    justify-content: space-between;
    align-items: center;
    font-size: 8px;
    color: var(--muted);
  }

  .header {
    display: flex;
    justify-content: space-between;
    align-items: flex-start;
    gap: 16px;
    padding-bottom: 8px;
    border-bottom: 1.8px solid var(--accent);
    margin-bottom: 9px;
  }

  .header-left {
    display: flex;
    flex-direction: column;
    justify-content: flex-start;
    gap: 4px;
    max-width: 74%;
  }

  .issuer-logo {
    height: 58px;
    max-width: 265px;
    width: auto;
    object-fit: contain;
    display: block;
  }

  .issuer-site {
    font-size: 8px;
    color: var(--muted);
  }

  .issuer-site a {
    color: inherit;
    text-decoration: none;
  }

  .issuer-meta {
    font-size: 8px;
    color: var(--muted);
  }

  .qr-block {
    flex: 0 0 auto;
    text-align: center;
  }

  .qr-block img {
    width: 86px;
    height: 86px;
    padding: 4px;
    background: #fff;
    border: 1px solid var(--line-strong);
    border-radius: 4px;
  }

  .qr-caption {
    margin-top: 4px;
    font-size: 7.5px;
    color: var(--ink);
    line-height: 1.2;
  }

  .title-zone {
    text-align: center;
    padding: 6px 0 6px;
  }

  .eyebrow {
    font-size: 8px;
    font-weight: 800;
    letter-spacing: 1.05px;
    text-transform: uppercase;
    color: var(--accent-2);
    margin-bottom: 4px;
  }

  .title-zone h1 {
    margin: 0;
    font-size: 19px;
    line-height: 1.08;
    font-weight: 800;
    color: var(--accent);
    text-transform: uppercase;
    letter-spacing: 0.2px;
  }

  .title-formal {
    margin-top: 4px;
    font-size: 9px;
    font-weight: 700;
    color: var(--ink);
  }

  .title-sub {
    margin-top: 2px;
    font-size: 8px;
    color: var(--muted);
  }

  .result-panel {
    display: flex;
    justify-content: center;
    margin: 6px 0 10px;
  }

  .result-box {
    width: 76%;
    border: 2px solid var(--accent);
    border-radius: 6px;
    background: #fff;
    padding: 9px 14px 10px;
    text-align: center;
    page-break-inside: avoid;
  }

  .result-label {
    font-size: 8px;
    font-weight: 800;
    letter-spacing: 0.7px;
    text-transform: uppercase;
    color: var(--muted);
    margin-bottom: 5px;
  }

  .result-value {
    font-family: var(--serif);
    font-size: 27px;
    line-height: 1;
    font-weight: 800;
    color: var(--accent);
    margin-bottom: 4px;
  }

  .result-value span {
    font-size: 13px;
    color: var(--accent-2);
    font-family: Inter, Arial, sans-serif;
    font-weight: 800;
  }

  .result-note {
    font-size: 8px;
    color: var(--muted);
  }

  .section-card {
    border: 1px solid var(--line);
    border-radius: 5px;
    background: #fff;
    padding: 8px 10px;
    margin-bottom: 7px;
    page-break-inside: avoid;
  }

  .section-card.tint {
    background: var(--bg-tint);
    border-color: #d8e8eb;
    border-left: 4px solid var(--accent-2);
    padding-left: 8px;
  }

  .section-title {
    margin: 0 0 5px;
    font-size: 9px;
    line-height: 1.15;
    font-weight: 800;
    color: var(--accent);
    text-transform: uppercase;
    letter-spacing: 0.45px;
  }

  .plain-text {
    margin: 0;
    font-size: 9px;
    color: var(--ink);
  }

  .summary-grid {
    display: grid;
    grid-template-columns: repeat(4, 1fr);
    border: 1px solid var(--line-strong);
    border-radius: 5px;
    overflow: hidden;
    margin-bottom: 7px;
    page-break-inside: avoid;
  }

  .summary-grid .cell {
    padding: 7px 9px 8px;
    background: #fff;
    border-right: 1px solid var(--line-strong);
  }

  .summary-grid .cell:last-child {
    border-right: none;
  }

  .summary-grid .cell-label {
    display: block;
    font-size: 7px;
    line-height: 1.15;
    text-transform: uppercase;
    letter-spacing: 0.5px;
    color: var(--muted-2);
    margin-bottom: 4px;
  }

  .summary-grid .cell-value {
    display: block;
    font-size: 9px;
    font-weight: 800;
    color: var(--ink);
  }

  .entity-grid {
    display: grid;
    grid-template-columns: 1fr 1fr;
    gap: 8px 24px;
  }

  .entity-item .label {
    display: block;
    font-size: 7px;
    line-height: 1.1;
    text-transform: uppercase;
    letter-spacing: 0.5px;
    color: var(--muted-2);
    margin-bottom: 3px;
  }

  .entity-item .value {
    display: block;
    font-size: 9px;
    font-weight: 800;
    color: var(--ink);
    line-height: 1.3;
  }

  .two-col {
    display: grid;
    grid-template-columns: 1fr 1fr;
    gap: 8px;
  }

  .feature-strip {
    display: grid;
    grid-template-columns: repeat(3, 1fr);
    gap: 8px;
  }

  .feature-card {
    border: 1px solid var(--line);
    border-radius: 5px;
    background: var(--bg-soft);
    padding: 8px 10px 9px;
    min-height: 72px;
    page-break-inside: avoid;
  }

  .feature-card .feature-title {
    margin: 0 0 4px;
    font-size: 8.6px;
    font-weight: 800;
    color: var(--accent);
    text-transform: uppercase;
    letter-spacing: 0.45px;
  }

  .feature-card .feature-text {
    font-size: 8.6px;
    line-height: 1.34;
    color: var(--ink);
  }

  .feature-card .feature-text ul {
    margin: 0;
    padding-left: 12px;
  }

  .feature-card .feature-text li {
    margin-bottom: 1px;
  }

  .page-two-header {
    display: flex;
    justify-content: space-between;
    align-items: flex-start;
    gap: 12px;
    margin-bottom: 9px;
    border-bottom: 1.8px solid var(--accent);
    padding-bottom: 7px;
  }

  .page-two-header .left {
    max-width: 72%;
  }

  .page-two-header .eyebrow {
    margin-bottom: 5px;
    color: var(--accent-2);
  }

  .page-two-header h2 {
    margin: 0 0 4px;
    font-size: 18px;
    line-height: 1.1;
    font-weight: 800;
    color: var(--accent);
    text-transform: uppercase;
  }

  .page-two-header .intro {
    margin: 0;
    font-size: 8.8px;
    color: var(--muted);
  }

  .page-two-reference {
    text-align: right;
    font-size: 8px;
    color: var(--muted);
    line-height: 1.3;
    padding-top: 3px;
  }

  .formula-box {
    margin-top: 8px;
    border-radius: 4px;
    background: #edf6f8;
    border: 1px solid #d8eaee;
    padding: 6px 10px;
    text-align: center;
    font-size: 8.6px;
    font-weight: 800;
    color: var(--accent);
  }

  .mini-grid {
    display: grid;
    grid-template-columns: 1fr 1fr;
    gap: 8px;
    margin-top: 7px;
  }

  .mini-pill {
    background: var(--bg-soft);
    border: 1px solid var(--line);
    border-radius: 4px;
    padding: 7px 8px;
  }

  .mini-pill .mini-label {
    display: block;
    font-size: 7px;
    text-transform: uppercase;
    letter-spacing: 0.45px;
    color: var(--muted-2);
    margin-bottom: 3px;
  }

  .mini-pill .mini-value {
    display: block;
    font-size: 8.8px;
    font-weight: 800;
    color: var(--ink);
    line-height: 1.3;
  }

  .reference-list {
    margin: 4px 0 0;
    padding-left: 15px;
    columns: 2;
    column-gap: 28px;
  }

  .reference-list li {
    margin-bottom: 3px;
    break-inside: avoid;
    font-size: 8.8px;
    color: var(--ink);
  }

  .small-note {
    margin-top: 6px;
    font-size: 8px;
    color: var(--muted);
  }

  .verification-grid {
    display: grid;
    grid-template-columns: 1fr 1fr;
    gap: 8px;
  }

  .verification-card {
    border: 1px solid var(--line);
    border-radius: 5px;
    padding: 9px 10px;
    background: var(--bg-soft);
    min-height: 104px;
    page-break-inside: avoid;
  }

  .verification-card.primary {
    background: #eef7f9;
    border-left: 4px solid var(--accent-2);
    padding-left: 8px;
  }

  .verification-card h3 {
    margin: 0 0 5px;
    font-size: 9px;
    font-weight: 800;
    color: var(--accent);
    text-transform: uppercase;
    letter-spacing: 0.45px;
  }

  .verification-card p {
    margin: 0 0 6px;
    font-size: 8.8px;
    color: var(--ink);
    line-height: 1.36;
  }

  .verification-link {
    color: var(--accent);
    text-decoration: none;
    word-break: break-all;
    font-weight: 800;
  }

  .technical-grid {
    display: grid;
    grid-template-columns: 132px 1fr;
    gap: 10px;
    row-gap: 4px;
    margin-top: 5px;
    font-size: 7.7px;
    color: var(--ink);
  }

  .technical-grid .tech-label {
    font-size: 7px;
    text-transform: uppercase;
    letter-spacing: 0.45px;
    color: var(--muted-2);
  }

  .technical-grid .tech-value {
    font-size: 7.5px;
    line-height: 1.25;
    word-break: break-all;
    color: var(--ink);
  }

  .limit-grid {
    display: grid;
    grid-template-columns: 1fr 1fr;
    gap: 10px;
    margin-top: 8px;
  }

  .limit-subtitle {
    margin: 0 0 3px;
    font-size: 8.5px;
    font-weight: 800;
    color: var(--accent);
    text-transform: uppercase;
    letter-spacing: 0.35px;
  }

  .language-note {
    margin-top: 8px;
    font-size: 8px;
    color: var(--ink);
    font-weight: 800;
  }

  .doc-standard {
    margin-top: 3px;
    font-size: 7.6px;
    color: var(--muted);
  }

  .dense {
    margin-bottom: 8px;
    padding: 10px 11px;
  }

  .spacious {
    padding-top: 10px;
    padding-bottom: 10px;
  }
</style>
</head>
<body>
  <div class="page page-one">
    <div class="page-main">
      <div class="header">
        <div class="header-left">
          ${
            CERTIF_SCOPE_LOGO_BASE64
              ? `<img src="data:image/png;base64,${CERTIF_SCOPE_LOGO_BASE64}" class="issuer-logo" alt="Certif-Scope" />`
              : ""
          }
          <div class="issuer-site">
            <a href="${metadata.issuerSite}" target="_blank">${metadata.issuerSite}</a>
          </div>
          <div class="issuer-meta">${i18n.headerTagline}</div>
        </div>

        <div class="qr-block">
          <img src="${qrDataUrl}" alt="QR verification" />
          <div class="qr-caption">${i18n.scanToVerifyLabel}</div>
        </div>
      </div>

      <div class="title-zone">
        <div class="eyebrow">${copy.headerBanner}</div>
        <h1>${i18n.title}</h1>
        <div class="title-formal">${i18n.standardReference}</div>
        <div class="title-sub">${i18n.subtitle}</div>
      </div>

      <div class="result-panel">
        <div class="result-box">
          <div class="result-label">${i18n.resultLabel}</div>
          <div class="result-value">${metadata.totalCO2e} <span>tCO₂e</span></div>
          <div class="result-note">${i18n.documentScopeSummaryText}</div>
        </div>
      </div>

      <div class="summary-grid">
        <div class="cell">
          <span class="cell-label">${i18n.attestationReferenceLabel}</span>
          <span class="cell-value">${metadata.attestationId}</span>
        </div>
        <div class="cell">
          <span class="cell-label">${i18n.issuedDateLabel}</span>
          <span class="cell-value">${metadata.issuedDate}</span>
        </div>
        <div class="cell">
          <span class="cell-label">${i18n.validUntilLabel}</span>
          <span class="cell-value">${metadata.validUntil}</span>
        </div>
        <div class="cell">
          <span class="cell-label">${i18n.issuerLabel}</span>
          <span class="cell-value">${metadata.issuerName}</span>
        </div>
      </div>

      <div class="section-card tint dense">
        <h2 class="section-title">${getFoundationTitle(locale)}</h2>
        <p class="plain-text">${i18n.documentNatureText}</p>
      </div>

      <div class="section-card dense">
        <h2 class="section-title">${getUseCaseTitle(locale)}</h2>
        <p class="plain-text">${i18n.documentScopeSummaryText}</p>
      </div>

      <div class="section-card spacious">
        <h2 class="section-title">${entitySectionTitle}</h2>
        <div class="entity-grid">
          <div class="entity-item">
            <span class="label">${i18n.entityNameLabel}</span>
            <span class="value">${metadata.companyName}</span>
          </div>
          <div class="entity-item">
            <span class="label">${i18n.countryLabel}</span>
            <span class="value">${metadata.country}</span>
          </div>
          <div class="entity-item">
            <span class="label">${i18n.activitySectorLabel}</span>
            <span class="value">${metadata.companySector}</span>
          </div>
          <div class="entity-item">
            <span class="label">${i18n.reportingYearLabel}</span>
            <span class="value">${metadata.year}</span>
          </div>
          <div class="entity-item">
            <span class="label">${i18n.entityIdentifierLabel}</span>
            <span class="value">${metadata.entityIdentifier}</span>
          </div>
          <div class="entity-item">
            <span class="label">${i18n.attestationReferenceLabel}</span>
            <span class="value">${metadata.attestationId}</span>
          </div>
        </div>
      </div>

      <div class="two-col">
        <div class="section-card">
          <h2 class="section-title">${documentNatureSectionTitle}</h2>
          <p class="plain-text"><strong>${i18n.documentNatureText}</strong></p>
        </div>

        <div class="section-card">
          <h2 class="section-title">${scopeSectionTitle}</h2>
          <p class="plain-text"><strong>${i18n.scopeDescriptionLabel} :</strong> ${i18n.scopeText}</p>
        </div>
      </div>

      <div class="feature-strip">
        <div class="feature-card">
          <div class="feature-title">${copy.confidentialityTitle}</div>
          <div class="feature-text">
            <ul>
              ${copy.privacyBullets.map((item) => `<li>${escapeHtml(item)}</li>`).join("")}
            </ul>
          </div>
        </div>

        <div class="feature-card">
          <div class="feature-title">${copy.authenticityTitle}</div>
          <div class="feature-text">
            ${i18n.authenticityOverviewText}
          </div>
        </div>

        <div class="feature-card">
          <div class="feature-title">${copy.validityTitle}</div>
          <div class="feature-text">
            ${i18n.documentValidityText}
          </div>
        </div>
      </div>
    </div>

    <div class="footer-static">
      <div>${i18n.footerText}</div>
      <div>${copy.footerPageLabel} 1 / 2</div>
    </div>
  </div>

  <div class="page page-two">
    <div class="page-main">
      <div class="page-two-header">
        <div class="left">
          <div class="eyebrow">${verificationSectionTitle}</div>
          <h2>${copy.pageTwoTitle}</h2>
          <p class="intro">${copy.pageTwoIntro}</p>
        </div>

        <div class="page-two-reference">
          <div><strong>${copy.referenceLabelShort} :</strong> ${metadata.attestationId}</div>
          <div><strong>${copy.issuerLabelShort} :</strong> ${metadata.issuerName}</div>
        </div>
      </div>

      <div class="section-card dense">
        <h2 class="section-title">${methodologySectionTitle}</h2>
        <p class="plain-text"><strong>${i18n.methodologyLabel} :</strong> ${metadata.methodology}</p>

        <div class="formula-box">
          ${copy.formulaTitle} · ${getFormulaLine(locale)}
        </div>

        <div class="mini-grid">
          <div class="mini-pill">
            <span class="mini-label">${getFactorVersionLabel(locale)}</span>
            <span class="mini-value">${metadata.factorVersion}</span>
          </div>
          <div class="mini-pill">
            <span class="mini-label">${i18n.transferabilityLabel}</span>
            <span class="mini-value">${i18n.transferabilityText}</span>
          </div>
        </div>
      </div>

      <div class="section-card dense">
        <h2 class="section-title">${referencesSectionTitle}</h2>
        <p class="plain-text"><em>${i18n.normativeText}</em></p>
        <ul class="reference-list">
          ${i18n.referencesList.map((ref: string) => `<li>${ref}</li>`).join("")}
        </ul>
        <div class="small-note">${i18n.scopeNote}</div>
      </div>

      <div class="verification-grid">
        <div class="verification-card primary">
          <h3>${copy.verificationSimpleTitle}</h3>
          <p>${copy.useQrOrReferencePrefix} ${metadata.attestationId}</p>
          <p>
            <strong>${copy.pageVerificationLabel} :</strong><br/>
            <a class="verification-link" href="${verificationDisplayUrl}">${verificationDisplayUrl}</a>
          </p>
          <p class="small-note">${i18n.pdfObjectText}</p>
        </div>

        <div class="verification-card">
          <h3>${copy.verifiableObjectTitle}</h3>
          <p>${i18n.verificationBoxTitle}</p>
          <p>${i18n.technicalVerificationNote}</p>
          <p class="small-note">${i18n.publicKeyNote}</p>
        </div>
      </div>

      <div class="section-card dense">
        <h2 class="section-title">${copy.technicalElementsTitle}</h2>
        <p class="small-note">${i18n.authenticityOverviewText}</p>

        <div class="technical-grid">
          <div class="tech-label">${i18n.algorithmLabel}</div>
          <div class="tech-value">${metadata.algorithm}</div>

          <div class="tech-label">${i18n.hashLabel}</div>
          <div class="tech-value">${metadata.hash}</div>

          <div class="tech-label">${i18n.signatureLabel}</div>
          <div class="tech-value">${metadata.signature}</div>

          <div class="tech-label">${i18n.publicKeyLabel}</div>
          <div class="tech-value">MCowBQYDK2VwAyEAbKp2pg4wmzE5Kqo9tEwv7JJjxQyT2cBmwiLLHp4cSac=</div>
        </div>
      </div>

      <div class="section-card tint dense">
        <h2 class="section-title">${copy.perimeterLimitsTitle}</h2>
        <p class="plain-text">${i18n.limitationsText}</p>

        <div class="limit-grid">
          <div>
            <div class="limit-subtitle">${i18n.liabilityLabel}</div>
            <div class="plain-text">${i18n.liabilityText}</div>
          </div>
          <div>
            <div class="limit-subtitle">${i18n.validityPeriodLabel}</div>
            <div class="plain-text">${i18n.documentValidityText}</div>
          </div>
        </div>

        <div class="language-note">${copy.languageNotice}</div>
        <div class="doc-standard">${metadata.standardRef} · ${i18n.methodologyNote}</div>
      </div>
    </div>

    <div class="footer-static">
      <div>${i18n.footerText}</div>
      <div>${copy.footerPageLabel} 2 / 2</div>
    </div>
  </div>
</body>
</html>
`;

    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 15000);

    const pdfResponse = await fetch("https://api.pdfshift.io/v3/convert/pdf", {
      method: "POST",
      headers: {
        "X-API-Key": process.env.PDFSHIFT_API_KEY!,
        "Content-Type": "application/json",
      },
      body: JSON.stringify({
        source: html,
        format: "A4",
        use_print: true,
      }),
      signal: controller.signal,
    });

    clearTimeout(timeoutId);

    if (!pdfResponse.ok) {
      const error = await pdfResponse.text();
      return new Response(error, { status: pdfResponse.status });
    }

    const pdfBuffer = Buffer.from(await pdfResponse.arrayBuffer());

    const safeIssuerName = metadata.issuerName
      .toLowerCase()
      .replace(/[^a-z0-9\-]/g, "");

    return new Response(pdfBuffer, {
      headers: {
        "Content-Type": "application/pdf",
        "Content-Disposition": `attachment; filename="${safeIssuerName}-${metadata.attestationId}.pdf"`,
        "Cache-Control": "no-store",
        "X-Content-Type-Options": "nosniff",
      },
    });
  } catch (err) {
    if (process.env.NODE_ENV !== "production") {
      console.error(err);
    }

    return new Response("Internal Server Error", { status: 500 });
  }
    }
