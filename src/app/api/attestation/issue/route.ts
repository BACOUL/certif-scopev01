export const runtime = "nodejs";

import Stripe from "stripe";
import QRCode from "qrcode";
import { signCanonicalPayload, makeAttestationId } from "@/lib/sign";
import {
  ATTESTATION_I18N,
  AttestationLocale,
  DEFAULT_ATTESTATION_LOCALE,
} from "@/lib/attestation-i18n/index";

const stripe = new Stripe(process.env.STRIPE_SECRET_KEY!);

/**
 * Logo Certif-Scope en base64.
 *
 * IMPORTANT :
 * - Le logo est intégré directement dans le fichier.
 * - Ne mets PAS "data:image/png;base64,".
 * - Le .replace(/\s+/g, "") supprime automatiquement les retours à la ligne.
 */
const RAW_CERTIF_SCOPE_LOGO_BASE64 = `

`.replace(/\s+/g, "");

const CERTIF_SCOPE_LOGO_BASE64 = RAW_CERTIF_SCOPE_LOGO_BASE64;

const PUBLIC_KEY_BASE64 =
  "MCowBQYDK2VwAyEAbKp2pg4wmzE5Kqo9tEwv7JJjxQyT2cBmwiLLHp4cSac=";

type LocaleCopy = {
  eyebrow: string;
  mainTitle: string;
  mainSubtitle: string;
  standardLine: string;
  resultMainLabel: string;
  resultSubLabel: string;
  documentPanelTitle: string;
  referenceLabel: string;
  issueDateLabel: string;
  validUntilLabel: string;
  issuerLabel: string;
  foundationTitle: string;
  foundationText: string;
  useCaseTitle: string;
  useCaseText: string;
  confidentialityTitle: string;
  confidentialityItems: string[];
  authenticityTitle: string;
  authenticityText: string;
  validityTitle: string;
  validityText: string;
  methodologyTitle: string;
  methodologyFormula: string;
  factorVersionLabel: string;
  transferabilityLabel: string;
  coherenceTitle: string;
  coherenceIntro: string;
  coherenceItems: string[];
  coherenceDisclaimer: string;
  simpleVerificationTitle: string;
  simpleVerificationText: string;
  verificationPageLabel: string;
  objectTitle: string;
  objectText: string;
  technicalTitle: string;
  technicalIntro: string;
  algorithmLabel: string;
  hashLabel: string;
  signatureLabel: string;
  publicKeyLabel: string;
  limitsTitle: string;
  limitsText: string;
  dataResponsibilityTitle: string;
  dataResponsibilityText: string;
  documentValidityTitle: string;
  documentValidityText: string;
  languageClause: string;
  methodNote: string;
};

function getLocaleCopy(locale: AttestationLocale): LocaleCopy {
  if (locale === "de") {
    return {
      eyebrow: "CO₂e-Dokument · standardisiert · überprüfbar",
      mainTitle: "Indikative CO₂e-Bescheinigung",
      mainSubtitle:
        "Methodisches Dokument für eine erste dokumentarische CO₂e-Schätzung",
      standardLine:
        "Ausgestellt nach der standardisierten Certif-Scope-Methode CS-SB-v1",
      resultMainLabel: "Deklarierte aggregierte indikative Emissionen",
      resultSubLabel:
        "Indikative Schätzung auf Grundlage der vom Unternehmen deklarierten Daten.",
      documentPanelTitle: "Überprüfbares Dokument",
      referenceLabel: "Referenz",
      issueDateLabel: "Ausstellungsdatum",
      validUntilLabel: "Gültig bis",
      issuerLabel: "Aussteller",
      foundationTitle: "Methodische Grundlage",
      foundationText:
        "Diese Bescheinigung liefert eine indikative, standardisierte und überprüfbare CO₂e-Schätzung, die auf einer spend-based Methode mit aggregierten Ausgabendaten und monetären Emissionsfaktoren beruht.",
      useCaseTitle: "Vorgesehene Nutzung",
      useCaseText:
        "Diese Bescheinigung kann als indikatives Dokument für Lieferantendossiers, Kundenanfragen, Ausschreibungen, Bank- oder Versicherungsanfragen oder interne erste CO₂e-Dokumentation verwendet werden.",
      confidentialityTitle: "Vertraulichkeit",
      confidentialityItems: [
        "Keine Speicherung detaillierter Finanzdaten.",
        "Nur aggregiertes CO₂e-Ergebnis.",
        "Unabhängig überprüfbares Dokument.",
      ],
      authenticityTitle: "Authentizität",
      authenticityText:
        "Die Bescheinigung enthält eine eindeutige Referenz, einen QR-Code und technische Prüfelemente.",
      validityTitle: "Gültigkeit",
      validityText:
        "Die dokumentarische Gültigkeit ist zeitlich begrenzt und auf der Bescheinigung angegeben.",
      methodologyTitle: "Methodik",
      methodologyFormula:
        "Deklarierte aggregierte Ausgaben × monetäre Emissionsfaktoren = indikative CO₂e-Schätzung",
      factorVersionLabel: "Faktorversion",
      transferabilityLabel: "Übertragbarkeit",
      coherenceTitle: "Kohärenzreferenzen",
      coherenceIntro:
        "Die Certif-Scope-Methode steht in einer Kohärenzlogik mit folgenden Referenzrahmen für Kohlenstoff- und Umwelttransparenz:",
      coherenceItems: [
        "GHG Protocol — Scope 3, spend-based Methode",
        "ISO 14083 — Emissionen aus Transportoperationen",
        "EU-Taxonomie — Nachhaltigkeits- und Berichterstattungskontext",
        "ISO 14064-1 — Quantifizierung und Berichterstattung von Treibhausgasen",
        "CSRD / ESRS — europäische Umwelttransparenz",
      ],
      coherenceDisclaimer:
        "Diese Referenzen dienen der methodischen und kontextuellen Kohärenz. Sie verwandeln diese Bescheinigung nicht in einen regulatorischen Bericht, ein zertifiziertes Audit oder eine unabhängige Drittprüfung.",
      simpleVerificationTitle: "Einfache Überprüfung",
      simpleVerificationText:
        "Scannen Sie den QR-Code oder verwenden Sie die Bescheinigungsreferenz auf der offiziellen Verifizierungsseite.",
      verificationPageLabel: "Dokumentarische Verifizierungsseite",
      objectTitle: "Überprüfbares Objekt",
      objectText:
        "Das signierte PDF und sein QR-Code bilden die dokumentarischen Prüfelemente.",
      technicalTitle: "Technische Prüfelemente",
      technicalIntro:
        "Die folgenden Elemente ermöglichen eine erweiterte dokumentarische Überprüfung. Für einen Standardleser ist keine Aktion erforderlich.",
      algorithmLabel: "Algorithmus",
      hashLabel: "Hash des signierten Inhalts (SHA-256)",
      signatureLabel: "Signatur (Base64)",
      publicKeyLabel: "Öffentlicher Prüfschlüssel des Ausstellers",
      limitsTitle: "Umfang und Grenzen",
      limitsText:
        "Diese Bescheinigung ist indikativ. Sie beruht auf vom Unternehmen deklarierten Daten und einer spend-based Methode. Sie ersetzt kein vollständiges GHG-Inventar, kein Carbon-Audit, keine unabhängige Drittprüfung, keine Bilan Carbone®-Leistung und keine regulatorische Berichterstattung, wenn diese ausdrücklich verlangt wird.",
      dataResponsibilityTitle: "Verantwortung für Daten",
      dataResponsibilityText:
        "Die Ergebnisse hängen von Genauigkeit, Vollständigkeit und Kohärenz der vom Unternehmen deklarierten Daten ab.",
      documentValidityTitle: "Dokumentarische Gültigkeit",
      documentValidityText:
        "Die Gültigkeitsdauer spiegelt die zeitliche Relevanz der deklarierten Daten, der verwendeten Faktoren und der angewandten Methode wider.",
      languageClause:
        "Bei Abweichungen zwischen Sprachversionen ist die englische Version maßgeblich.",
      methodNote:
        "CS-SB-v1 ist eine interne standardisierte Methode, die von Certif-Scope gepflegt wird.",
    };
  }

  if (locale === "fr") {
    return {
      eyebrow: "Document CO₂e indicatif · standardisé · vérifiable",
      mainTitle: "Attestation indicative d’émissions de carbone",
      mainSubtitle:
        "Document méthodologique pour première estimation carbone documentaire",
      standardLine:
        "Émise conformément à la méthodologie standardisée Certif-Scope CS-SB-v1",
      resultMainLabel: "Émissions indicatives agrégées déclarées",
      resultSubLabel:
        "Estimation indicative issue des données déclarées par l’entité.",
      documentPanelTitle: "Document vérifiable",
      referenceLabel: "Référence",
      issueDateLabel: "Date d’émission",
      validUntilLabel: "Valable jusqu’au",
      issuerLabel: "Émetteur",
      foundationTitle: "Fondement méthodologique",
      foundationText:
        "Cette attestation fournit une estimation CO₂e indicative, standardisée et vérifiable, fondée sur une méthodologie spend-based utilisant des données de dépenses agrégées déclarées et des facteurs d’émission monétaires.",
      useCaseTitle: "Usage prévu",
      useCaseText:
        "Cette attestation peut être utilisée comme document indicatif dans un dossier fournisseur, une demande client, un appel d’offres, une demande bancaire, une demande d’assurance ou une démarche interne de première estimation carbone, lorsque le besoin porte sur un indicateur CO₂e agrégé et documenté.",
      confidentialityTitle: "Confidentialité",
      confidentialityItems: [
        "Aucune conservation des données financières détaillées.",
        "Résultat CO₂e agrégé uniquement.",
        "Attestation vérifiable indépendamment.",
      ],
      authenticityTitle: "Authenticité",
      authenticityText:
        "L’attestation comporte une référence unique, un QR code et des éléments de contrôle documentaire.",
      validityTitle: "Validité",
      validityText:
        "La validité documentaire est limitée dans le temps et indiquée sur l’attestation.",
      methodologyTitle: "Méthodologie",
      methodologyFormula:
        "Dépenses agrégées déclarées × facteurs d’émission monétaires = estimation CO₂e indicative",
      factorVersionLabel: "Version des facteurs",
      transferabilityLabel: "Transférabilité",
      coherenceTitle: "Références de cohérence",
      coherenceIntro:
        "La méthodologie Certif-Scope s’inscrit dans une logique de cohérence avec les cadres de référence carbone et de transparence environnementale suivants :",
      coherenceItems: [
        "GHG Protocol — Scope 3, méthode spend-based",
        "ISO 14083 — émissions liées aux opérations de transport",
        "Taxonomie européenne — contexte de durabilité et reporting extra-financier",
        "ISO 14064-1 — quantification et reporting GES",
        "CSRD / ESRS — transparence environnementale européenne",
      ],
      coherenceDisclaimer:
        "Ces références sont citées comme cadre de cohérence méthodologique et contextuel. Elles ne transforment pas cette attestation en rapport réglementaire, audit certifié ou vérification par tiers indépendant.",
      simpleVerificationTitle: "Vérification simple",
      simpleVerificationText:
        "Scanner le QR code ou utiliser la référence d’attestation sur la page officielle de vérification.",
      verificationPageLabel: "Page de vérification documentaire",
      objectTitle: "Objet vérifiable",
      objectText:
        "Le PDF signé et son QR code constituent les éléments de contrôle documentaire.",
      technicalTitle: "Éléments techniques de vérification",
      technicalIntro:
        "Les éléments ci-dessous permettent une vérification documentaire avancée. Aucune action n’est requise pour une lecture standard du document.",
      algorithmLabel: "Algorithme",
      hashLabel: "Empreinte du contenu signé (SHA-256)",
      signatureLabel: "Signature (Base64)",
      publicKeyLabel: "Clé publique de vérification de l’émetteur",
      limitsTitle: "Périmètre et limites",
      limitsText:
        "Cette attestation est indicative. Elle repose sur les données déclarées par l’entité et sur une méthodologie spend-based. Elle ne remplace pas un inventaire GES complet, un audit carbone, une vérification par tiers indépendant, un Bilan Carbone® ou un reporting réglementaire lorsque ceux-ci sont explicitement exigés.",
      dataResponsibilityTitle: "Responsabilité des données",
      dataResponsibilityText:
        "Les résultats dépendent de l’exactitude, de la complétude et de la cohérence des données déclarées par l’entité.",
      documentValidityTitle: "Validité documentaire",
      documentValidityText:
        "La période de validité reflète la pertinence temporelle des données déclarées, des facteurs d’émission utilisés et de la méthodologie appliquée.",
      languageClause:
        "En cas de divergence entre les versions linguistiques, la version anglaise prévaut.",
      methodNote:
        "CS-SB-v1 est une méthodologie standardisée interne maintenue par Certif-Scope.",
    };
  }

  return {
    eyebrow: "Indicative CO₂e document · standardized · verifiable",
    mainTitle: "Indicative carbon emissions attestation",
    mainSubtitle:
      "Methodological document for a first documentary carbon estimate",
    standardLine:
      "Issued under the standardized Certif-Scope methodology CS-SB-v1",
    resultMainLabel: "Declared aggregated indicative emissions",
    resultSubLabel:
      "Indicative estimate based on data declared by the entity.",
    documentPanelTitle: "Verifiable document",
    referenceLabel: "Reference",
    issueDateLabel: "Issue date",
    validUntilLabel: "Valid until",
    issuerLabel: "Issuer",
    foundationTitle: "Methodological basis",
    foundationText:
      "This attestation provides an indicative, standardized and verifiable CO₂e estimate based on a spend-based methodology using declared aggregated expenditure data and monetary emission factors.",
    useCaseTitle: "Intended use",
    useCaseText:
      "This attestation may be used as an indicative document for a supplier file, client request, tender, banking request, insurance request or internal first carbon estimate when the need concerns an aggregated and documented CO₂e indicator.",
    confidentialityTitle: "Confidentiality",
    confidentialityItems: [
      "No storage of detailed financial data.",
      "Aggregated CO₂e result only.",
      "Independently verifiable attestation.",
    ],
    authenticityTitle: "Authenticity",
    authenticityText:
      "The attestation includes a unique reference, a QR code and documentary control elements.",
    validityTitle: "Validity",
    validityText:
      "The documentary validity is time-limited and stated on the attestation.",
    methodologyTitle: "Methodology",
    methodologyFormula:
      "Declared aggregated expenditure × monetary emission factors = indicative CO₂e estimate",
    factorVersionLabel: "Factor version",
    transferabilityLabel: "Transferability",
    coherenceTitle: "Coherence references",
    coherenceIntro:
      "The Certif-Scope methodology follows a coherence approach with the following carbon and environmental transparency reference frameworks:",
    coherenceItems: [
      "GHG Protocol — Scope 3, spend-based method",
      "ISO 14083 — emissions from transport operations",
      "EU Taxonomy — sustainability and extra-financial reporting context",
      "ISO 14064-1 — GHG quantification and reporting",
      "CSRD / ESRS — European environmental transparency",
    ],
    coherenceDisclaimer:
      "These references are cited as methodological and contextual coherence frameworks. They do not turn this attestation into a regulatory report, certified audit or independent third-party verification.",
    simpleVerificationTitle: "Simple verification",
    simpleVerificationText:
      "Scan the QR code or use the attestation reference on the official verification page.",
    verificationPageLabel: "Documentary verification page",
    objectTitle: "Verifiable object",
    objectText:
      "The signed PDF and its QR code constitute the documentary control elements.",
    technicalTitle: "Technical verification elements",
    technicalIntro:
      "The elements below allow advanced documentary verification. No action is required for a standard reading of the document.",
    algorithmLabel: "Algorithm",
    hashLabel: "Signed content hash (SHA-256)",
    signatureLabel: "Signature (Base64)",
    publicKeyLabel: "Issuer public verification key",
    limitsTitle: "Scope and limits",
    limitsText:
      "This attestation is indicative. It relies on data declared by the entity and on a spend-based methodology. It does not replace a full GHG inventory, carbon audit, independent third-party verification, Bilan Carbone® engagement or regulatory reporting when such elements are explicitly required.",
    dataResponsibilityTitle: "Data responsibility",
    dataResponsibilityText:
      "Results depend on the accuracy, completeness and consistency of the data declared by the entity.",
    documentValidityTitle: "Document validity",
    documentValidityText:
      "The validity period reflects the time relevance of the declared data, the emission factors used and the methodology applied.",
    languageClause:
      "In case of discrepancy between language versions, the English version shall prevail.",
    methodNote:
      "CS-SB-v1 is an internal standardized methodology maintained by Certif-Scope.",
  };
}

function escapeHtml(input: string) {
  return input
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
    .replace(/"/g, "&quot;")
    .replace(/'/g, "&#39;");
}

function toBase64Url(input: string) {
  return Buffer.from(input, "utf8")
    .toString("base64")
    .replace(/\+/g, "-")
    .replace(/\//g, "_")
    .replace(/=+$/g, "");
}

function addMonthsToISODate(date: string, months: number) {
  const parsed = new Date(`${date}T00:00:00.000Z`);

  if (Number.isNaN(parsed.getTime())) {
    return "";
  }

  parsed.setUTCMonth(parsed.getUTCMonth() + months);

  return parsed.toISOString().slice(0, 10);
}

function formatCO2Value(value: number, locale: AttestationLocale) {
  const normalizedLocale =
    locale === "fr" ? "fr-FR" : locale === "de" ? "de-DE" : "en-US";

  return new Intl.NumberFormat(normalizedLocale, {
    minimumFractionDigits: value % 1 === 0 ? 0 : 1,
    maximumFractionDigits: 2,
  }).format(value);
}

function safeText(value: unknown, fallback = "—") {
  const text = String(value ?? "").trim();
  return text ? text : fallback;
}

function removeLeadingSectionNumber(value: string, sectionNumber: number) {
  return value
    .replace(new RegExp(`^${sectionNumber}\\.\\s*${sectionNumber}\\.\\s*`, "u"), "")
    .replace(new RegExp(`^${sectionNumber}\\.\\s*`, "u"), "")
    .trim();
}

export async function GET(req: Request) {
  try {
    if (!process.env.PDFSHIFT_API_KEY) {
      return new Response("PDFSHIFT_API_KEY missing", { status: 500 });
    }

    const { searchParams } = new URL(req.url);
    const sessionId = searchParams.get("session_id");

    if (!sessionId) {
      return new Response("Missing session_id", { status: 400 });
    }

    let metadataRaw: Record<string, any> = {};

    if (sessionId.startsWith("key_")) {
      metadataRaw = Object.fromEntries(searchParams.entries());
    } else {
      const session = await stripe.checkout.sessions.retrieve(sessionId);

      if (session.payment_status !== "paid") {
        return new Response("Payment not completed", { status: 403 });
      }

      metadataRaw = session.metadata || {};
    }

    const locale =
      (metadataRaw.attestationLocale as AttestationLocale) ||
      DEFAULT_ATTESTATION_LOCALE;

    const i18n = ATTESTATION_I18N[locale] || ATTESTATION_I18N.en;
    const copy = getLocaleCopy(locale);

    const required = ["companyName", "totalCO2e", "year"];
    const missing = required.filter((k) => {
      const v = metadataRaw[k];
      return v === undefined || v === null || String(v).trim() === "";
    });

    if (missing.length) {
      return new Response(`Missing metadata: ${missing.join(", ")}`, {
        status: 400,
      });
    }

    const totalCO2eNum = Number(String(metadataRaw.totalCO2e).replace(",", "."));

    if (Number.isNaN(totalCO2eNum)) {
      return new Response("Invalid metadata: totalCO2e must be a number", {
        status: 400,
      });
    }

    const issuedDate = new Date().toISOString();

    const canonicalPayload = {
      issuer: "Certif-Scope" as const,
      standard: "CS-SB-v1" as const,
      attestationId: "",
      companyName: String(metadataRaw.companyName),
      country: String(metadataRaw.country || "—"),
      year: String(metadataRaw.year),
      totalCO2e: String(totalCO2eNum),
      issuedDate,
    };

    const tempSignature = signCanonicalPayload({
      ...canonicalPayload,
      attestationId: "TEMP",
    });

    const attestationId = makeAttestationId(
      canonicalPayload.year,
      tempSignature.hashHex
    );

    const signatureResult = signCanonicalPayload({
      ...canonicalPayload,
      attestationId,
    });

    const issuedDateForDisplay = issuedDate.slice(0, 10);
    const validityMonths = String(metadataRaw.validityMonths || "12");
    const validUntil =
      String(metadataRaw.validUntil || "").trim() ||
      addMonthsToISODate(issuedDateForDisplay, Number(validityMonths) || 12);

    const factorVersion = String(
      metadataRaw.factorVersion ||
        metadataRaw.emissionFactorVersion ||
        "Certif-Scope factors v1"
    );

    const metadata = {
      attestationId,
      issuerName: escapeHtml(safeText(metadataRaw.issuerName, "Certif-Scope")),
      issuerSite: escapeHtml(
        safeText(metadataRaw.issuerSite, "https://www.certif-scope.com")
      ),
      companyName: escapeHtml(safeText(metadataRaw.companyName)),
      companySector: escapeHtml(safeText(metadataRaw.companySector)),
      entityIdentifier: escapeHtml(safeText(metadataRaw.entityIdentifier)),
      country: escapeHtml(safeText(metadataRaw.country)),
      year: escapeHtml(safeText(metadataRaw.year)),
      totalCO2e: escapeHtml(formatCO2Value(totalCO2eNum, locale)),
      totalCO2eRaw: String(totalCO2eNum),
      methodology: escapeHtml(
        safeText(
          metadataRaw.methodology,
          "Certif-Scope deterministic spend-based methodology v1.0"
        )
      ),
      issuedDate: escapeHtml(issuedDateForDisplay),
      validUntil: escapeHtml(validUntil),
      validityMonths: escapeHtml(validityMonths),
      standardRef: escapeHtml(safeText(metadataRaw.standardRef, "CS-SB-v1")),
      factorVersion: escapeHtml(factorVersion),
      signature: signatureResult.signatureBase64,
      hash: signatureResult.hashHex,
      algorithm: signatureResult.algorithm,
    };

    const verificationPayload = {
      certificateId: metadata.attestationId,
      issuer: "Certif-Scope",
      issuedAt: metadata.issuedDate,
      validUntil: metadata.validUntil,
      methodVersion: "CS-SB-v1",
      factorVersion,
      algorithm: metadata.algorithm,
      hash: metadata.hash,
      signature: metadata.signature,
    };

    const verificationToken = toBase64Url(JSON.stringify(verificationPayload));

    const verifyUrl =
      locale === "fr"
        ? `https://www.certif-scope.com/fr/verify/?v=${verificationToken}`
        : `https://www.certif-scope.com/verify/?v=${verificationToken}`;

    const verificationDisplayUrl =
      locale === "fr"
        ? "https://www.certif-scope.com/fr/verify"
        : "https://www.certif-scope.com/verify";

    const qrDataUrl = await QRCode.toDataURL(verifyUrl, {
      width: 150,
      margin: 1,
    });

    const logoHtml = CERTIF_SCOPE_LOGO_BASE64
      ? `<img src="data:image/png;base64,${CERTIF_SCOPE_LOGO_BASE64}" class="issuer-logo" alt="Certif-Scope logo" />`
      : `<div class="issuer-logo-fallback">
          <span class="mark">✓</span>
          <span>Certif-Scope</span>
        </div>`;

    const html = `
<!doctype html>
<html lang="${locale}">
<head>
<meta charset="utf-8"/>
<title>${metadata.issuerName} — Attestation</title>
<style>
  @page {
    size: A4;
    margin: 10mm 10mm 10mm 10mm;
  }

  * {
    box-sizing: border-box;
  }

  html,
  body {
    margin: 0;
    padding: 0;
  }

  body {
    font-family: Inter, "Helvetica Neue", Arial, Helvetica, sans-serif;
    font-size: 8.7px;
    line-height: 1.28;
    color: #111827;
    background: #ffffff;
    -webkit-font-smoothing: antialiased;
  }

  :root {
    --blue: #0B3A63;
    --blue-dark: #082F52;
    --turquoise: #1FB6C1;
    --soft: #F8FAFC;
    --soft-blue: #EEF7F9;
    --border: #D7E1EA;
    --muted: #64748B;
    --text: #111827;
  }

  .page {
    width: 100%;
    min-height: 277mm;
    page-break-after: always;
    position: relative;
    padding: 0;
  }

  .page:last-child {
    page-break-after: auto;
  }

  .top-header {
    display: flex;
    justify-content: space-between;
    align-items: flex-start;
    gap: 12px;
    padding-bottom: 8px;
    border-bottom: 3px solid var(--blue);
  }

  .issuer-block {
    display: flex;
    flex-direction: column;
    gap: 3px;
  }

  .issuer-logo {
    display: block;
    width: auto;
    max-width: 145px;
    max-height: 34px;
    object-fit: contain;
    object-position: left center;
  }

  .issuer-logo-fallback {
    display: flex;
    align-items: center;
    gap: 6px;
    color: var(--blue);
    font-size: 12px;
    font-weight: 800;
  }

  .issuer-logo-fallback .mark {
    width: 22px;
    height: 22px;
    border: 2px solid var(--blue);
    border-radius: 999px;
    display: inline-flex;
    align-items: center;
    justify-content: center;
    color: var(--turquoise);
    font-weight: 900;
  }

  .issuer-site {
    color: var(--muted);
    font-size: 7.2px;
  }

  .issuer-meta {
    color: var(--muted);
    font-size: 7.2px;
  }

  .qr-block {
    text-align: center;
    color: var(--blue);
    font-size: 6.8px;
    font-weight: 700;
  }

  .qr-block img {
    width: 70px;
    height: 70px;
    border: 1px solid var(--border);
    padding: 3px;
    background: #ffffff;
    display: block;
  }

  .hero {
    text-align: center;
    margin: 8px 0 8px;
  }

  .eyebrow {
    color: var(--turquoise);
    font-size: 7px;
    letter-spacing: 1.5px;
    text-transform: uppercase;
    font-weight: 800;
    margin-bottom: 3px;
  }

  .hero h1 {
    margin: 0;
    color: var(--blue);
    font-size: 20px;
    line-height: 1.05;
    text-transform: uppercase;
    letter-spacing: 0.4px;
    font-weight: 900;
  }

  .standard-line {
    margin-top: 4px;
    color: #111827;
    font-size: 8px;
    font-weight: 800;
  }

  .subtitle {
    margin-top: 3px;
    color: var(--muted);
    font-size: 7.5px;
  }

  .result-card {
    width: 77%;
    margin: 8px auto 8px;
    border: 2px solid var(--blue);
    border-radius: 5px;
    padding: 9px 10px;
    text-align: center;
  }

  .result-label {
    color: var(--blue);
    text-transform: uppercase;
    letter-spacing: 0.8px;
    font-size: 7.5px;
    font-weight: 900;
  }

  .result-value {
    margin-top: 3px;
    font-family: Georgia, "Times New Roman", serif;
    color: var(--blue);
    font-size: 24px;
    font-weight: 900;
    line-height: 1;
  }

  .result-value small {
    color: var(--turquoise);
    font-size: 11px;
    font-family: Inter, Arial, sans-serif;
    font-weight: 900;
  }

  .result-note {
    margin-top: 4px;
    color: var(--muted);
    font-size: 7.3px;
  }

  .doc-panel {
    border: 1px solid var(--border);
    border-radius: 5px;
    overflow: hidden;
    margin-bottom: 8px;
  }

  .doc-panel-title {
    background: linear-gradient(90deg, #EEF9FA 0%, #F8FAFC 100%);
    border-bottom: 1px solid var(--border);
    color: var(--blue);
    font-size: 7.5px;
    text-transform: uppercase;
    letter-spacing: 0.7px;
    font-weight: 900;
    padding: 5px 8px;
  }

  .doc-grid {
    display: grid;
    grid-template-columns: 1.2fr 1fr 1fr 0.9fr;
  }

  .doc-cell {
    padding: 7px 8px;
    border-right: 1px solid var(--border);
    min-height: 34px;
  }

  .doc-cell:last-child {
    border-right: 0;
  }

  .cell-label {
    color: var(--muted);
    text-transform: uppercase;
    letter-spacing: 0.5px;
    font-size: 6.5px;
    margin-bottom: 3px;
  }

  .cell-value {
    color: var(--text);
    font-size: 8px;
    font-weight: 800;
    word-break: break-word;
  }

  .highlight-box {
    border: 1px solid #CFE7E9;
    background: #F0FBFC;
    border-left: 4px solid var(--turquoise);
    border-radius: 5px;
    padding: 7px 9px;
    margin-bottom: 7px;
  }

  .plain-box {
    border: 1px solid var(--border);
    background: #ffffff;
    border-radius: 5px;
    padding: 7px 9px;
    margin-bottom: 7px;
  }

  .box-title {
    color: var(--blue);
    font-size: 8px;
    text-transform: uppercase;
    letter-spacing: 0.5px;
    font-weight: 900;
    margin-bottom: 3px;
  }

  .box-text {
    color: var(--text);
    font-size: 8px;
  }

  .entity-grid {
    border: 1px solid var(--border);
    border-radius: 5px;
    padding: 8px;
    margin-bottom: 7px;
  }

  .section-number-title {
    color: var(--blue);
    font-size: 8.5px;
    font-weight: 900;
    text-transform: uppercase;
    letter-spacing: 0.4px;
    margin-bottom: 6px;
  }

  .entity-fields {
    display: grid;
    grid-template-columns: 1.1fr 0.65fr 0.65fr;
    gap: 7px 14px;
  }

  .field-label {
    color: var(--muted);
    font-size: 6.5px;
    text-transform: uppercase;
    letter-spacing: 0.5px;
    margin-bottom: 2px;
  }

  .field-value {
    color: var(--text);
    font-size: 8px;
    font-weight: 800;
    word-break: break-word;
  }

  .two-cards {
    display: grid;
    grid-template-columns: 1fr 1fr;
    gap: 7px;
    margin-bottom: 7px;
  }

  .small-card {
    border: 1px solid var(--border);
    border-radius: 5px;
    padding: 7px 8px;
    min-height: 58px;
  }

  .mini-cards {
    display: grid;
    grid-template-columns: 1fr 1fr 1fr;
    gap: 7px;
    margin-bottom: 8px;
  }

  .mini-card {
    border: 1px solid var(--border);
    border-radius: 5px;
    background: var(--soft);
    padding: 7px 8px;
    min-height: 55px;
  }

  .mini-card ul {
    margin: 2px 0 0 11px;
    padding: 0;
  }

  .mini-card li {
    margin-bottom: 1px;
  }

  .page-title-row {
    display: flex;
    justify-content: space-between;
    align-items: flex-start;
    border-bottom: 3px solid var(--blue);
    padding-bottom: 6px;
    margin-bottom: 10px;
  }

  .page-title-row h2 {
    margin: 0;
    color: var(--blue);
    font-size: 18px;
    text-transform: uppercase;
    letter-spacing: 0.4px;
    font-weight: 900;
  }

  .page-ref {
    color: var(--muted);
    font-size: 7px;
    text-align: right;
    line-height: 1.3;
  }

  .method-card {
    border: 1px solid var(--border);
    border-radius: 5px;
    padding: 9px;
    margin-bottom: 8px;
  }

  .formula {
    background: #EEF9FA;
    border: 1px solid #CFE7E9;
    border-radius: 4px;
    padding: 8px;
    text-align: center;
    color: var(--blue);
    font-weight: 900;
    margin: 7px 0;
  }

  .mini-grid {
    display: grid;
    grid-template-columns: 1fr 1fr;
    gap: 7px;
  }

  .mini-pill {
    background: var(--soft);
    border: 1px solid var(--border);
    border-radius: 4px;
    padding: 7px;
  }

  .mini-label {
    display: block;
    color: var(--muted);
    text-transform: uppercase;
    font-size: 6.4px;
    letter-spacing: 0.5px;
    margin-bottom: 2px;
  }

  .mini-value {
    display: block;
    color: var(--text);
    font-weight: 800;
    font-size: 7.8px;
  }

  .coherence-card {
    border: 1px solid var(--border);
    border-radius: 5px;
    padding: 9px;
    margin-bottom: 8px;
  }

  .coherence-grid {
    display: grid;
    grid-template-columns: 1fr 1fr;
    gap: 0 18px;
    margin: 6px 0;
  }

  .coherence-grid ul {
    margin: 0;
    padding-left: 13px;
  }

  .coherence-grid li {
    margin-bottom: 2px;
  }

  .verification-grid {
    display: grid;
    grid-template-columns: 1fr 1fr;
    gap: 7px;
    margin-bottom: 8px;
  }

  .verification-simple {
    border: 1px solid #CFE7E9;
    border-left: 4px solid var(--turquoise);
    border-radius: 5px;
    background: #F0FBFC;
    padding: 8px 9px;
  }

  .verification-object {
    border: 1px solid var(--border);
    border-radius: 5px;
    background: var(--soft);
    padding: 8px 9px;
  }

  .technical-card {
    border: 1px solid var(--border);
    border-radius: 5px;
    padding: 8px 9px;
    margin-bottom: 8px;
  }

  .tech-table {
    border-top: 1px solid var(--border);
    margin-top: 6px;
    padding-top: 6px;
  }

  .tech-row {
    display: grid;
    grid-template-columns: 185px 1fr;
    gap: 8px;
    margin-bottom: 3px;
  }

  .tech-label {
    color: var(--muted);
    text-transform: uppercase;
    font-size: 6.5px;
    letter-spacing: 0.4px;
  }

  .tech-value {
    color: var(--text);
    font-size: 6.8px;
    word-break: break-all;
  }

  .limits-card {
    border: 1px solid #CFE7E9;
    border-left: 4px solid var(--blue);
    border-radius: 5px;
    background: #F7FCFD;
    padding: 9px;
  }

  .limits-subgrid {
    display: grid;
    grid-template-columns: 1fr 1fr;
    gap: 7px;
    margin-top: 7px;
  }

  .limits-subbox {
    background: #ffffff;
    border: 1px solid var(--border);
    border-radius: 4px;
    padding: 7px;
  }

  .footer {
    position: absolute;
    left: 0;
    right: 0;
    bottom: 0;
    border-top: 1px solid var(--border);
    padding-top: 5px;
    color: var(--muted);
    font-size: 6.8px;
    display: flex;
    justify-content: space-between;
  }

  .muted {
    color: var(--muted);
  }

  .strong-blue {
    color: var(--blue);
    font-weight: 900;
  }
</style>
</head>
<body>
  <div class="page">
    <header class="top-header">
      <div class="issuer-block">
        ${logoHtml}
        <div class="issuer-site">${metadata.issuerSite}</div>
        <div class="issuer-meta">${i18n.headerTagline}</div>
      </div>

      <div class="qr-block">
        <img src="${qrDataUrl}" alt="QR verification" />
        <div>${i18n.scanToVerifyLabel}</div>
      </div>
    </header>

    <section class="hero">
      <div class="eyebrow">${copy.eyebrow}</div>
      <h1>${copy.mainTitle}</h1>
      <div class="standard-line">${copy.standardLine}</div>
      <div class="subtitle">${i18n.subtitle} · ${copy.mainSubtitle} · Certif-Scope ${metadata.standardRef}</div>
    </section>

    <section class="result-card">
      <div class="result-label">${copy.resultMainLabel}</div>
      <div class="result-value">${metadata.totalCO2e} <small>tCO₂e</small></div>
      <div class="result-note">${copy.resultSubLabel}</div>
    </section>

    <section class="doc-panel">
      <div class="doc-panel-title">${copy.documentPanelTitle}</div>
      <div class="doc-grid">
        <div class="doc-cell">
          <div class="cell-label">${copy.referenceLabel}</div>
          <div class="cell-value">${metadata.attestationId}</div>
        </div>
        <div class="doc-cell">
          <div class="cell-label">${copy.issueDateLabel}</div>
          <div class="cell-value">${metadata.issuedDate}</div>
        </div>
        <div class="doc-cell">
          <div class="cell-label">${copy.validUntilLabel}</div>
          <div class="cell-value">${metadata.validUntil}</div>
        </div>
        <div class="doc-cell">
          <div class="cell-label">${copy.issuerLabel}</div>
          <div class="cell-value">${metadata.issuerName}</div>
        </div>
      </div>
    </section>

    <section class="highlight-box">
      <div class="box-title">${copy.foundationTitle}</div>
      <div class="box-text">${copy.foundationText}</div>
    </section>

    <section class="plain-box">
      <div class="box-title">${copy.useCaseTitle}</div>
      <div class="box-text">${copy.useCaseText}</div>
    </section>

    <section class="entity-grid">
      <div class="section-number-title">3. ${removeLeadingSectionNumber(
        i18n.entitySectionTitle,
        3
      )}</div>
      <div class="entity-fields">
        <div>
          <div class="field-label">${i18n.entityNameLabel}</div>
          <div class="field-value">${metadata.companyName}</div>
        </div>
        <div>
          <div class="field-label">${i18n.countryLabel}</div>
          <div class="field-value">${metadata.country}</div>
        </div>
        <div>
          <div class="field-label">${i18n.reportingYearLabel}</div>
          <div class="field-value">${metadata.year}</div>
        </div>
        <div>
          <div class="field-label">${i18n.activitySectorLabel}</div>
          <div class="field-value">${metadata.companySector}</div>
        </div>
        <div>
          <div class="field-label">${i18n.entityIdentifierLabel}</div>
          <div class="field-value">${metadata.entityIdentifier}</div>
        </div>
        <div>
          <div class="field-label">${i18n.uniqueIdentifierLabel}</div>
          <div class="field-value">${metadata.attestationId}</div>
        </div>
      </div>
    </section>

    <section class="two-cards">
      <div class="small-card">
        <div class="box-title">2. ${removeLeadingSectionNumber(
          i18n.documentNatureSectionTitle,
          2
        )}</div>
        <div class="box-text"><strong>${i18n.documentNatureText}</strong></div>
      </div>
      <div class="small-card">
        <div class="box-title">4. ${removeLeadingSectionNumber(
          i18n.scopeSectionTitle,
          4
        )}</div>
        <div class="box-text"><strong>${i18n.scopeDescriptionLabel} :</strong> ${i18n.scopeText}</div>
      </div>
    </section>

    <section class="mini-cards">
      <div class="mini-card">
        <div class="box-title">${copy.confidentialityTitle}</div>
        <ul>
          ${copy.confidentialityItems.map((item) => `<li>${item}</li>`).join("")}
        </ul>
      </div>
      <div class="mini-card">
        <div class="box-title">${copy.authenticityTitle}</div>
        <div class="box-text">${copy.authenticityText}</div>
      </div>
      <div class="mini-card">
        <div class="box-title">${copy.validityTitle}</div>
        <div class="box-text">${copy.validityText}</div>
      </div>
    </section>

    <div class="footer">
      <div>${i18n.footerText}</div>
      <div>${i18n.pageLabel} 1 / 2</div>
    </div>
  </div>

  <div class="page">
    <section class="page-title-row">
      <h2>7. ${removeLeadingSectionNumber(i18n.methodologySectionTitle, 7)}</h2>
      <div class="page-ref">
        ${copy.referenceLabel} : ${metadata.attestationId}<br />
        ${copy.issuerLabel} : ${metadata.issuerName}
      </div>
    </section>

    <section class="method-card">
      <div class="box-title">${copy.methodologyTitle}</div>
      <div class="box-text"><strong>${metadata.methodology}</strong></div>

      <div class="formula">${copy.methodologyFormula}</div>

      <div class="mini-grid">
        <div class="mini-pill">
          <span class="mini-label">${copy.factorVersionLabel}</span>
          <span class="mini-value">${metadata.factorVersion}</span>
        </div>
        <div class="mini-pill">
          <span class="mini-label">${copy.transferabilityLabel}</span>
          <span class="mini-value">${i18n.transferabilityText}</span>
        </div>
      </div>
    </section>

    <section class="coherence-card">
      <div class="box-title">${copy.coherenceTitle}</div>
      <div class="box-text">${copy.coherenceIntro}</div>

      <div class="coherence-grid">
        <ul>
          ${copy.coherenceItems
            .slice(0, 3)
            .map((item) => `<li>${item}</li>`)
            .join("")}
        </ul>
        <ul>
          ${copy.coherenceItems
            .slice(3)
            .map((item) => `<li>${item}</li>`)
            .join("")}
        </ul>
      </div>

      <div class="muted">${copy.coherenceDisclaimer}</div>
    </section>

    <section class="verification-grid">
      <div class="verification-simple">
        <div class="box-title">${copy.simpleVerificationTitle}</div>
        <div class="box-text">${copy.simpleVerificationText}</div>
        <div style="margin-top: 6px;">
          <strong>${copy.verificationPageLabel}</strong><br />
          <span class="strong-blue">${verificationDisplayUrl}</span><br />
          <span class="muted">${copy.referenceLabel} : ${metadata.attestationId}</span>
        </div>
      </div>

      <div class="verification-object">
        <div class="box-title">${copy.objectTitle}</div>
        <div class="box-text">${copy.objectText}</div>
      </div>
    </section>

    <section class="technical-card">
      <div class="box-title">${copy.technicalTitle}</div>
      <div class="muted">${copy.technicalIntro}</div>

      <div class="tech-table">
        <div class="tech-row">
          <div class="tech-label">${copy.algorithmLabel}</div>
          <div class="tech-value">${metadata.algorithm}</div>
        </div>
        <div class="tech-row">
          <div class="tech-label">${copy.hashLabel}</div>
          <div class="tech-value">${metadata.hash}</div>
        </div>
        <div class="tech-row">
          <div class="tech-label">${copy.signatureLabel}</div>
          <div class="tech-value">${metadata.signature}</div>
        </div>
        <div class="tech-row">
          <div class="tech-label">${copy.publicKeyLabel}</div>
          <div class="tech-value">${PUBLIC_KEY_BASE64}</div>
        </div>
      </div>
    </section>

    <section class="limits-card">
      <div class="box-title">${copy.limitsTitle}</div>
      <div class="box-text">${copy.limitsText}</div>

      <div class="limits-subgrid">
        <div class="limits-subbox">
          <div class="box-title">${copy.dataResponsibilityTitle}</div>
          <div class="box-text">${copy.dataResponsibilityText}</div>
        </div>
        <div class="limits-subbox">
          <div class="box-title">${copy.documentValidityTitle}</div>
          <div class="box-text">${copy.documentValidityText}</div>
        </div>
      </div>

      <div style="margin-top: 7px;">
        <strong>${copy.languageClause}</strong><br />
        <span class="muted">${copy.methodNote}</span>
      </div>
    </section>

    <div class="footer">
      <div>${i18n.footerText}</div>
      <div>${i18n.pageLabel} 2 / 2</div>
    </div>
  </div>
</body>
</html>
`;

    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 15000);

    const pdfResponse = await fetch("https://api.pdfshift.io/v3/convert/pdf", {
      method: "POST",
      headers: {
        "X-API-Key": process.env.PDFSHIFT_API_KEY!,
        "Content-Type": "application/json",
      },
      body: JSON.stringify({
        source: html,
        format: "A4",
        use_print: true,
      }),
      signal: controller.signal,
    });

    clearTimeout(timeoutId);

    if (!pdfResponse.ok) {
      const error = await pdfResponse.text();
      return new Response(error, { status: pdfResponse.status });
    }

    const pdfBuffer = Buffer.from(await pdfResponse.arrayBuffer());

    const safeIssuerName = metadata.issuerName
      .toLowerCase()
      .replace(/[^a-z0-9\-]/g, "");

    return new Response(pdfBuffer, {
      headers: {
        "Content-Type": "application/pdf",
        "Content-Disposition": `attachment; filename="${safeIssuerName}-${metadata.attestationId}.pdf"`,
        "Cache-Control": "no-store",
        "X-Content-Type-Options": "nosniff",
      },
    });
  } catch (err) {
    if (process.env.NODE_ENV !== "production") {
      console.error(err);
    }

    return new Response("Internal Server Error", { status: 500 });
  }
      }
