...
const PREMIUM_COPY: Record<"fr" | "en", PremiumCopy> = {
  fr: {
    ...
    referencesItems: [
      "GHG Protocol — Scope 3, approche spend-based pour certaines estimations indirectes",
      "ISO 14064-1 — principes généraux de quantification et de reporting des émissions de gaz à effet de serre",
      "ISO 14083 — contexte de quantification des émissions liées aux opérations de transport",
      "CSRD / ESRS — contexte européen de transparence environnementale",
      "Taxonomie européenne — contexte de durabilité et de reporting extra-financier",
    ],
    referencesContext:
  ...
  en: {
    ...
    referencesItems: [
      "GHG Protocol — Scope 3, spend-based approach for selected indirect estimates",
      "ISO 14064-1 — general principles for greenhouse gas quantification and reporting",
      "ISO 14083 — context for quantifying emissions from transport operations",
  ...
  try {
    ...
    const metadata = {
      ...
      companyName: escapeHtml(
        clampText(String(metadataRaw.companyName || "—"), 100)
      ),
      companySector: escapeHtml(
        clampText(String(metadataRaw.companySector || "—"), 90)
      ),
      entityIdentifier: escapeHtml(
        clampText(String(metadataRaw.entityIdentifier || "—"), 80)
      ),
      country: escapeHtml(clampText(String(metadataRaw.country || "—"), 50)),
...
<div class="container">
  ...
  <section class="top-grid avoid-break">
    ...
    <div class="status-card">
      <div class="status-title">${premium.documentVerificationTitle}</div>
      <div class="status-row">
        <span class="status-label">${premium.statusReference}</span>
        <span class="status-value">${metadata.attestationId}</span>
      </div>
      <div class="status-row">
        <span class="status-label">${i18n.issuedDateLabel}</span>
        <span class="status-value">${metadata.issuedDate}</span>
      </div>
      <div class="status-row">
        <span class="status-label">${i18n.validUntilLabel}</span>
        <span class="status-value">${metadata.validUntil}</span>
      </div>
      <div class="status-row">
        <span class="status-label">${premium.statusIssuer}</span>
        <span class="status-value">${metadata.issuerName}</span>
      </div>
    </div>
  </section>
  ...
  <div class="two-col clearfix">
    <div class="main-col">
      <section class="section avoid-break" aria-labelledby="entity">
      ...
      <section class="section avoid-break" aria-labelledby="nature">
        <h2 class="section-title" id="nature">${i18n.documentNatureSectionTitle}</h2>
        <div class="card">
          <strong>${i18n.documentNatureText}</strong>
        </div>
      </section>

      <section class="section avoid-break" aria-labelledby="scope">
        <h2 class="section-title" id="scope">${i18n.scopeSectionTitle}</h2>
        <div class="card">
    ...
    <aside>
      <div class="side-card avoid-break">
        <div class="side-title">${premium.privacyTitle}</div>
        <div class="side-text">
          <ul class="privacy-list">
            ${premium.privacyBullets.map((item) => `<li>${item}</li>`).join("")}
          </ul>
        </div>
      </div>

      <div class="side-card avoid-break">
        <div class="side-title">${i18n.authenticityOverviewTitle}</div>
        <div class="side-text">${i18n.authenticityOverviewText}</div>
      </div>

      <div class="side-card avoid-break">
        <div class="side-title">${i18n.documentScopeSummaryTitle}</div>
        <div class="side-text">${i18n.documentScopeSummaryText}</div>
      </div>

      <div class="side-card avoid-break">
        <div class="side-title">${i18n.documentValidityTitle}</div>
        <div class="side-text">${i18n.documentValidityText}</div>
      </div>
    </aside>
  </div>
  ...
  <div class="two-col clearfix">
    <div class="main-col">
      <section class="section avoid-break" aria-labelledby="methodology">
        <h2 class="section-title" id="methodology">${i18n.methodologySectionTitle}</h2>
        <div class="card">
          <div><strong>${i18n.methodologyLabel} :</strong> ${metadata.methodology}</div>
          <div class="formula">${premium.methodFormula}</div>
          <ul class="compact-list">
            <li><strong>${premium.factorVersionLabel} :</strong> ${metadata.factorVersion}</li>
            <li><strong>${i18n.transferabilityLabel} :</strong> ${i18n.transferabilityText}</li>
          </ul>
        </div>
      </section>

      <section class="section avoid-break" aria-labelledby="references">
        <h2 class="section-title" id="references">${premium.referencesTitle}</h2>
        <div class="card">
          <div class="small" style="margin-bottom:4px;">
            ${premium.referencesIntro}
          </div>
      ...
      <section class="section avoid-break" aria-labelledby="verification">
        <h2 class="section-title" id="verification">${i18n.verificationSectionTitle}</h2>

        <div class="verification-simple">
          <div class="side-title">${premium.simpleVerificationTitle}</div>
        ...
        <div class="verification-tech">
          <div class="side-title">${premium.technicalVerificationTitle}</div>
          <div class="small">${i18n.technicalVerificationNote}</div>

          <div class="tech-line">
            <strong>${i18n.algorithmLabel} :</strong> ${metadata.algorithm}
          </div>

          <div class="tech-line">
            <strong>${i18n.hashLabel} :</strong><br/>
            <span class="break">${metadata.hash}</span>
          </div>

          <div class="tech-line">
            <strong>${i18n.signatureLabel} :</strong><br/>
            <span class="break">${metadata.signature}</span>
          </div>

          <div class="tech-line">
            <strong>${i18n.publicKeyLabel} :</strong><br/>
            <span class="break">MCowBQYDK2VwAyEAbKp2pg4wmzE5Kqo9tEwv7JJjxQyT2cBmwiLLHp4cSac=</span>
          </div>
        </div>
      </section>
    </div>
    ...
    <aside>
      <div class="side-card avoid-break">
        <div class="side-title">${i18n.declarationSectionTitle}</div>
        <div class="side-text">
          <strong>${i18n.declarationLabel} :</strong> ${i18n.declarationText}
        </div>
      </div>

      <div class="side-card avoid-break">
        <div class="side-title">${i18n.privacyLabel}</div>
        <div class="side-text">${i18n.privacyText}</div>
      </div>

      <div class="side-card avoid-break">
        <div class="side-title">${premium.verifiableObjectTitle}</div>
        <div class="side-text">${i18n.pdfObjectText}</div>
      </div>
    </aside>
  </div>
