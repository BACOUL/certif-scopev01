 ...
 *
 * IMPORTANT :
 * - Remets ici exactement ton base64 complet actuel.
 * - Le reste du fichier est prêt avec la nouvelle mise en page premium.
 * - Si tu as déjà le très long RAW_CERTIF_SCOPE_LOGO_BASE64 dans ton fichier actuel,
 *   conserve simplement son contenu complet entre les backticks.
 */
...
function getLocaleCopy(locale: AttestationLocale) {
  if (locale === "fr") {
    return {
      ...
      confidentialityText:
        "Aucune donnée financière détaillée n’est affichée dans cette attestation. Le document présente uniquement un résultat agrégé afin de faciliter une transmission externe sans divulguer les dépenses internes détaillées.",
  ...
  if (locale === "de") {
    return {
      ...
      confidentialityText:
        "In dieser Bescheinigung werden keine detaillierten Finanzdaten angezeigt. Das Dokument enthält ausschließlich ein aggregiertes Ergebnis, um eine externe Weitergabe ohne Offenlegung interner Ausgabendaten zu ermöglichen.",
  ...
  return {
    ...
    confidentialityText:
      "No detailed financial data is displayed in this attestation. The document only presents an aggregated result to enable external transmission without disclosing detailed internal expenditures.",
