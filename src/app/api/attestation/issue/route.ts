...
export async function GET(req: Request) {
  try {
    ...
    if (sessionId.startsWith("key_")) {
      // Mode clé autorisé en production (packs / partenaires)
...
<style>
  /* LAYOUT 2 PAGES STRICT 
     Modifié : Marges plus amples pour aspect institutionnel A4
  */
  @page {
    size: A4;
  ...
  /* Liens dans le footer (si présents dans i18n) héritent de la couleur pour rester discrets */
  .footer-static a {
    color: inherit;
    text-decoration: none;
  }

  /* Header - Modifié : centré verticalement, plus d'espace */
  header {
    display:flex;
    justify-content:space-between;
    align-items:center;
  ...
  /* Logo - Modifié : plus grand et imposant */
  .issuer-logo {
    height: 90px;
    max-width: 320px;
    display: block;
  ...
  /* QR - Modifié : plus grand, bordure plus visible */
  ...
  /* Titre H1 - Modifié : plus grand (22px) et espacé */
  .title h1 { 
    font-size:22px; 
    margin:0; 
    font-weight:700; 
  ...
  /* Result Box - Modifié : bordure plus fine (2px), pas d'ombre, plus sobre */
  .result-box {
    width:100%;
    max-width:600px;
    background:#ffffff;
  ...
  /* Sections - Modifié : espacement augmenté (12px) pour respiration */
