export const runtime = "nodejs";

import Stripe from "stripe";
import QRCode from "qrcode";
import { signCanonicalPayload, makeAttestationId } from "@/lib/sign";
import {
  ATTESTATION_I18N,
  type AttestationLocale,
  DEFAULT_ATTESTATION_LOCALE,
} from "@/lib/attestation-i18n/index";

const stripe = new Stripe(process.env.STRIPE_SECRET_KEY!);

const RAW_CERTIF_SCOPE_LOGO_BASE64 = `

`.replace(/\s+/g, "");

const CERTIF_SCOPE_LOGO_BASE64 = RAW_CERTIF_SCOPE_LOGO_BASE64;

type I18nDictionary = Record<string, unknown>;

function escapeHtml(input: string) {
  return input
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
    .replace(/"/g, "&quot;")
    .replace(/'/g, "&#39;");
}

function toBase64Url(input: string) {
  return Buffer.from(input, "utf8")
    .toString("base64")
    .replace(/\+/g, "-")
    .replace(/\//g, "_")
    .replace(/=+$/g, "");
}

function addMonthsToISODate(date: string, months: number) {
  const parsed = new Date(`${date}T00:00:00.000Z`);
  if (Number.isNaN(parsed.getTime())) return "";
  parsed.setUTCMonth(parsed.getUTCMonth() + months);
  return parsed.toISOString().slice(0, 10);
}

function normalizeSectionTitle(input: string) {
  const raw = String(input || "").replace(/\s+/g, " ").trim();

  return raw
    .replace(/^(\d+)\.\s*\1\.\s*/u, "$1. ")
    .replace(/^(\d+)\.\s*(\d+)\.\s*/u, (_, a, b) =>
      a === b ? `${a}. ` : `${a}. ${b}. `
    );
}

function stripLeadingNumber(input: string) {
  return normalizeSectionTitle(input).replace(/^\d+\.\s*/u, "").trim();
}

function resolveLocale(input: unknown): AttestationLocale {
  const value = String(input || DEFAULT_ATTESTATION_LOCALE).toLowerCase();
  if (value === "fr" || value === "de" || value === "en") {
    return value as AttestationLocale;
  }
  return DEFAULT_ATTESTATION_LOCALE;
}

function getLocaleCopy(locale: AttestationLocale) {
  if (locale === "fr") {
    return {
      headerTagline: "Émission automatisée · Attestation indicative standardisée",
      title: "ATTESTATION INDICATIVE D’ÉMISSIONS DE CARBONE",
      standardReference:
        "Émise conformément à la méthodologie standardisée Certif-Scope CS-SB-v1",
      subtitle:
        "Non réglementaire · Fondée sur une méthodologie · Attestation indicative · Certif-Scope CS-SB-v1",
      resultLabel: "ÉMISSIONS INDICATIVES AGRÉGÉES DÉCLARÉES",
      scanToVerifyLabel: "Scanner le QR code\npour vérifier",
      foundationMethodologyTitle: "FONDEMENT MÉTHODOLOGIQUE",
      foundationMethodologyText:
        "Cette attestation fournit une estimation CO₂e indicative, standardisée et vérifiable, fondée sur une méthodologie spend-based utilisant des données de dépenses agrégées déclarées et des facteurs d’émission monétaires.",
      documentScopeSummaryTitle: "USAGE PRÉVU",
      documentScopeSummaryText:
        "Cette attestation peut être utilisée comme document indicatif dans un dossier fournisseur, une demande client, un appel d’offres, une demande bancaire, une demande d’assurance ou une démarche interne de première estimation carbone, lorsque le besoin porte sur un indicateur CO₂e agrégé et documenté.",
      attestationReferenceLabel: "RÉFÉRENCE",
      issuedDateLabel: "DATE D’ÉMISSION",
      validUntilLabel: "VALABLE JUSQU’AU",
      issuerLabel: "ÉMETTEUR",
      entitySectionTitle: "IDENTIFICATION DE L’ENTITÉ",
      documentNatureSectionTitle: "NATURE DU DOCUMENT",
      scopeSectionTitle: "PÉRIMÈTRE",
      entityNameLabel: "NOM DE L’ENTITÉ",
      countryLabel: "PAYS",
      activitySectorLabel: "SECTEUR D’ACTIVITÉ",
      reportingYearLabel: "ANNÉE DE RÉFÉRENCE",
      entityIdentifierLabel: "IDENTIFIANT DE L’ENTITÉ",
      scopeDescriptionLabel: "Description du périmètre",
      documentNatureText:
        "Ce document constitue une attestation indicative d’émissions de carbone, émise exclusivement à des fins d’information, d’aide à la décision et d’évaluation préliminaire.",
      scopeText:
        "Cette attestation fournit une estimation indicative des émissions de gaz à effet de serre, dérivée exclusivement de données de dépenses agrégées, selon une méthodologie basée sur les dépenses (spend-based).",
      confidentialityTitle: "CONFIDENTIALITÉ",
      authenticityTitle: "AUTHENTICITÉ",
      validityTitle: "VALIDITÉ",
      privacyText: "Aucune conservation des données financières détaillées.",
      authenticityOverviewText:
        "L’attestation comporte une référence unique, un QR code et des éléments de contrôle documentaire.",
      documentValidityText:
        "La validité documentaire est limitée dans le temps et indiquée sur l’attestation.",
      pageTwoTitle: "7. MÉTHODOLOGIE ET LIMITES",
      pageTwoReferenceLabel: "Référence",
      pageTwoIssuerLabel: "Émetteur",
      methodologyTitle: "MÉTHODOLOGIE",
      methodologyLabel: "Méthodologie",
      methodologyValue: "Certif-Scope deterministic spend-based methodology v1.0",
      formulaText:
        "Dépenses agrégées déclarées × facteurs d’émission monétaires = estimation CO₂e indicative",
      factorVersionLabel: "VERSION DES FACTEURS",
      transferabilityLabel: "TRANSFÉRABILITÉ",
      transferabilityText: "Non transférable.",
      referencesTitle: "RÉFÉRENCES DE COHÉRENCE",
      normativeText:
        "La méthodologie Certif-Scope s’inscrit dans une logique de cohérence avec les cadres de référence carbone et de transparence environnementale suivants :",
      referencesList: [
        "GHG Protocol — Scope 3, méthode spend-based",
        "ISO 14083 — émissions liées aux opérations de transport",
        "Taxonomie européenne — contexte de durabilité et reporting extra-financier",
        "ISO 14064-1 — quantification et reporting GES",
        "CSRD / ESRS — transparence environnementale européenne",
      ],
      scopeNote:
        "Ces références sont citées comme cadre de cohérence méthodologique et contextuel. Elles ne transforment pas cette attestation en rapport réglementaire, audit certifié ou vérification par tiers indépendant.",
      verificationSimpleTitle: "VÉRIFICATION SIMPLE",
      verifiableObjectTitle: "OBJET VÉRIFIABLE",
      pageVerificationLabel: "PAGE DE VÉRIFICATION DOCUMENTAIRE",
      useQrOrReferencePrefix:
        "Scanner le QR code ou utiliser la référence d’attestation sur la page officielle de vérification.",
      pdfObjectText:
        "Le document PDF signé et son QR code constituent les éléments de contrôle documentaire.",
      verificationBoxTitle:
        "Le PDF signé et son QR code constituent les éléments de contrôle documentaire.",
      technicalVerificationNote:
        "Le document PDF signé et son QR code de vérification constituent les éléments de contrôle documentaire.",
      publicKeyNote:
        "Le document peut être vérifié indépendamment à l’aide de la clé publique de l’émetteur.",
      technicalElementsTitle: "ÉLÉMENTS TECHNIQUES DE VÉRIFICATION",
      technicalElementsIntro:
        "Les éléments ci-dessous permettent une vérification documentaire avancée. Aucune action n’est requise pour un lecteur standard du document.",
      algorithmLabel: "ALGORITHME",
      hashLabel: "EMPREINTE DU CONTENU SIGNÉ (SHA-256)",
      signatureLabel: "SIGNATURE (BASE64)",
      publicKeyLabel: "CLÉ PUBLIQUE DE VÉRIFICATION DE L’ÉMETTEUR",
      perimeterLimitsTitle: "PÉRIMÈTRE ET LIMITES",
      limitationsText:
        "Cette attestation est indicative. Elle repose sur les données déclarées par l’entité et sur une méthodologie spend-based. Elle ne remplace pas un audit carbone, un inventaire GES complet, une vérification par tiers indépendant, un Bilan Carbone® ou un reporting réglementaire lorsque ceux-ci sont explicitement exigés.",
      liabilityLabel: "RESPONSABILITÉ DES DONNÉES",
      liabilityText:
        "Les résultats dépendent de l’exactitude, de la complétude et de la cohérence des données déclarées par l’entité.",
      validityPeriodLabel: "VALIDITÉ DOCUMENTAIRE",
      methodologyNote:
        "CS-SB-v1 est une méthodologie standardisée interne maintenue par Certif-Scope.",
      languageNotice:
        "Langue du document : En cas de divergence entre les versions linguistiques, la version anglaise prévaut.",
      footerText:
        "Attestation indicative d’émissions de carbone · Émise par Certif-Scope · certif-scope.com",
      footerPageLabel: "Page",
    };
  }

  if (locale === "de") {
    return {
      headerTagline:
        "Automatisierte Ausstellung · Standardisierte indikative Bescheinigung",
      title: "INDIKATIVE BESCHEINIGUNG ZU CO₂e-EMISSIONEN",
      standardReference:
        "Ausgestellt gemäß der standardisierten Methodik Certif-Scope CS-SB-v1",
      subtitle:
        "Nicht regulatorisch · Methodikbasiert · Indikative Bescheinigung · Certif-Scope CS-SB-v1",
      resultLabel: "DEKLARIERTE AGGREGIERTE INDIKATIVE EMISSIONEN",
      scanToVerifyLabel: "QR-Code scannen\nzur Verifizierung",
      foundationMethodologyTitle: "METHODISCHE GRUNDLAGE",
      foundationMethodologyText:
        "Diese Bescheinigung liefert eine indikative, standardisierte und verifizierbare CO₂e-Schätzung auf Basis einer spend-based Methodik mit deklarierten aggregierten Ausgabendaten und monetären Emissionsfaktoren.",
      documentScopeSummaryTitle: "VORGESEHENE VERWENDUNG",
      documentScopeSummaryText:
        "Diese Bescheinigung kann als indikatives Dokument in Lieferantenunterlagen, Kundenanfragen, Ausschreibungen, Bank- oder Versicherungsanfragen oder für eine interne Erstschätzung des Carbon Footprints verwendet werden, wenn ein aggregierter CO₂e-Indikator benötigt wird.",
      attestationReferenceLabel: "REFERENZ",
      issuedDateLabel: "AUSSTELLUNGSDATUM",
      validUntilLabel: "GÜLTIG BIS",
      issuerLabel: "AUSSTELLER",
      entitySectionTitle: "IDENTIFIKATION DER EINHEIT",
      documentNatureSectionTitle: "ART DES DOKUMENTS",
      scopeSectionTitle: "UMFANG",
      entityNameLabel: "NAME DER EINHEIT",
      countryLabel: "LAND",
      activitySectorLabel: "TÄTIGKEITSBEREICH",
      reportingYearLabel: "BERICHTSJAHR",
      entityIdentifierLabel: "IDENTIFIKATOR DER EINHEIT",
      scopeDescriptionLabel: "Beschreibung des Umfangs",
      documentNatureText:
        "Dieses Dokument stellt eine indikative Bescheinigung zu Kohlenstoffemissionen dar und wird ausschließlich zu Informations-, Entscheidungs- und Vorbewertungszwecken ausgestellt.",
      scopeText:
        "Diese Bescheinigung liefert eine indikative Schätzung der Treibhausgasemissionen, die ausschließlich aus aggregierten Ausgabendaten auf Basis einer spend-based Methodik abgeleitet wird.",
      confidentialityTitle: "VERTRAULICHKEIT",
      authenticityTitle: "AUTHENTIZITÄT",
      validityTitle: "GÜLTIGKEIT",
      privacyText: "Keine Speicherung detaillierter Finanzdaten.",
      authenticityOverviewText:
        "Die Bescheinigung enthält eine eindeutige Referenz, einen QR-Code und dokumentarische Kontrollelemente.",
      documentValidityText:
        "Die dokumentarische Gültigkeit ist zeitlich begrenzt und auf der Bescheinigung angegeben.",
      pageTwoTitle: "7. METHODIK UND GRENZEN",
      pageTwoReferenceLabel: "Referenz",
      pageTwoIssuerLabel: "Aussteller",
      methodologyTitle: "METHODIK",
      methodologyLabel: "Methodik",
      methodologyValue: "Certif-Scope deterministic spend-based methodology v1.0",
      formulaText:
        "Deklarierte aggregierte Ausgaben × monetäre Emissionsfaktoren = indikative CO₂e-Schätzung",
      factorVersionLabel: "FAKTORENVERSION",
      transferabilityLabel: "ÜBERTRAGBARKEIT",
      transferabilityText: "Nicht übertragbar.",
      referencesTitle: "KOHÄRENZREFERENZEN",
      normativeText:
        "Die Certif-Scope-Methodik steht im Einklang mit folgenden Bezugsrahmen für CO₂-Bilanzierung und Umwelttransparenz:",
      referencesList: [
        "GHG Protocol — Scope 3, spend-based Methode",
        "ISO 14083 — transportbezogene Emissionen",
        "EU-Taxonomie — Nachhaltigkeits- und Berichtsrahmen",
        "ISO 14064-1 — THG-Quantifizierung und Berichterstattung",
        "CSRD / ESRS — europäische Umwelttransparenz",
      ],
      scopeNote:
        "Diese Referenzen werden als methodischer und kontextueller Kohärenzrahmen genannt. Sie machen diese Bescheinigung nicht zu einem regulatorischen Bericht, zertifizierten Audit oder einer unabhängigen Drittprüfung.",
      verificationSimpleTitle: "EINFACHE VERIFIKATION",
      verifiableObjectTitle: "VERIFIZIERBARES OBJEKT",
      pageVerificationLabel: "DOKUMENTARISCHE VERIFIKATIONSSEITE",
      useQrOrReferencePrefix:
        "QR-Code scannen oder die Bescheinigungsreferenz auf der offiziellen Verifikationsseite verwenden.",
      pdfObjectText:
        "Das signierte PDF-Dokument und sein QR-Code bilden die dokumentarischen Kontrollelemente.",
      verificationBoxTitle:
        "Das signierte PDF-Dokument und sein QR-Code bilden die dokumentarischen Kontrollelemente.",
      technicalVerificationNote:
        "Das signierte PDF-Dokument und sein Verifikations-QR-Code bilden die dokumentarischen Kontrollelemente.",
      publicKeyNote:
        "Das Dokument kann unabhängig mit dem öffentlichen Schlüssel des Ausstellers geprüft werden.",
      technicalElementsTitle: "TECHNISCHE VERIFIKATIONSELEMENTE",
      technicalElementsIntro:
        "Die nachstehenden Elemente ermöglichen eine fortgeschrittene dokumentarische Verifikation. Für einen Standardleser ist keine Aktion erforderlich.",
      algorithmLabel: "ALGORITHMUS",
      hashLabel: "FINGERABDRUCK DES SIGNIERTEN INHALTS (SHA-256)",
      signatureLabel: "SIGNATUR (BASE64)",
      publicKeyLabel: "ÖFFENTLICHER VERIFIKATIONSSCHLÜSSEL DES AUSSTELLERS",
      perimeterLimitsTitle: "UMFANG UND GRENZEN",
      limitationsText:
        "Diese Bescheinigung ist indikativ. Sie beruht auf den von der Einheit bereitgestellten Daten und auf einer spend-based Methodik. Sie ersetzt kein CO₂-Audit, kein vollständiges THG-Inventar, keine unabhängige Drittprüfung, kein Bilan Carbone® und kein regulatorisches Reporting, wenn diese ausdrücklich verlangt werden.",
      liabilityLabel: "VERANTWORTUNG FÜR DIE DATEN",
      liabilityText:
        "Die Ergebnisse hängen von der Genauigkeit, Vollständigkeit und Kohärenz der von der Einheit bereitgestellten Daten ab.",
      validityPeriodLabel: "DOKUMENTARISCHE GÜLTIGKEIT",
      methodologyNote:
        "CS-SB-v1 ist eine interne standardisierte Methodik, die von Certif-Scope gepflegt wird.",
      languageNotice:
        "Dokumentsprache: Bei Abweichungen zwischen den Sprachversionen hat die englische Version Vorrang.",
      footerText:
        "Indikative Bescheinigung zu Kohlenstoffemissionen · Ausgestellt von Certif-Scope · certif-scope.com",
      footerPageLabel: "Seite",
    };
  }

  return {
    headerTagline:
      "Automated issuance · Standardized indicative attestation",
    title: "INDICATIVE CARBON EMISSIONS ATTESTATION",
    standardReference:
      "Issued pursuant to the standardized Certif-Scope methodology CS-SB-v1",
    subtitle:
      "Non-regulatory · Methodology-based · Indicative attestation · Certif-Scope CS-SB-v1",
    resultLabel: "DECLARED AGGREGATED INDICATIVE EMISSIONS",
    scanToVerifyLabel: "Scan QR code\nto verify",
    foundationMethodologyTitle: "METHODOLOGICAL BASIS",
    foundationMethodologyText:
      "This attestation provides an indicative, standardized and verifiable CO₂e estimate based on a spend-based methodology using declared aggregated expenditure data and monetary emission factors.",
    documentScopeSummaryTitle: "INTENDED USE",
    documentScopeSummaryText:
      "This attestation may be used as an indicative document in supplier files, client requests, tenders, banking requests, insurance requests or an internal first carbon estimate, when the need concerns an aggregated and documented CO₂e indicator.",
    attestationReferenceLabel: "REFERENCE",
    issuedDateLabel: "ISSUED ON",
    validUntilLabel: "VALID UNTIL",
    issuerLabel: "ISSUER",
    entitySectionTitle: "IDENTIFICATION OF THE ENTITY",
    documentNatureSectionTitle: "NATURE OF THE DOCUMENT",
    scopeSectionTitle: "SCOPE",
    entityNameLabel: "ENTITY NAME",
    countryLabel: "COUNTRY",
    activitySectorLabel: "ACTIVITY SECTOR",
    reportingYearLabel: "REPORTING YEAR",
    entityIdentifierLabel: "ENTITY IDENTIFIER",
    scopeDescriptionLabel: "Scope description",
    documentNatureText:
      "This document constitutes an indicative carbon emissions attestation issued exclusively for information, decision-support and preliminary assessment purposes.",
    scopeText:
      "This attestation provides an indicative estimate of greenhouse gas emissions derived exclusively from aggregated expenditure data, using a spend-based methodology.",
    confidentialityTitle: "CONFIDENTIALITY",
    authenticityTitle: "AUTHENTICITY",
    validityTitle: "VALIDITY",
    privacyText: "No retention of detailed financial data.",
    authenticityOverviewText:
      "The attestation includes a unique reference, a QR code and documentary control elements.",
    documentValidityText:
      "Document validity is time-limited and indicated on the attestation.",
    pageTwoTitle: "7. METHODOLOGY AND LIMITATIONS",
    pageTwoReferenceLabel: "Reference",
    pageTwoIssuerLabel: "Issuer",
    methodologyTitle: "METHODOLOGY",
    methodologyLabel: "Methodology",
    methodologyValue: "Certif-Scope deterministic spend-based methodology v1.0",
    formulaText:
      "Declared aggregated expenditures × monetary emission factors = indicative CO₂e estimate",
    factorVersionLabel: "FACTOR VERSION",
    transferabilityLabel: "TRANSFERABILITY",
    transferabilityText: "Non-transferable.",
    referencesTitle: "CONSISTENCY REFERENCES",
    normativeText:
      "The Certif-Scope methodology is positioned within a consistency logic with the following carbon-accounting and environmental transparency reference frameworks:",
    referencesList: [
      "GHG Protocol — Scope 3, spend-based method",
      "ISO 14083 — emissions related to transport operations",
      "EU Taxonomy — sustainability and extra-financial reporting context",
      "ISO 14064-1 — GHG quantification and reporting",
      "CSRD / ESRS — European environmental transparency",
    ],
    scopeNote:
      "These references are cited as a methodological and contextual consistency framework. They do not transform this attestation into a regulatory report, certified audit or independent third-party verification.",
    verificationSimpleTitle: "SIMPLE VERIFICATION",
    verifiableObjectTitle: "VERIFIABLE OBJECT",
    pageVerificationLabel: "DOCUMENT VERIFICATION PAGE",
    useQrOrReferencePrefix:
      "Scan the QR code or use the attestation reference on the official verification page.",
    pdfObjectText:
      "The signed PDF document and its QR code form the documentary control elements.",
    verificationBoxTitle:
      "The signed PDF document and its QR code form the documentary control elements.",
    technicalVerificationNote:
      "The signed PDF document and its verification QR code form the documentary control elements.",
    publicKeyNote:
      "The document can be independently verified using the issuer public key.",
    technicalElementsTitle: "TECHNICAL VERIFICATION ELEMENTS",
    technicalElementsIntro:
      "The elements below enable advanced documentary verification. No action is required from a standard reader of the document.",
    algorithmLabel: "ALGORITHM",
    hashLabel: "SIGNED CONTENT HASH (SHA-256)",
    signatureLabel: "SIGNATURE (BASE64)",
    publicKeyLabel: "ISSUER PUBLIC VERIFICATION KEY",
    perimeterLimitsTitle: "SCOPE AND LIMITATIONS",
    limitationsText:
      "This attestation is indicative. It relies on the data declared by the entity and on a spend-based methodology. It does not replace a carbon audit, a full GHG inventory, an independent third-party verification, a Bilan Carbone® or regulatory reporting when these are explicitly required.",
    liabilityLabel: "DATA LIABILITY",
    liabilityText:
      "Results depend on the accuracy, completeness and consistency of the data declared by the entity.",
    validityPeriodLabel: "DOCUMENT VALIDITY",
    methodologyNote:
      "CS-SB-v1 is an internal standardized methodology maintained by Certif-Scope.",
    languageNotice:
      "Document language: In case of divergence between language versions, the English version shall prevail.",
    footerText:
      "Indicative carbon emissions attestation · Issued by Certif-Scope · certif-scope.com",
    footerPageLabel: "Page",
  };
}

function getText(
  i18n: I18nDictionary,
  key: string,
  fallback: string
): string {
  const value = i18n[key];
  if (typeof value === "string" && value.trim().length > 0) {
    return value;
  }
  return fallback;
}

function getList(
  i18n: I18nDictionary,
  key: string,
  fallback: string[]
): string[] {
  const value = i18n[key];
  if (Array.isArray(value)) {
    return value.map((item) => String(item));
  }
  return fallback;
}

function formatNumberForLocale(value: number, locale: AttestationLocale) {
  const localeTag =
    locale === "fr" ? "fr-FR" : locale === "de" ? "de-DE" : "en-US";

  return new Intl.NumberFormat(localeTag, {
    minimumFractionDigits: 0,
    maximumFractionDigits: 2,
  }).format(value);
}

export async function GET(req: Request) {
  try {
    if (!process.env.PDFSHIFT_API_KEY) {
      return new Response("PDFSHIFT_API_KEY missing", { status: 500 });
    }

    const { searchParams } = new URL(req.url);
    const sessionId = searchParams.get("session_id");

    if (!sessionId) {
      return new Response("Missing session_id", { status: 400 });
    }

    let metadataRaw: Record<string, unknown> = {};

    if (sessionId.startsWith("key_")) {
      metadataRaw = Object.fromEntries(searchParams.entries());
    } else {
      const session = await stripe.checkout.sessions.retrieve(sessionId);

      if (session.payment_status !== "paid") {
        return new Response("Payment not completed", { status: 403 });
      }

      metadataRaw = (session.metadata || {}) as Record<string, unknown>;
    }

    const locale = resolveLocale(metadataRaw.attestationLocale);
    const i18n = (ATTESTATION_I18N[locale] ||
      ATTESTATION_I18N.en) as unknown as I18nDictionary;
    const copy = getLocaleCopy(locale);

    const required = ["companyName", "totalCO2e", "year"];
    const missing = required.filter((k) => {
      const value = metadataRaw[k];
      return value === undefined || value === null || String(value).trim() === "";
    });

    if (missing.length > 0) {
      return new Response(`Missing metadata: ${missing.join(", ")}`, {
        status: 400,
      });
    }

    const totalCO2eNum = Number(String(metadataRaw.totalCO2e).replace(",", "."));
    if (Number.isNaN(totalCO2eNum)) {
      return new Response("Invalid metadata: totalCO2e must be a number", {
        status: 400,
      });
    }

    const issuerNameRaw = String(metadataRaw.issuerName || "Certif-Scope");
    const issuerSiteRaw = String(
      metadataRaw.issuerSite || "https://www.certif-scope.com"
    );
    const companyNameRaw = String(metadataRaw.companyName || "");
    const companySectorRaw = String(metadataRaw.companySector || "—");
    const entityIdentifierRaw = String(metadataRaw.entityIdentifier || "—");
    const countryRaw = String(metadataRaw.country || "—");
    const yearRaw = String(metadataRaw.year || "");
    const issuedDateIso = new Date().toISOString();
    const issuedDate = issuedDateIso.slice(0, 10);
    const validityMonths = Number(metadataRaw.validityMonths || 12);
    const validUntilRaw =
      String(metadataRaw.validUntil || "").trim() ||
      addMonthsToISODate(issuedDate, Number.isFinite(validityMonths) ? validityMonths : 12);
    const standardRefRaw = String(
      metadataRaw.standardRef || "Certif-Scope CS-SB-v1"
    );
    const methodologyRaw = String(
      metadataRaw.methodology || copy.methodologyValue
    );
    const factorVersionRaw = String(
      metadataRaw.factorVersion ||
        metadataRaw.emissionFactorVersion ||
        "Certif-Scope factors v1"
    );

    const canonicalPayload = {
      issuer: "Certif-Scope" as const,
      standard: "CS-SB-v1" as const,
      attestationId: "",
      companyName: companyNameRaw,
      country: countryRaw,
      year: yearRaw,
      totalCO2e: String(totalCO2eNum),
      issuedDate: issuedDateIso,
    };

    const tempSignature = signCanonicalPayload({
      ...canonicalPayload,
      attestationId: "TEMP",
    });

    const attestationId = makeAttestationId(
      canonicalPayload.year,
      tempSignature.hashHex
    );

    const signatureResult = signCanonicalPayload({
      ...canonicalPayload,
      attestationId,
    });

    const verificationPayload = {
      certificateId: attestationId,
      issuer: "Certif-Scope",
      issuedAt: issuedDate,
      validUntil: validUntilRaw,
      methodVersion: "CS-SB-v1",
      factorVersion: factorVersionRaw,
      algorithm: signatureResult.algorithm,
      hash: signatureResult.hashHex,
      signature: signatureResult.signatureBase64,
    };

    const verificationToken = toBase64Url(JSON.stringify(verificationPayload));
    const verifyUrl =
      locale === "fr"
        ? `https://www.certif-scope.com/fr/verify/?v=${verificationToken}`
        : `https://www.certif-scope.com/verify/?v=${verificationToken}`;

    const verificationDisplayUrl =
      locale === "fr"
        ? "https://www.certif-scope.com/fr/verify"
        : "https://www.certif-scope.com/verify";

    const qrDataUrl = await QRCode.toDataURL(verifyUrl, {
      width: 160,
      margin: 1,
    });

    const metadata = {
      issuerName: escapeHtml(issuerNameRaw),
      issuerSite: escapeHtml(issuerSiteRaw),
      companyName: escapeHtml(companyNameRaw),
      companySector: escapeHtml(companySectorRaw),
      entityIdentifier: escapeHtml(entityIdentifierRaw),
      country: escapeHtml(countryRaw),
      year: escapeHtml(yearRaw),
      totalCO2e: escapeHtml(formatNumberForLocale(totalCO2eNum, locale)),
      attestationId: escapeHtml(attestationId),
      issuedDate: escapeHtml(issuedDate),
      validUntil: escapeHtml(validUntilRaw),
      standardRef: escapeHtml(standardRefRaw),
      methodology: escapeHtml(methodologyRaw),
      factorVersion: escapeHtml(factorVersionRaw),
      algorithm: escapeHtml(signatureResult.algorithm),
      hash: escapeHtml(signatureResult.hashHex),
      signature: escapeHtml(signatureResult.signatureBase64),
      publicKey: escapeHtml(
        "MCowBQYDK2VwAyEAbKp2pg4wmzE5Kqo9tEwv7JJjxQyT2cBmwiLLHp4cSac="
      ),
    };

    const headerTagline = escapeHtml(
      getText(i18n, "headerTagline", copy.headerTagline)
    );
    const title = escapeHtml(getText(i18n, "title", copy.title));
    const standardReference = escapeHtml(
      getText(i18n, "standardReference", copy.standardReference)
    );
    const subtitle = escapeHtml(getText(i18n, "subtitle", copy.subtitle));
    const resultLabel = escapeHtml(
      getText(i18n, "resultLabel", copy.resultLabel)
    );
    const foundationMethodologyTitle = escapeHtml(copy.foundationMethodologyTitle);
    const foundationMethodologyText = escapeHtml(
      getText(
        i18n,
        "documentNatureText",
        copy.foundationMethodologyText
      ) === getText(i18n, "documentNatureText", copy.foundationMethodologyText)
        ? copy.foundationMethodologyText
        : copy.foundationMethodologyText
    );
    const documentScopeSummaryTitle = escapeHtml(
      getText(i18n, "documentScopeSummaryTitle", copy.documentScopeSummaryTitle)
    );
    const documentScopeSummaryText = escapeHtml(
      getText(i18n, "documentScopeSummaryText", copy.documentScopeSummaryText)
    );
    const documentNatureText = escapeHtml(
      getText(i18n, "documentNatureText", copy.documentNatureText)
    );
    const scopeText = escapeHtml(getText(i18n, "scopeText", copy.scopeText));
    const privacyText = escapeHtml(
      getText(i18n, "privacyText", copy.privacyText)
    );
    const authenticityOverviewText = escapeHtml(
      getText(
        i18n,
        "authenticityOverviewText",
        copy.authenticityOverviewText
      )
    );
    const documentValidityText = escapeHtml(
      getText(i18n, "documentValidityText", copy.documentValidityText)
    );
    const methodologyLabel = escapeHtml(
      getText(i18n, "methodologyLabel", copy.methodologyLabel)
    );
    const transferabilityLabel = escapeHtml(
      getText(i18n, "transferabilityLabel", copy.transferabilityLabel)
    );
    const transferabilityText = escapeHtml(
      getText(i18n, "transferabilityText", copy.transferabilityText)
    );
    const normativeText = escapeHtml(
      getText(i18n, "normativeText", copy.normativeText)
    );
    const referencesList = getList(i18n, "referencesList", copy.referencesList);
    const scopeNote = escapeHtml(getText(i18n, "scopeNote", copy.scopeNote));
    const verificationBoxTitle = escapeHtml(
      getText(i18n, "verificationBoxTitle", copy.verificationBoxTitle)
    );
    const technicalVerificationNote = escapeHtml(
      getText(
        i18n,
        "technicalVerificationNote",
        copy.technicalVerificationNote
      )
    );
    const limitationsText = escapeHtml(
      getText(i18n, "limitationsText", copy.limitationsText)
    );
    const liabilityText = escapeHtml(
      getText(i18n, "liabilityText", copy.liabilityText)
    );
    const methodologyNote = escapeHtml(
      getText(i18n, "methodologyNote", copy.methodologyNote)
    );
    const footerText = escapeHtml(getText(i18n, "footerText", copy.footerText));
    const scanToVerifyLabel = escapeHtml(
      getText(i18n, "scanToVerifyLabel", copy.scanToVerifyLabel)
    ).replace(/\n/g, "<br/>");

    const entitySectionTitle = escapeHtml(
      `3. ${stripLeadingNumber(
        getText(i18n, "entitySectionTitle", copy.entitySectionTitle)
      )}`
    );
    const documentNatureSectionTitle = escapeHtml(
      `2. ${stripLeadingNumber(
        getText(
          i18n,
          "documentNatureSectionTitle",
          copy.documentNatureSectionTitle
        )
      )}`
    );
    const scopeSectionTitle = escapeHtml(
      `4. ${stripLeadingNumber(
        getText(i18n, "scopeSectionTitle", copy.scopeSectionTitle)
      )}`
    );

    const html = `
<!doctype html>
<html lang="${locale}">
<head>
  <meta charset="utf-8" />
  <title>${title}</title>
  <style>
    @page {
      size: A4;
      margin: 10mm 12mm 10mm 12mm;
    }

    :root {
      --ink: #223243;
      --heading: #0B3A63;
      --accent: #1FB6C1;
      --line: #d8e0e6;
      --line-strong: #bfd0dc;
      --muted: #6d7c8a;
      --muted-2: #8593a1;
      --panel: #ffffff;
      --panel-soft: #f7fafc;
      --panel-tint: #eff6f8;
      --serif: "Times New Roman", Georgia, serif;
    }

    * {
      box-sizing: border-box;
    }

    html, body {
      margin: 0;
      padding: 0;
      background: #ffffff;
      color: var(--ink);
      font-family: Inter, Arial, Helvetica, sans-serif;
      font-size: 9px;
      line-height: 1.38;
      -webkit-font-smoothing: antialiased;
    }

    body {
      width: 100%;
    }

    .page {
      min-height: 272mm;
      display: flex;
      flex-direction: column;
    }

    .page.page-one {
      page-break-after: always;
    }

    .content {
      flex: 1;
      display: flex;
      flex-direction: column;
    }

    .footer {
      margin-top: 8px;
      padding-top: 4px;
      border-top: 1px solid var(--line);
      display: flex;
      justify-content: space-between;
      align-items: center;
      color: var(--muted);
      font-size: 7.7px;
    }

    .header {
      display: flex;
      justify-content: space-between;
      align-items: flex-start;
      gap: 16px;
      margin-bottom: 10px;
      padding-bottom: 8px;
      border-bottom: 1.5px solid var(--heading);
    }

    .header-left {
      display: flex;
      flex-direction: column;
      align-items: flex-start;
      gap: 4px;
      flex: 1;
    }

    .logo {
      height: 60px;
      width: auto;
      max-width: 255px;
      object-fit: contain;
      display: block;
    }

    .issuer-site,
    .header-tagline {
      color: var(--muted);
      font-size: 8px;
      line-height: 1.2;
    }

    .issuer-site a {
      color: inherit;
      text-decoration: none;
    }

    .qr-block {
      text-align: center;
      flex: 0 0 auto;
    }

    .qr {
      width: 86px;
      height: 86px;
      border: 1px solid var(--line);
      padding: 4px;
      background: #fff;
      border-radius: 3px;
      display: block;
    }

    .qr-caption {
      margin-top: 4px;
      font-size: 7px;
      line-height: 1.15;
      color: var(--ink);
    }

    .title-zone {
      text-align: center;
      margin-bottom: 8px;
    }

    .eyebrow {
      color: var(--accent);
      font-size: 7.5px;
      font-weight: 700;
      letter-spacing: 1.05px;
      text-transform: uppercase;
      margin-bottom: 4px;
    }

    h1 {
      margin: 0;
      color: var(--heading);
      font-size: 18px;
      line-height: 1.08;
      font-weight: 800;
      text-transform: uppercase;
      letter-spacing: 0.2px;
    }

    .title-formal {
      margin-top: 4px;
      color: var(--ink);
      font-size: 8.8px;
      font-weight: 700;
    }

    .title-sub {
      margin-top: 2px;
      color: var(--muted);
      font-size: 7.7px;
    }

    .result-wrap {
      display: flex;
      justify-content: center;
      margin-bottom: 9px;
    }

    .result-box {
      width: 78%;
      border: 2px solid var(--heading);
      border-radius: 6px;
      background: #fff;
      padding: 9px 12px 10px;
      text-align: center;
      page-break-inside: avoid;
    }

    .result-label {
      color: var(--muted);
      font-size: 7.7px;
      font-weight: 700;
      letter-spacing: 0.7px;
      text-transform: uppercase;
      margin-bottom: 4px;
    }

    .result-value {
      color: var(--heading);
      font-family: var(--serif);
      font-size: 22px;
      line-height: 1;
      font-weight: 800;
      margin-bottom: 4px;
    }

    .result-note {
      color: var(--muted);
      font-size: 7.7px;
    }

    .summary-grid {
      display: grid;
      grid-template-columns: repeat(4, 1fr);
      border: 1px solid var(--line-strong);
      border-radius: 5px;
      overflow: hidden;
      margin-bottom: 8px;
      page-break-inside: avoid;
    }

    .summary-cell {
      background: #fff;
      padding: 7px 8px 8px;
      border-right: 1px solid var(--line-strong);
    }

    .summary-cell:last-child {
      border-right: none;
    }

    .summary-label {
      display: block;
      color: var(--muted-2);
      font-size: 6.8px;
      font-weight: 700;
      letter-spacing: 0.45px;
      text-transform: uppercase;
      margin-bottom: 3px;
    }

    .summary-value {
      display: block;
      color: var(--ink);
      font-size: 8.8px;
      font-weight: 700;
      line-height: 1.25;
    }

    .card {
      border: 1px solid var(--line);
      border-radius: 5px;
      background: #fff;
      padding: 8px 10px;
      margin-bottom: 7px;
      page-break-inside: avoid;
    }

    .card.tint {
      background: var(--panel-tint);
      border-color: #d8e7ea;
    }

    .card.soft {
      background: var(--panel-soft);
    }

    .card-title {
      margin: 0 0 4px;
      color: var(--heading);
      font-size: 8.7px;
      line-height: 1.15;
      font-weight: 800;
      letter-spacing: 0.35px;
      text-transform: uppercase;
    }

    .card-text {
      margin: 0;
      color: var(--ink);
      font-size: 8.8px;
      line-height: 1.37;
    }

    .entity-grid {
      display: grid;
      grid-template-columns: 1fr 1fr;
      gap: 8px 24px;
    }

    .entity-item .label {
      display: block;
      color: var(--muted-2);
      font-size: 6.8px;
      font-weight: 700;
      letter-spacing: 0.45px;
      text-transform: uppercase;
      margin-bottom: 2px;
    }

    .entity-item .value {
      display: block;
      color: var(--ink);
      font-size: 8.8px;
      font-weight: 700;
      line-height: 1.3;
    }

    .two-col {
      display: grid;
      grid-template-columns: 1fr 1fr;
      gap: 8px;
    }

    .features {
      display: grid;
      grid-template-columns: repeat(3, 1fr);
      gap: 8px;
    }

    .feature-card {
      min-height: 74px;
      border: 1px solid var(--line);
      border-radius: 5px;
      padding: 8px 10px;
      background: var(--panel-soft);
      page-break-inside: avoid;
    }

    .feature-title {
      margin: 0 0 4px;
      color: var(--heading);
      font-size: 8.5px;
      line-height: 1.15;
      font-weight: 800;
      letter-spacing: 0.35px;
      text-transform: uppercase;
    }

    .feature-text {
      color: var(--ink);
      font-size: 8.5px;
      line-height: 1.34;
    }

    .feature-text ul {
      margin: 0;
      padding-left: 12px;
    }

    .feature-text li {
      margin: 0 0 2px 0;
    }

    .page-two-top {
      display: flex;
      justify-content: space-between;
      align-items: flex-start;
      margin-bottom: 10px;
      gap: 12px;
    }

    .page-two-title {
      margin: 0;
      color: var(--heading);
      font-size: 15px;
      font-weight: 800;
      line-height: 1.1;
      text-transform: uppercase;
      letter-spacing: 0.2px;
    }

    .page-two-ref {
      text-align: right;
      color: var(--muted);
      font-size: 7.8px;
      line-height: 1.25;
      padding-top: 2px;
    }

    .formula-box {
      margin-top: 7px;
      border: 1px solid #d9eaee;
      border-radius: 4px;
      background: #eef7f8;
      padding: 6px 9px;
      text-align: center;
      color: var(--heading);
      font-size: 8.5px;
      font-weight: 700;
    }

    .mini-grid {
      display: grid;
      grid-template-columns: 1fr 1fr;
      gap: 8px;
      margin-top: 7px;
    }

    .mini-box {
      border: 1px solid var(--line);
      border-radius: 4px;
      background: var(--panel-soft);
      padding: 7px 8px;
    }

    .mini-label {
      display: block;
      color: var(--muted-2);
      font-size: 6.8px;
      font-weight: 700;
      letter-spacing: 0.45px;
      text-transform: uppercase;
      margin-bottom: 3px;
    }

    .mini-value {
      display: block;
      color: var(--ink);
      font-size: 8.6px;
      font-weight: 700;
      line-height: 1.28;
    }

    .reference-list {
      margin: 4px 0 0;
      padding-left: 15px;
      columns: 2;
      column-gap: 24px;
    }

    .reference-list li {
      margin-bottom: 3px;
      break-inside: avoid;
      color: var(--ink);
      font-size: 8.6px;
      line-height: 1.32;
    }

    .small-note {
      margin-top: 5px;
      color: var(--muted);
      font-size: 7.8px;
      line-height: 1.3;
    }

    .verification-grid {
      display: grid;
      grid-template-columns: 1fr 1fr;
      gap: 8px;
    }

    .verification-card {
      min-height: 102px;
      border: 1px solid var(--line);
      border-radius: 5px;
      background: var(--panel-soft);
      padding: 9px 10px;
      page-break-inside: avoid;
    }

    .verification-card.primary {
      background: #edf7f8;
      border-left: 4px solid var(--accent);
      padding-left: 8px;
    }

    .verification-card h3 {
      margin: 0 0 5px;
      color: var(--heading);
      font-size: 8.7px;
      line-height: 1.12;
      font-weight: 800;
      text-transform: uppercase;
      letter-spacing: 0.35px;
    }

    .verification-card p {
      margin: 0 0 5px;
      color: var(--ink);
      font-size: 8.6px;
      line-height: 1.34;
    }

    .verification-link {
      color: var(--heading);
      font-weight: 700;
      text-decoration: none;
      word-break: break-all;
    }

    .tech-card .small-note {
      margin-top: 0;
      margin-bottom: 6px;
    }

    .technical-grid {
      display: grid;
      grid-template-columns: 132px 1fr;
      gap: 8px 10px;
      margin-top: 4px;
    }

    .tech-label {
      color: var(--muted-2);
      font-size: 6.8px;
      font-weight: 700;
      letter-spacing: 0.45px;
      text-transform: uppercase;
      line-height: 1.25;
    }

    .tech-value {
      color: var(--ink);
      font-size: 7.3px;
      line-height: 1.25;
      word-break: break-all;
    }

    .limit-grid {
      display: grid;
      grid-template-columns: 1fr 1fr;
      gap: 10px;
      margin-top: 8px;
    }

    .limit-subtitle {
      margin: 0 0 3px;
      color: var(--heading);
      font-size: 8.3px;
      font-weight: 800;
      line-height: 1.12;
      letter-spacing: 0.3px;
      text-transform: uppercase;
    }

    .language-note {
      margin-top: 7px;
      color: var(--ink);
      font-size: 7.9px;
      font-weight: 700;
    }

    .doc-standard {
      margin-top: 3px;
      color: var(--muted);
      font-size: 7.5px;
    }
  </style>
</head>
<body>
  <div class="page page-one">
    <div class="content">
      <div class="header">
        <div class="header-left">
          ${
            CERTIF_SCOPE_LOGO_BASE64
              ? `<img src="data:image/png;base64,${CERTIF_SCOPE_LOGO_BASE64}" alt="Certif-Scope" class="logo" />`
              : ""
          }
          <div class="issuer-site"><a href="${metadata.issuerSite}">${metadata.issuerSite}</a></div>
          <div class="header-tagline">${headerTagline}</div>
        </div>
        <div class="qr-block">
          <img src="${qrDataUrl}" alt="QR verification" class="qr" />
          <div class="qr-caption">${scanToVerifyLabel}</div>
        </div>
      </div>

      <div class="title-zone">
        <div class="eyebrow">DOCUMENT CO₂e INDICATIF · STANDARDISÉ · VÉRIFIABLE</div>
        <h1>${title}</h1>
        <div class="title-formal">${standardReference}</div>
        <div class="title-sub">${subtitle}</div>
      </div>

      <div class="result-wrap">
        <div class="result-box">
          <div class="result-label">${resultLabel}</div>
          <div class="result-value">${metadata.totalCO2e} tCO₂e</div>
          <div class="result-note">${documentScopeSummaryText}</div>
        </div>
      </div>

      <div class="summary-grid">
        <div class="summary-cell">
          <span class="summary-label">${escapeHtml(copy.attestationReferenceLabel)}</span>
          <span class="summary-value">${metadata.attestationId}</span>
        </div>
        <div class="summary-cell">
          <span class="summary-label">${escapeHtml(copy.issuedDateLabel)}</span>
          <span class="summary-value">${metadata.issuedDate}</span>
        </div>
        <div class="summary-cell">
          <span class="summary-label">${escapeHtml(copy.validUntilLabel)}</span>
          <span class="summary-value">${metadata.validUntil}</span>
        </div>
        <div class="summary-cell">
          <span class="summary-label">${escapeHtml(copy.issuerLabel)}</span>
          <span class="summary-value">${metadata.issuerName}</span>
        </div>
      </div>

      <div class="card tint">
        <h2 class="card-title">${foundationMethodologyTitle}</h2>
        <p class="card-text">${escapeHtml(copy.foundationMethodologyText)}</p>
      </div>

      <div class="card soft">
        <h2 class="card-title">${documentScopeSummaryTitle}</h2>
        <p class="card-text">${documentScopeSummaryText}</p>
      </div>

      <div class="card">
        <h2 class="card-title">${entitySectionTitle}</h2>
        <div class="entity-grid">
          <div class="entity-item">
            <span class="label">${escapeHtml(copy.entityNameLabel)}</span>
            <span class="value">${metadata.companyName}</span>
          </div>
          <div class="entity-item">
            <span class="label">${escapeHtml(copy.countryLabel)}</span>
            <span class="value">${metadata.country}</span>
          </div>
          <div class="entity-item">
            <span class="label">${escapeHtml(copy.activitySectorLabel)}</span>
            <span class="value">${metadata.companySector}</span>
          </div>
          <div class="entity-item">
            <span class="label">${escapeHtml(copy.reportingYearLabel)}</span>
            <span class="value">${metadata.year}</span>
          </div>
          <div class="entity-item">
            <span class="label">${escapeHtml(copy.entityIdentifierLabel)}</span>
            <span class="value">${metadata.entityIdentifier}</span>
          </div>
          <div class="entity-item">
            <span class="label">${escapeHtml(copy.attestationReferenceLabel)}</span>
            <span class="value">${metadata.attestationId}</span>
          </div>
        </div>
      </div>

      <div class="two-col">
        <div class="card">
          <h2 class="card-title">${documentNatureSectionTitle}</h2>
          <p class="card-text"><strong>${documentNatureText}</strong></p>
        </div>

        <div class="card">
          <h2 class="card-title">${scopeSectionTitle}</h2>
          <p class="card-text"><strong>${escapeHtml(copy.scopeDescriptionLabel)} :</strong> ${scopeText}</p>
        </div>
      </div>

      <div class="features">
        <div class="feature-card">
          <div class="feature-title">${escapeHtml(copy.confidentialityTitle)}</div>
          <div class="feature-text">
            <ul>
              <li>${privacyText}</li>
              <li>${documentScopeSummaryText}</li>
              <li>${authenticityOverviewText}</li>
            </ul>
          </div>
        </div>

        <div class="feature-card">
          <div class="feature-title">${escapeHtml(copy.authenticityTitle)}</div>
          <div class="feature-text">${authenticityOverviewText}</div>
        </div>

        <div class="feature-card">
          <div class="feature-title">${escapeHtml(copy.validityTitle)}</div>
          <div class="feature-text">${documentValidityText}</div>
        </div>
      </div>
    </div>

    <div class="footer">
      <div>${footerText}</div>
      <div>${escapeHtml(copy.footerPageLabel)} 1 / 2</div>
    </div>
  </div>

  <div class="page page-two">
    <div class="content">
      <div class="page-two-top">
        <div>
          <h2 class="page-two-title">${escapeHtml(copy.pageTwoTitle)}</h2>
        </div>
        <div class="page-two-ref">
          <div><strong>${escapeHtml(copy.pageTwoReferenceLabel)} :</strong> ${metadata.attestationId}</div>
          <div><strong>${escapeHtml(copy.pageTwoIssuerLabel)} :</strong> ${metadata.issuerName}</div>
        </div>
      </div>

      <div class="card">
        <h2 class="card-title">${escapeHtml(copy.methodologyTitle)}</h2>
        <p class="card-text"><strong>${methodologyLabel}</strong><br/>${metadata.methodology}</p>

        <div class="formula-box">${escapeHtml(copy.formulaText)}</div>

        <div class="mini-grid">
          <div class="mini-box">
            <span class="mini-label">${escapeHtml(copy.factorVersionLabel)}</span>
            <span class="mini-value">${metadata.factorVersion}</span>
          </div>
          <div class="mini-box">
            <span class="mini-label">${transferabilityLabel}</span>
            <span class="mini-value">${transferabilityText}</span>
          </div>
        </div>
      </div>

      <div class="card">
        <h2 class="card-title">${escapeHtml(copy.referencesTitle)}</h2>
        <p class="card-text">${normativeText}</p>
        <ul class="reference-list">
          ${referencesList.map((item) => `<li>${escapeHtml(item)}</li>`).join("")}
        </ul>
        <div class="small-note">${scopeNote}</div>
      </div>

      <div class="verification-grid">
        <div class="verification-card primary">
          <h3>${escapeHtml(copy.verificationSimpleTitle)}</h3>
          <p>${escapeHtml(copy.useQrOrReferencePrefix)}</p>
          <p>
            <strong>${escapeHtml(copy.pageVerificationLabel)}</strong><br/>
            <a href="${verificationDisplayUrl}" class="verification-link">${verificationDisplayUrl}</a><br/>
            <span class="small-note">${escapeHtml(copy.pageTwoReferenceLabel)} : ${metadata.attestationId}</span>
          </p>
        </div>

        <div class="verification-card">
          <h3>${escapeHtml(copy.verifiableObjectTitle)}</h3>
          <p>${verificationBoxTitle}</p>
          <p>${technicalVerificationNote}</p>
        </div>
      </div>

      <div class="card tech-card">
        <h2 class="card-title">${escapeHtml(copy.technicalElementsTitle)}</h2>
        <p class="small-note">${escapeHtml(copy.technicalElementsIntro)}</p>

        <div class="technical-grid">
          <div class="tech-label">${escapeHtml(copy.algorithmLabel)}</div>
          <div class="tech-value">${metadata.algorithm}</div>

          <div class="tech-label">${escapeHtml(copy.hashLabel)}</div>
          <div class="tech-value">${metadata.hash}</div>

          <div class="tech-label">${escapeHtml(copy.signatureLabel)}</div>
          <div class="tech-value">${metadata.signature}</div>

          <div class="tech-label">${escapeHtml(copy.publicKeyLabel)}</div>
          <div class="tech-value">${metadata.publicKey}</div>
        </div>
      </div>

      <div class="card tint">
        <h2 class="card-title">${escapeHtml(copy.perimeterLimitsTitle)}</h2>
        <p class="card-text">${limitationsText}</p>

        <div class="limit-grid">
          <div>
            <div class="limit-subtitle">${escapeHtml(copy.liabilityLabel)}</div>
            <div class="card-text">${liabilityText}</div>
          </div>

          <div>
            <div class="limit-subtitle">${escapeHtml(copy.validityPeriodLabel)}</div>
            <div class="card-text">${documentValidityText}</div>
          </div>
        </div>

        <div class="language-note">${escapeHtml(copy.languageNotice)}</div>
        <div class="doc-standard">${metadata.standardRef} · ${methodologyNote}</div>
      </div>
    </div>

    <div class="footer">
      <div>${footerText}</div>
      <div>${escapeHtml(copy.footerPageLabel)} 2 / 2</div>
    </div>
  </div>
</body>
</html>
`;

    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 15000);

    const pdfResponse = await fetch("https://api.pdfshift.io/v3/convert/pdf", {
      method: "POST",
      headers: {
        "X-API-Key": process.env.PDFSHIFT_API_KEY!,
        "Content-Type": "application/json",
      },
      body: JSON.stringify({
        source: html,
        format: "A4",
        use_print: true,
      }),
      signal: controller.signal,
    });

    clearTimeout(timeoutId);

    if (!pdfResponse.ok) {
      const errorText = await pdfResponse.text();
      return new Response(errorText, { status: pdfResponse.status });
    }

    const pdfBuffer = Buffer.from(await pdfResponse.arrayBuffer());

    const safeIssuerName = issuerNameRaw
      .toLowerCase()
      .replace(/[^a-z0-9-]+/g, "-")
      .replace(/^-+|-+$/g, "") || "certif-scope";

    return new Response(pdfBuffer, {
      headers: {
        "Content-Type": "application/pdf",
        "Content-Disposition": `attachment; filename="${safeIssuerName}-${attestationId}.pdf"`,
        "Cache-Control": "no-store",
        "X-Content-Type-Options": "nosniff",
      },
    });
  } catch (error) {
    if (process.env.NODE_ENV !== "production") {
      console.error(error);
    }

    return new Response("Internal Server Error", { status: 500 });
  }
      }
