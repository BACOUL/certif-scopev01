export const runtime = "nodejs";

import Stripe from "stripe";
import QRCode from "qrcode";
import { signCanonicalPayload, makeAttestationId } from "@/lib/sign";
import {
  ATTESTATION_I18N,
  AttestationLocale,
  DEFAULT_ATTESTATION_LOCALE,
} from "@/lib/attestation-i18n/index";

const stripe = new Stripe(process.env.STRIPE_SECRET_KEY!);

/**
 * Logo Certif-Scope en base64.
 *
 * IMPORTANT :
 * - Colle ton base64 complet ici.
 * - Tu peux le coller sur plusieurs lignes.
 * - Ne mets PAS "data:image/png;base64,".
 * - Le .replace(/\s+/g, "") supprime automatiquement les retours à la ligne.
 */
const RAW_CERTIF_SCOPE_LOGO_BASE64 = `

`.replace(/\s+/g, "");

const CERTIF_SCOPE_LOGO_BASE64 = RAW_CERTIF_SCOPE_LOGO_BASE64;

function escapeHtml(input: string) {
  return input
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
    .replace(/"/g, "&quot;")
    .replace(/'/g, "&#39;");
}

function toBase64Url(input: string) {
  return Buffer.from(input, "utf8")
    .toString("base64")
    .replace(/\+/g, "-")
    .replace(/\//g, "_")
    .replace(/=+$/g, "");
}

function addMonthsToISODate(date: string, months: number) {
  const parsed = new Date(`${date}T00:00:00.000Z`);

  if (Number.isNaN(parsed.getTime())) {
    return "";
  }

  parsed.setUTCMonth(parsed.getUTCMonth() + months);

  return parsed.toISOString().slice(0, 10);
}

function cleanSectionTitle(title: string) {
  return title.replace(/^\s*\d+\.\s*/g, "").trim();
}

function sectionTitle(index: number, title: string) {
  return `${index}. ${cleanSectionTitle(title)}`;
}

export async function GET(req: Request) {
  try {
    if (!process.env.PDFSHIFT_API_KEY) {
      return new Response("PDFSHIFT_API_KEY missing", { status: 500 });
    }

    const { searchParams } = new URL(req.url);
    const sessionId = searchParams.get("session_id");

    if (!sessionId) {
      return new Response("Missing session_id", { status: 400 });
    }

    let metadataRaw: Record<string, any> = {};

    if (sessionId.startsWith("key_")) {
      metadataRaw = Object.fromEntries(searchParams.entries());
    } else {
      const session = await stripe.checkout.sessions.retrieve(sessionId);

      if (session.payment_status !== "paid") {
        return new Response("Payment not completed", { status: 403 });
      }

      metadataRaw = session.metadata || {};
    }

    const locale =
      (metadataRaw.attestationLocale as AttestationLocale) ||
      DEFAULT_ATTESTATION_LOCALE;

    const i18n = ATTESTATION_I18N[locale] || ATTESTATION_I18N.en;

    const required = ["companyName", "totalCO2e", "year"];
    const missing = required.filter((k) => {
      const v = metadataRaw[k];
      return v === undefined || v === null || String(v).trim() === "";
    });

    if (missing.length) {
      return new Response(`Missing metadata: ${missing.join(", ")}`, {
        status: 400,
      });
    }

    const totalCO2eNum = Number(String(metadataRaw.totalCO2e).replace(",", "."));

    if (Number.isNaN(totalCO2eNum)) {
      return new Response("Invalid metadata: totalCO2e must be a number", {
        status: 400,
      });
    }

    const issuedDate = new Date().toISOString();

    const canonicalPayload = {
      issuer: "Certif-Scope" as const,
      standard: "CS-SB-v1" as const,
      attestationId: "",
      companyName: String(metadataRaw.companyName),
      country: String(metadataRaw.country || "—"),
      year: String(metadataRaw.year),
      totalCO2e: String(totalCO2eNum),
      issuedDate,
    };

    const tempSignature = signCanonicalPayload({
      ...canonicalPayload,
      attestationId: "TEMP",
    });

    const attestationId = makeAttestationId(
      canonicalPayload.year,
      tempSignature.hashHex
    );

    const signatureResult = signCanonicalPayload({
      ...canonicalPayload,
      attestationId,
    });

    const issuedDateForDisplay = issuedDate.slice(0, 10);
    const validityMonths = String(metadataRaw.validityMonths || "12");
    const validUntil =
      String(metadataRaw.validUntil || "").trim() ||
      addMonthsToISODate(issuedDateForDisplay, Number(validityMonths) || 12);

    const metadata = {
      attestationId,
      issuerName: escapeHtml(String(metadataRaw.issuerName || "Certif-Scope")),
      issuerSite: escapeHtml(
        String(metadataRaw.issuerSite || "https://www.certif-scope.com")
      ),
      companyName: escapeHtml(String(metadataRaw.companyName)),
      companySector: escapeHtml(String(metadataRaw.companySector || "—")),
      entityIdentifier: escapeHtml(String(metadataRaw.entityIdentifier || "—")),
      country: escapeHtml(String(metadataRaw.country || "—")),
      year: escapeHtml(String(metadataRaw.year)),
      totalCO2e: escapeHtml(
        totalCO2eNum.toLocaleString("fr-FR", {
          maximumFractionDigits: 2,
          minimumFractionDigits: totalCO2eNum % 1 === 0 ? 0 : 2,
        })
      ),
      methodology: escapeHtml(
        String(
          metadataRaw.methodology ||
            "Certif-Scope deterministic spend-based methodology v1.0"
        )
      ),
      issuedDate: escapeHtml(issuedDateForDisplay),
      validUntil: escapeHtml(validUntil),
      validityMonths: escapeHtml(validityMonths),
      standardRef: escapeHtml(
        String(metadataRaw.standardRef || "Certif-Scope CS-SB-v1")
      ),
      signature: signatureResult.signatureBase64,
      hash: signatureResult.hashHex,
      algorithm: signatureResult.algorithm,
    };

    const verificationPayload = {
      certificateId: metadata.attestationId,
      issuer: "Certif-Scope",
      issuedAt: metadata.issuedDate,
      validUntil: metadata.validUntil,
      methodVersion: "CS-SB-v1",
      factorVersion: String(
        metadataRaw.factorVersion ||
          metadataRaw.emissionFactorVersion ||
          "Certif-Scope factors v1"
      ),
      algorithm: metadata.algorithm,
      hash: metadata.hash,
      signature: metadata.signature,
    };

    const verificationToken = toBase64Url(JSON.stringify(verificationPayload));

    const verifyUrl =
      locale === "fr"
        ? `https://www.certif-scope.com/fr/verify/?v=${verificationToken}`
        : `https://www.certif-scope.com/verify/?v=${verificationToken}`;

    const verificationDisplayUrl =
      locale === "fr"
        ? "https://www.certif-scope.com/fr/verify"
        : "https://www.certif-scope.com/verify";

    const qrDataUrl = await QRCode.toDataURL(verifyUrl, {
      width: 140,
      margin: 1,
    });

    const html = `
<!doctype html>
<html lang="${locale}">
<head>
<meta charset="utf-8"/>
<title>${metadata.issuerName} — Attestation</title>
<style>
  @page {
    size: A4;
    margin: 10mm 12mm 9mm 12mm;
  }

  * {
    box-sizing: border-box;
  }

  body {
    font-family: Inter, "Helvetica Neue", Arial, Helvetica, sans-serif;
    font-size: 9.4px;
    line-height: 1.3;
    margin: 0;
    color: #102033;
    -webkit-font-smoothing: antialiased;
  }

  :root {
    --serif: "Times New Roman", Georgia, "Garamond", serif;
    --muted: #64748b;
    --text: #102033;
    --accent: #0B3A63;
    --turquoise: #1FB6C1;
    --border: #cfd8e3;
    --border-soft: #e2e8f0;
    --bg-soft: #f8fafc;
    --bg-blue: #f3fbfc;
  }

  a {
    color: inherit;
    text-decoration: none;
  }

  .container {
    padding: 0;
    width: 100%;
  }

  .page {
    width: 100%;
  }

  .page-two {
    padding-top: 0;
    margin-top: 0;
  }

  .footer-static {
    width: 100%;
    font-size: 7.8px;
    color: var(--muted);
    display: flex;
    justify-content: space-between;
    align-items: center;
    border-top: 1px solid var(--border-soft);
    padding-top: 5px;
    margin-top: 7px;
    clear: both;
  }

  .top-header {
    display: flex;
    justify-content: space-between;
    align-items: flex-start;
    gap: 16px;
    border-bottom: 3px solid var(--accent);
    padding-bottom: 6px;
    margin-bottom: 7px;
  }

  .issuer {
    max-width: 72%;
    display: flex;
    flex-direction: column;
    justify-content: flex-start;
  }
  
  .issuer-logo {
    height: 56px;
    max-width: 250px;
    width: auto;
    display: block;
    margin-bottom: 3px;
    object-fit: contain;
  }

  .issuer-site {
    font-size: 7.6px;
    color: var(--muted);
    margin-bottom: 2px;
  }

  .issuer-meta {
    font-size: 7.6px;
    color: var(--muted);
  }

  .qr {
    text-align: center;
    font-size: 7.4px;
    flex: 0 0 auto;
    color: var(--accent);
    font-weight: 700;
  }

  .qr img { 
    width: 80px;
    height: 80px;
    border: 1px solid var(--border-soft);
    border-radius: 4px;
    padding: 4px;
    background: #fff;
  }

  .kicker {
    text-align: center;
    font-size: 7.5px;
    color: var(--turquoise);
    font-weight: 800;
    letter-spacing: 2px;
    text-transform: uppercase;
    margin: 5px 0 3px;
  }

  .title {
    text-align: center;
    margin: 4px 0 7px;
    font-family: var(--serif);
  }

  .title h1 { 
    font-size: 20px; 
    margin: 0; 
    font-weight: 800; 
    letter-spacing: 0.6px; 
    text-transform: uppercase; 
    color: var(--accent); 
  }

  .title .formal-line {
    margin-top: 3px;
    font-size: 9px;
    color: #111827;
    font-weight: 700;
    font-family: Inter, Arial, sans-serif;
  }

  .title .subtitle {
    margin-top: 3px;
    font-size: 8px;
    color: var(--muted);
    font-family: Inter, Arial, sans-serif;
  }

  .result-box {
    width: 74%;
    margin: 0 auto 8px;
    background: #ffffff;
    border: 2px solid var(--accent);
    border-radius: 5px;
    padding: 8px 14px 7px;
    text-align: center;
  }

  .result-label {
    font-size: 8px;
    font-weight: 800;
    color: #334155;
    margin-bottom: 3px;
    text-transform: uppercase;
    letter-spacing: 0.7px;
  }

  .result-value {
    font-family: var(--serif);
    font-size: 25px;
    font-weight: 900;
    color: var(--accent);
    margin: 0;
    letter-spacing: 1px;
  }

  .result-value span {
    font-size: 11px;
    color: var(--turquoise);
    font-family: Inter, Arial, sans-serif;
    letter-spacing: 0;
  }

  .result-note {
    font-size: 7.8px;
    color: var(--muted);
    margin-top: 2px;
  }

  .card {
    border: 1px solid var(--border);
    border-radius: 5px;
    background: #fff;
    overflow: hidden;
    margin-bottom: 7px;
    page-break-inside: avoid;
    break-inside: avoid;
  }

  .card-soft {
    background: var(--bg-soft);
  }

  .card-blue {
    background: #f3fbfc;
    border-color: #b8e6ea;
  }

  .card-head {
    padding: 5px 8px;
    background: linear-gradient(90deg, #eefafa, #f8fafc);
    border-bottom: 1px solid var(--border-soft);
    font-size: 8px;
    font-weight: 900;
    color: var(--accent);
    letter-spacing: 0.6px;
    text-transform: uppercase;
  }

  .card-body {
    padding: 7px 8px;
  }

  .verification-table {
    width: 100%;
    border-collapse: collapse;
    table-layout: fixed;
  }

  .verification-table td {
    border-right: 1px solid var(--border-soft);
    padding: 5px 8px;
    vertical-align: top;
  }

  .verification-table td:last-child {
    border-right: 0;
  }

  .label {
    display: block;
    font-size: 7px;
    color: var(--muted);
    text-transform: uppercase;
    letter-spacing: 0.6px;
    margin-bottom: 2px;
  }

  .value {
    font-size: 8.5px;
    font-weight: 800;
    color: #111827;
    word-break: break-word;
  }

  .highlight-card {
    border-left: 4px solid var(--turquoise);
    background: #f2fbfc;
  }

  .highlight-card .card-body {
    padding: 7px 9px;
  }

  .plain-card .card-body {
    padding: 7px 9px;
  }

  .entity-grid {
    display: grid;
    grid-template-columns: 1fr 1fr 1fr;
    gap: 7px 12px;
  }

  .two-cards {
    display: grid;
    grid-template-columns: 1fr 1fr;
    gap: 8px;
    margin-bottom: 7px;
  }

  .three-cards {
    display: grid;
    grid-template-columns: 1fr 1fr 1fr;
    gap: 8px;
    margin-bottom: 7px;
  }

  .mini-card {
    border: 1px solid var(--border);
    border-radius: 5px;
    padding: 7px 8px;
    background: var(--bg-soft);
    min-height: 58px;
  }

  .mini-card h3 {
    margin: 0 0 4px;
    color: var(--accent);
    font-size: 8.5px;
    text-transform: uppercase;
    letter-spacing: 0.5px;
  }

  .mini-card p,
  .mini-card li {
    margin: 0;
    font-size: 8px;
    color: #334155;
  }

  .mini-card ul {
    margin: 0;
    padding-left: 11px;
  }

  .page-two-header {
    display: flex;
    justify-content: space-between;
    align-items: flex-end;
    border-bottom: 3px solid var(--accent);
    padding-bottom: 5px;
    margin-bottom: 8px;
  }

  .page-two-header h2 {
    margin: 0;
    color: var(--accent);
    font-size: 17px;
    letter-spacing: 0.8px;
    text-transform: uppercase;
  }

  .page-two-header .doc-meta {
    text-align: right;
    color: var(--muted);
    font-size: 7.5px;
    line-height: 1.25;
  }

  .method-box {
    text-align: center;
    background: #eefafa;
    border: 1px solid #cbecef;
    border-radius: 4px;
    padding: 6px;
    color: var(--accent);
    font-weight: 900;
    margin-top: 6px;
  }

  .two-info {
    display: grid;
    grid-template-columns: 1fr 1fr;
    gap: 8px;
    margin-top: 7px;
  }

  .info-cell {
    background: var(--bg-soft);
    border: 1px solid var(--border-soft);
    border-radius: 4px;
    padding: 6px 7px;
  }

  .references-grid {
    display: grid;
    grid-template-columns: 1fr 1fr;
    gap: 3px 20px;
    padding-left: 12px;
    margin: 5px 0 0;
  }

  .references-grid li {
    margin-bottom: 2px;
  }

  .technical-table {
    width: 100%;
    border-collapse: collapse;
    table-layout: fixed;
    margin-top: 5px;
    border-top: 1px solid var(--border-soft);
  }

  .technical-table td {
    padding: 4px 5px;
    vertical-align: top;
    border-bottom: 1px solid #edf2f7;
    font-size: 7.4px;
  }

  .technical-table td:first-child {
    width: 31%;
    color: var(--muted);
    text-transform: uppercase;
    letter-spacing: 0.4px;
    font-weight: 700;
  }

  .technical-table td:last-child {
    color: #334155;
    word-break: break-all;
  }

  .bottom-card {
    border: 1px solid var(--border);
    border-left: 4px solid var(--accent);
    border-radius: 5px;
    background: #fbfeff;
    padding: 8px 10px;
    margin-top: 7px;
    page-break-inside: avoid;
    break-inside: avoid;
  }

  .bottom-card h3 {
    margin: 0 0 5px;
    color: var(--accent);
    font-size: 9.2px;
    text-transform: uppercase;
    letter-spacing: 0.5px;
  }

  .bottom-grid {
    display: grid;
    grid-template-columns: 1fr 1fr;
    gap: 8px;
    margin-top: 6px;
  }

  .bottom-cell {
    background: var(--bg-soft);
    border: 1px solid var(--border-soft);
    border-radius: 4px;
    padding: 6px 7px;
  }

  .small {
    font-size: 7.8px;
    color: var(--muted);
    line-height: 1.25;
  }

  .text {
    font-size: 8.3px;
    color: #334155;
    line-height: 1.3;
  }

  .strong {
    font-weight: 800;
    color: #111827;
  }

  .mt-4 {
    margin-top: 4px;
  }

  .mt-6 {
    margin-top: 6px;
  }

  .avoid-break {
    page-break-inside: avoid;
    break-inside: avoid;
  }
</style>
</head>
<body>
<div class="container">
  <div class="page page-one">
    <header class="top-header">
      <div class="issuer">
        ${
          CERTIF_SCOPE_LOGO_BASE64
            ? `<img src="data:image/png;base64,${CERTIF_SCOPE_LOGO_BASE64}" class="issuer-logo" alt="Certif-Scope logo" />`
            : ""
        }
        <div class="issuer-site">${metadata.issuerSite}</div>
        <div class="issuer-meta">${i18n.headerTagline}</div>
      </div>

      <div class="qr">
        <img src="${qrDataUrl}" alt="QR verification" />
        <div>${i18n.scanToVerifyLabel}</div>
      </div>
    </header>

    <div class="kicker">Document CO₂e indicatif · standardisé · vérifiable</div>

    <div class="title">
      <h1>Attestation CO₂e indicative</h1>
      <div class="formal-line">Estimation agrégée fondée sur une méthodologie spend-based standardisée</div>
      <div class="subtitle">Document méthodologique pour première estimation carbone documentaire · ${metadata.standardRef}</div>
    </div>

    <div class="result-box">
      <div class="result-label">${i18n.resultLabel}</div>
      <div class="result-value">${metadata.totalCO2e} <span>tCO₂e</span></div>
      <div class="result-note">Estimation indicative issue des données déclarées par l’entité.</div>
    </div>

    <section class="card avoid-break">
      <div class="card-head">Document vérifiable</div>
      <table class="verification-table">
        <tr>
          <td>
            <span class="label">Référence</span>
            <span class="value">${metadata.attestationId}</span>
          </td>
          <td>
            <span class="label">${i18n.issuedDateLabel}</span>
            <span class="value">${metadata.issuedDate}</span>
          </td>
          <td>
            <span class="label">${i18n.validUntilLabel}</span>
            <span class="value">${metadata.validUntil}</span>
          </td>
          <td>
            <span class="label">${i18n.issuerLabel}</span>
            <span class="value">${metadata.issuerName}</span>
          </td>
        </tr>
      </table>
    </section>

    <section class="card highlight-card avoid-break">
      <div class="card-body">
        <div class="label" style="color: var(--accent); font-weight: 900;">Fondement méthodologique</div>
        <div class="text">
          Cette attestation fournit une estimation CO₂e indicative, standardisée et vérifiable, fondée sur une méthodologie spend-based utilisant des données de dépenses agrégées déclarées et des facteurs d’émission monétaires.
        </div>
      </div>
    </section>

    <section class="card plain-card avoid-break">
      <div class="card-body">
        <div class="label" style="color: var(--accent); font-weight: 900;">Usage prévu</div>
        <div class="text">
          Cette attestation peut être utilisée comme document indicatif dans un dossier fournisseur, une demande client, un appel d’offres, une demande bancaire, une demande d’assurance ou une démarche interne de première estimation carbone, lorsque le besoin porte sur un indicateur CO₂e agrégé et documenté.
        </div>
      </div>
    </section>

    <section class="card avoid-break">
      <div class="card-head">${sectionTitle(3, i18n.entitySectionTitle)}</div>
      <div class="card-body">
        <div class="entity-grid">
          <div>
            <span class="label">${i18n.entityNameLabel}</span>
            <span class="value">${metadata.companyName}</span>
          </div>
          <div>
            <span class="label">${i18n.countryLabel}</span>
            <span class="value">${metadata.country}</span>
          </div>
          <div>
            <span class="label">${i18n.reportingYearLabel}</span>
            <span class="value">${metadata.year}</span>
          </div>
          <div>
            <span class="label">${i18n.activitySectorLabel}</span>
            <span class="value">${metadata.companySector}</span>
          </div>
          <div>
            <span class="label">${i18n.entityIdentifierLabel}</span>
            <span class="value">${metadata.entityIdentifier}</span>
          </div>
          <div>
            <span class="label">${i18n.uniqueIdentifierLabel}</span>
            <span class="value">${metadata.attestationId}</span>
          </div>
        </div>
      </div>
    </section>

    <div class="two-cards">
      <section class="card plain-card avoid-break">
        <div class="card-body">
          <div class="label" style="color: var(--accent); font-weight: 900;">${sectionTitle(2, i18n.documentNatureSectionTitle)}</div>
          <div class="text strong">
            ${i18n.documentNatureText}
          </div>
        </div>
      </section>

      <section class="card plain-card avoid-break">
        <div class="card-body">
          <div class="label" style="color: var(--accent); font-weight: 900;">${sectionTitle(4, i18n.scopeSectionTitle)}</div>
          <div class="text">
            <span class="strong">${i18n.scopeDescriptionLabel} :</span> ${i18n.scopeText}
          </div>
        </div>
      </section>
    </div>

    <div class="three-cards">
      <div class="mini-card">
        <h3>Confidentialité</h3>
        <ul>
          <li>Aucune conservation des données financières détaillées.</li>
          <li>Résultat CO₂e agrégé uniquement.</li>
          <li>Attestation vérifiable indépendamment.</li>
        </ul>
      </div>

      <div class="mini-card">
        <h3>Authenticité</h3>
        <p>L’attestation comporte une référence unique, un QR code et des éléments de contrôle documentaire.</p>
      </div>

      <div class="mini-card">
        <h3>Validité</h3>
        <p>La validité documentaire est limitée dans le temps et indiquée sur l’attestation.</p>
      </div>
    </div>

    <div class="footer-static">
      <div>${i18n.footerText}</div>
      <div>${i18n.pageLabel} 1 / 2</div>
    </div>
  </div>

  <div style="page-break-after: always;"></div>

  <div class="page page-two">
    <header class="page-two-header">
      <h2>Méthodologie, références et vérification</h2>
      <div class="doc-meta">
        Référence : ${metadata.attestationId}<br/>
        Émetteur : ${metadata.issuerName}
      </div>
    </header>

    <section class="card avoid-break">
      <div class="card-body">
        <div class="label" style="color: var(--accent); font-weight: 900;">Méthodologie</div>
        <div class="text strong">${metadata.methodology}</div>

        <div class="method-box">
          Dépenses agrégées déclarées × facteurs d’émission monétaires = estimation CO₂e indicative
        </div>

        <div class="two-info">
          <div class="info-cell">
            <span class="label">Version des facteurs</span>
            <span class="value">${escapeHtml(
              String(
                metadataRaw.factorVersion ||
                  metadataRaw.emissionFactorVersion ||
                  "Certif-Scope factors v1"
              )
            )}</span>
          </div>
          <div class="info-cell">
            <span class="label">${i18n.transferabilityLabel}</span>
            <span class="value">${i18n.transferabilityText}</span>
          </div>
        </div>
      </div>
    </section>

    <section class="card avoid-break">
      <div class="card-body">
        <div class="label" style="color: var(--accent); font-weight: 900;">Références de cohérence</div>
        <div class="text">
          La méthodologie Certif-Scope s’inscrit dans une logique de cohérence avec les cadres de référence carbone et de transparence environnementale suivants :
        </div>

        <ul class="references-grid text">
          <li>GHG Protocol — Scope 3, méthode spend-based</li>
          <li>ISO 14064-1 — quantification et reporting GES</li>
          <li>ISO 14083 — émissions liées aux opérations de transport</li>
          <li>CSRD / ESRS — transparence environnementale européenne</li>
          <li>Taxonomie européenne — contexte durabilité et reporting extra-financier</li>
        </ul>

        <div class="small mt-4">
          Ces références sont citées comme cadre de cohérence méthodologique et contextuel. Elles ne transforment pas cette attestation en rapport réglementaire, audit certifié ou vérification par tiers indépendant.
        </div>
      </div>
    </section>

    <div class="two-cards">
      <section class="card highlight-card avoid-break">
        <div class="card-body">
          <div class="label" style="color: var(--accent); font-weight: 900;">Vérification simple</div>
          <div class="text">
            Scanner le QR code ou utiliser la référence d’attestation sur la page officielle de vérification.
          </div>
          <div class="mt-4">
            <span class="label" style="color: var(--accent); font-weight: 900;">Page de vérification documentaire</span>
            <span class="value">${verificationDisplayUrl}</span>
          </div>
          <div class="small mt-4">Référence : ${metadata.attestationId}</div>
        </div>
      </section>

      <section class="card card-soft avoid-break">
        <div class="card-body">
          <div class="label" style="color: var(--accent); font-weight: 900;">Objet vérifiable</div>
          <div class="text strong">
            Le PDF signé et son QR code constituent les éléments de contrôle documentaire.
          </div>
          <div class="small mt-6">
            Le document PDF signé et son QR code de vérification constituent les éléments de contrôle documentaire.
          </div>
        </div>
      </section>
    </div>

    <section class="card avoid-break">
      <div class="card-body">
        <div class="label" style="color: var(--accent); font-weight: 900;">Éléments techniques de vérification</div>
        <div class="small">
          Les éléments ci-dessous permettent une vérification documentaire avancée. Aucune action n’est requise pour un lecteur standard du document.
        </div>

        <table class="technical-table">
          <tr>
            <td>${i18n.algorithmLabel}</td>
            <td>${metadata.algorithm}</td>
          </tr>
          <tr>
            <td>${i18n.hashLabel}</td>
            <td>${metadata.hash}</td>
          </tr>
          <tr>
            <td>${i18n.signatureLabel}</td>
            <td>${metadata.signature}</td>
          </tr>
          <tr>
            <td>${i18n.publicKeyLabel}</td>
            <td>MCowBQYDK2VwAyEAbKp2pg4wmzE5Kqo9tEwv7JJjxQyT2cBmwiLLHp4cSac=</td>
          </tr>
        </table>
      </div>
    </section>

    <section class="bottom-card avoid-break">
      <h3>Périmètre et limites</h3>
      <div class="text">
        Cette attestation fournit un indicateur CO₂e agrégé, documenté et vérifiable, destiné aux échanges administratifs, fournisseurs, bancaires ou internes nécessitant une première estimation carbone.
      </div>

      <div class="text mt-4">
        Elle repose sur les données déclarées par l’entité et sur une méthodologie spend-based. Elle ne remplace pas un audit carbone, un inventaire GES complet, une vérification par tiers indépendant, un Bilan Carbone® ou un reporting réglementaire lorsque ceux-ci sont explicitement exigés.
      </div>

      <div class="bottom-grid">
        <div class="bottom-cell">
          <span class="label" style="color: var(--accent); font-weight: 900;">Responsabilité des données</span>
          <div class="text">
            Les résultats dépendent de l’exactitude, de la complétude et de la cohérence des données déclarées par l’entité.
          </div>
        </div>

        <div class="bottom-cell">
          <span class="label" style="color: var(--accent); font-weight: 900;">Validité documentaire</span>
          <div class="text">
            La période de validité reflète la pertinence temporelle des données déclarées, des facteurs d’émission utilisés et de la méthodologie appliquée.
          </div>
        </div>
      </div>

      <div class="small mt-6">
        <strong>Langue du document :</strong> ${i18n.englishPrevailsNotice}
      </div>

      <div class="small mt-4">
        CS-SB-v1 est une méthodologie standardisée interne maintenue par Certif-Scope.
      </div>
    </section>

    <div class="footer-static">
      <div>${i18n.footerText}</div>
      <div>${i18n.pageLabel} 2 / 2</div>
    </div>
  </div>
</div>
</body>
</html>
`;

    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 15000);

    const pdfResponse = await fetch("https://api.pdfshift.io/v3/convert/pdf", {
      method: "POST",
      headers: {
        "X-API-Key": process.env.PDFSHIFT_API_KEY!,
        "Content-Type": "application/json",
      },
      body: JSON.stringify({
        source: html,
        format: "A4",
        use_print: true,
      }),
      signal: controller.signal,
    });

    clearTimeout(timeoutId);

    if (!pdfResponse.ok) {
      const error = await pdfResponse.text();
      return new Response(error, { status: pdfResponse.status });
    }

    const pdfBuffer = Buffer.from(await pdfResponse.arrayBuffer());

    const safeIssuerName = metadata.issuerName
      .toLowerCase()
      .replace(/[^a-z0-9\-]/g, "");

    return new Response(pdfBuffer, {
      headers: {
        "Content-Type": "application/pdf",
        "Content-Disposition": `attachment; filename="${safeIssuerName}-${metadata.attestationId}.pdf"`,
        "Cache-Control": "no-store",
        "X-Content-Type-Options": "nosniff",
      },
    });
  } catch (err) {
    if (process.env.NODE_ENV !== "production") {
      console.error(err);
    }

    return new Response("Internal Server Error", { status: 500 });
  }
                          }
