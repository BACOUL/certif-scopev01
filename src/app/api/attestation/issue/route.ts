export const runtime = "nodejs";

import Stripe from "stripe";
import QRCode from "qrcode";
import { signCanonicalPayload, makeAttestationId } from "@/lib/sign";
import {
  ATTESTATION_I18N,
  AttestationLocale,
  DEFAULT_ATTESTATION_LOCALE,
} from "@/lib/attestation-i18n/index";

const stripe = new Stripe(process.env.STRIPE_SECRET_KEY!);

/**
 * Logo Certif-Scope en base64.
 *
 * IMPORTANT :
 * - Colle ici ton vrai base64 complet.
 * - Ne mets PAS "data:image/png;base64,".
 * - Tu peux le coller sur plusieurs lignes.
 * - Le .replace(/\s+/g, "") supprime automatiquement les retours à la ligne.
 */
const RAW_CERTIF_SCOPE_LOGO_BASE64 = `
";
`.replace(/\s+/g, "");

const CERTIF_SCOPE_LOGO_BASE64 = RAW_CERTIF_SCOPE_LOGO_BASE64;

type PremiumCopy = {
  eyebrow: string;
  title: string;
  subtitle: string;
  positioningLine: string;

  resultFootnote: string;

  documentVerificationTitle: string;
  statusReference: string;
  statusIssuedDate: string;
  statusValidUntil: string;
  statusIssuer: string;

  methodologicalBasisTitle: string;
  methodologicalBasisText: string;

  intendedUseTitle: string;
  intendedUseText: string;

  referencesTitle: string;
  referencesIntro: string;
  referencesItems: string[];
  referencesContext: string;

  privacyTitle: string;
  privacyBullets: string[];

  authenticityTitle: string;
  authenticityText: string;

  validityTitle: string;
  validityText: string;

  methodFormula: string;
  factorVersionLabel: string;

  simpleVerificationTitle: string;
  simpleVerificationText: string;
  technicalVerificationTitle: string;
  technicalVerificationIntro: string;
  technicalElementsTitle: string;

  verifiableObjectTitle: string;
  verifiableObjectText: string;

  scopeAndLimitsTitle: string;
  scopeAndLimitsText: string;

  finalResponsibilityTitle: string;
  finalResponsibilityText: string;

  finalValidityTitle: string;
  finalValidityText: string;

  finalLanguageNoticeTitle: string;
};

const PREMIUM_COPY: Record<"fr" | "en", PremiumCopy> = {
  fr: {
    eyebrow: "Document CO₂e indicatif · standardisé · vérifiable",
    title: "Attestation CO₂e indicative",
    subtitle:
      "Estimation agrégée fondée sur une méthodologie spend-based standardisée",
    positioningLine:
      "Document méthodologique pour première estimation carbone documentaire",

    resultFootnote:
      "Estimation indicative issue des données déclarées par l’entité.",

    documentVerificationTitle: "Document vérifiable",
    statusReference: "Référence",
    statusIssuedDate: "Date d’émission",
    statusValidUntil: "Valable jusqu’au",
    statusIssuer: "Émetteur",

    methodologicalBasisTitle: "Fondement méthodologique",
    methodologicalBasisText:
      "Cette attestation fournit une estimation CO₂e indicative, standardisée et vérifiable, fondée sur une méthodologie spend-based utilisant des données de dépenses agrégées déclarées et des facteurs d’émission monétaires.",

    intendedUseTitle: "Usage prévu",
    intendedUseText:
      "Cette attestation peut être utilisée comme document indicatif dans un dossier fournisseur, une demande client, un appel d’offres, une demande bancaire, une demande d’assurance ou une démarche interne de première estimation carbone, lorsque le besoin porte sur un indicateur CO₂e agrégé et documenté.",

    referencesTitle: "Références de cohérence",
    referencesIntro:
      "La méthodologie Certif-Scope s’inscrit dans une logique de cohérence avec les cadres de référence carbone et de transparence environnementale suivants :",
    referencesItems: [
      "GHG Protocol — Scope 3, méthode spend-based",
      "ISO 14064-1 — quantification et reporting GES",
      "ISO 14083 — émissions liées aux opérations de transport",
      "CSRD / ESRS — transparence environnementale européenne",
      "Taxonomie européenne — contexte durabilité et reporting extra-financier",
    ],
    referencesContext:
      "Ces références sont citées comme cadre de cohérence méthodologique et contextuelle. Elles ne transforment pas cette attestation en rapport réglementaire, audit certifié ou vérification par tiers indépendant.",

    privacyTitle: "Confidentialité",
    privacyBullets: [
      "Aucune conservation des données financières détaillées.",
      "Résultat CO₂e agrégé uniquement.",
      "Attestation vérifiable indépendamment.",
    ],

    authenticityTitle: "Authenticité",
    authenticityText:
      "L’attestation comporte une référence unique, un QR code et des éléments de contrôle documentaire.",

    validityTitle: "Validité",
    validityText:
      "La validité documentaire est limitée dans le temps et indiquée sur l’attestation.",

    methodFormula:
      "Dépenses agrégées déclarées × facteurs d’émission monétaires = estimation CO₂e indicative",
    factorVersionLabel: "Version des facteurs",

    simpleVerificationTitle: "Vérification simple",
    simpleVerificationText:
      "Scanner le QR code ou utiliser la référence d’attestation sur la page officielle de vérification.",
    technicalVerificationTitle: "Vérification technique indépendante",
    technicalVerificationIntro:
      "Les éléments ci-dessous permettent une vérification documentaire avancée. Aucune action n’est requise pour une lecture standard du document.",
    technicalElementsTitle: "Éléments techniques de vérification",

    verifiableObjectTitle: "Objet vérifiable",
    verifiableObjectText:
      "Le PDF signé et son QR code constituent les éléments de contrôle documentaire.",

    scopeAndLimitsTitle: "Périmètre et limites",
    scopeAndLimitsText:
      "Cette attestation est indicative. Elle repose sur les données déclarées par l’entité et sur une méthodologie spend-based. Elle ne remplace pas un audit carbone, un inventaire GES complet, une vérification par tiers indépendant, un Bilan Carbone® ou un reporting réglementaire lorsque ceux-ci sont explicitement exigés.",

    finalResponsibilityTitle: "Responsabilité des données",
    finalResponsibilityText:
      "Les résultats dépendent de l’exactitude, de la complétude et de la cohérence des données déclarées par l’entité.",

    finalValidityTitle: "Validité documentaire",
    finalValidityText:
      "La période de validité reflète la pertinence temporelle des données déclarées, des facteurs d’émission utilisés et de la méthodologie appliquée.",

    finalLanguageNoticeTitle: "Langue du document",
  },

  en: {
    eyebrow: "Indicative CO₂e document · standardized · verifiable",
    title: "Indicative CO₂e Attestation",
    subtitle:
      "Aggregated estimate based on a standardized spend-based methodology",
    positioningLine:
      "Methodological document for first-level documentary carbon estimation",

    resultFootnote:
      "Indicative estimate based on data declared by the entity.",

    documentVerificationTitle: "Verifiable document",
    statusReference: "Reference",
    statusIssuedDate: "Issued date",
    statusValidUntil: "Valid until",
    statusIssuer: "Issuer",

    methodologicalBasisTitle: "Methodological basis",
    methodologicalBasisText:
      "This attestation provides a standardized and verifiable indicative CO₂e estimate based on a spend-based methodology using declared aggregated spend data and monetary emission factors.",

    intendedUseTitle: "Intended use",
    intendedUseText:
      "This attestation may be used as an indicative document in a supplier file, client request, tender file, banking request, insurance request or internal first-level carbon estimate, where the need relates to an aggregated and documented CO₂e indicator.",

    referencesTitle: "Consistency references",
    referencesIntro:
      "The Certif-Scope methodology is designed to be consistent with the following carbon and environmental transparency reference frameworks:",
    referencesItems: [
      "GHG Protocol — Scope 3, spend-based method",
      "ISO 14064-1 — GHG quantification and reporting",
      "ISO 14083 — transport operations emissions",
      "CSRD / ESRS — European environmental transparency context",
      "EU Taxonomy — sustainability and non-financial reporting context",
    ],
    referencesContext:
      "These references are cited as a methodological and contextual consistency framework. They do not turn this attestation into regulatory reporting, certified auditing or third-party independent verification.",

    privacyTitle: "Privacy",
    privacyBullets: [
      "No storage of detailed financial data.",
      "Aggregated CO₂e result only.",
      "Independently verifiable attestation.",
    ],

    authenticityTitle: "Authenticity",
    authenticityText:
      "The attestation includes a unique reference, a QR code and documentary control elements.",

    validityTitle: "Validity",
    validityText:
      "Document validity is time-limited and stated on the attestation.",

    methodFormula:
      "Declared aggregated spend × monetary emission factors = indicative CO₂e estimate",
    factorVersionLabel: "Factor version",

    simpleVerificationTitle: "Simple verification",
    simpleVerificationText:
      "Scan the QR code or use the attestation reference on the official verification page.",
    technicalVerificationTitle: "Independent technical verification",
    technicalVerificationIntro:
      "The elements below enable advanced documentary verification. No action is required for standard reading of the document.",
    technicalElementsTitle: "Technical verification elements",

    verifiableObjectTitle: "Verifiable object",
    verifiableObjectText:
      "The signed PDF and its QR code form the documentary control elements.",

    scopeAndLimitsTitle: "Scope and limitations",
    scopeAndLimitsText:
      "This attestation is indicative. It relies on data declared by the entity and on a spend-based methodology. It does not replace a carbon audit, a complete GHG inventory, third-party independent verification, Bilan Carbone® work or regulatory reporting where these are explicitly required.",

    finalResponsibilityTitle: "Data responsibility",
    finalResponsibilityText:
      "The results depend on the accuracy, completeness and consistency of the data declared by the entity.",

    finalValidityTitle: "Document validity",
    finalValidityText:
      "The validity period reflects the temporal relevance of the declared data, the emission factors used and the methodology applied.",

    finalLanguageNoticeTitle: "Document language",
  },
};

function escapeHtml(input: string) {
  return input
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
    .replace(/"/g, "&quot;")
    .replace(/'/g, "&#39;");
}

function toBase64Url(input: string) {
  return Buffer.from(input, "utf8")
    .toString("base64")
    .replace(/\+/g, "-")
    .replace(/\//g, "_")
    .replace(/=+$/g, "");
}

function addMonthsToISODate(date: string, months: number) {
  const parsed = new Date(`${date}T00:00:00.000Z`);

  if (Number.isNaN(parsed.getTime())) {
    return "";
  }

  parsed.setUTCMonth(parsed.getUTCMonth() + months);

  return parsed.toISOString().slice(0, 10);
}

function normalizeLocale(locale: AttestationLocale): "fr" | "en" {
  return locale === "fr" ? "fr" : "en";
}

function formatCO2Value(value: number, locale: AttestationLocale) {
  if (!Number.isFinite(value)) {
    return "0";
  }

  return new Intl.NumberFormat(locale === "fr" ? "fr-FR" : "en-US", {
    maximumFractionDigits: 2,
    minimumFractionDigits: value < 10 ? 2 : 0,
  }).format(value);
}

function clampText(value: string, maxLength: number) {
  const clean = value.trim();

  if (clean.length <= maxLength) {
    return clean;
  }

  return `${clean.slice(0, maxLength - 1)}…`;
}

function translateSector(value: string, locale: "fr" | "en") {
  const clean = value.trim();

  const frMap: Record<string, string> = {
    "manufacturing & industry": "Industrie manufacturière",
    "construction & real estate": "Construction et immobilier",
    "transport & logistics": "Transport et logistique",
    "business services": "Services aux entreprises",
    "retail & commerce": "Commerce et distribution",
    "food & hospitality": "Restauration et hôtellerie",
    "healthcare": "Santé",
    "technology": "Technologie",
    "energy & utilities": "Énergie et services publics",
    "agriculture": "Agriculture",
  };

  const enMap: Record<string, string> = {
    "manufacturing & industry": "Manufacturing & industry",
    "construction & real estate": "Construction & real estate",
    "transport & logistics": "Transport & logistics",
    "business services": "Business services",
    "retail & commerce": "Retail & commerce",
    "food & hospitality": "Food & hospitality",
    "healthcare": "Healthcare",
    "technology": "Technology",
    "energy & utilities": "Energy & utilities",
    "agriculture": "Agriculture",
  };

  const key = clean.toLowerCase();

  if (locale === "fr") {
    return frMap[key] || clean;
  }

  return enMap[key] || clean;
}

export async function GET(req: Request) {
  try {
    if (!process.env.PDFSHIFT_API_KEY) {
      return new Response("PDFSHIFT_API_KEY missing", { status: 500 });
    }

    const { searchParams } = new URL(req.url);
    const sessionId = searchParams.get("session_id");

    if (!sessionId) {
      return new Response("Missing session_id", { status: 400 });
    }

    let metadataRaw: Record<string, any> = {};

    if (sessionId.startsWith("key_")) {
      metadataRaw = Object.fromEntries(searchParams.entries());
    } else {
      const session = await stripe.checkout.sessions.retrieve(sessionId);

      if (session.payment_status !== "paid") {
        return new Response("Payment not completed", { status: 403 });
      }

      metadataRaw = session.metadata || {};
    }

    const locale =
      (metadataRaw.attestationLocale as AttestationLocale) ||
      DEFAULT_ATTESTATION_LOCALE;

    const normalizedLocale = normalizeLocale(locale);
    const i18n = ATTESTATION_I18N[locale] || ATTESTATION_I18N.en;
    const premium = PREMIUM_COPY[normalizedLocale];

    const required = ["companyName", "totalCO2e", "year"];
    const missing = required.filter((k) => {
      const v = metadataRaw[k];
      return v === undefined || v === null || String(v).trim() === "";
    });

    if (missing.length) {
      return new Response(`Missing metadata: ${missing.join(", ")}`, {
        status: 400,
      });
    }

    const totalCO2eNum = Number(String(metadataRaw.totalCO2e).replace(",", "."));

    if (Number.isNaN(totalCO2eNum)) {
      return new Response("Invalid metadata: totalCO2e must be a number", {
        status: 400,
      });
    }

    const issuedDate = new Date().toISOString();

    const canonicalPayload = {
      issuer: "Certif-Scope" as const,
      standard: "CS-SB-v1" as const,
      attestationId: "",
      companyName: String(metadataRaw.companyName),
      country: String(metadataRaw.country || "—"),
      year: String(metadataRaw.year),
      totalCO2e: String(totalCO2eNum),
      issuedDate,
    };

    const tempSignature = signCanonicalPayload({
      ...canonicalPayload,
      attestationId: "TEMP",
    });

    const attestationId = makeAttestationId(
      canonicalPayload.year,
      tempSignature.hashHex
    );

    const signatureResult = signCanonicalPayload({
      ...canonicalPayload,
      attestationId,
    });

    const issuedDateForDisplay = issuedDate.slice(0, 10);
    const validityMonths = String(metadataRaw.validityMonths || "12");
    const validUntil =
      String(metadataRaw.validUntil || "").trim() ||
      addMonthsToISODate(issuedDateForDisplay, Number(validityMonths) || 12);

    const methodology =
      metadataRaw.methodology ||
      "Certif-Scope deterministic spend-based methodology v1.0";

    const factorVersion =
      metadataRaw.factorVersion ||
      metadataRaw.emissionFactorVersion ||
      "Certif-Scope factors v1";

    const sector = translateSector(
      String(metadataRaw.companySector || "—"),
      normalizedLocale
    );

    const metadata = {
      attestationId,
      issuerName: escapeHtml(String(metadataRaw.issuerName || "Certif-Scope")),
      issuerSite: escapeHtml(
        String(metadataRaw.issuerSite || "https://www.certif-scope.com")
      ),
      companyName: escapeHtml(
        clampText(String(metadataRaw.companyName || "—"), 150)
      ),
      companySector: escapeHtml(clampText(sector, 100)),
      entityIdentifier: escapeHtml(
        clampText(String(metadataRaw.entityIdentifier || "—"), 90)
      ),
      country: escapeHtml(clampText(String(metadataRaw.country || "—"), 55)),
      year: escapeHtml(String(metadataRaw.year)),
      totalCO2e: escapeHtml(formatCO2Value(totalCO2eNum, locale)),
      methodology: escapeHtml(String(methodology)),
      factorVersion: escapeHtml(String(factorVersion)),
      issuedDate: escapeHtml(issuedDateForDisplay),
      validUntil: escapeHtml(validUntil),
      validityMonths: escapeHtml(validityMonths),
      standardRef: escapeHtml(
        String(metadataRaw.standardRef || "Certif-Scope CS-SB-v1")
      ),
      signature: signatureResult.signatureBase64,
      hash: signatureResult.hashHex,
      algorithm: signatureResult.algorithm,
    };

    const verificationPayload = {
      certificateId: metadata.attestationId,
      issuer: "Certif-Scope",
      issuedAt: metadata.issuedDate,
      validUntil: metadata.validUntil,
      methodVersion: "CS-SB-v1",
      factorVersion: String(factorVersion),
      algorithm: metadata.algorithm,
      hash: metadata.hash,
      signature: metadata.signature,
    };

    const verificationToken = toBase64Url(JSON.stringify(verificationPayload));

    const verifyUrl =
      normalizedLocale === "fr"
        ? `https://www.certif-scope.com/fr/verify/?v=${verificationToken}`
        : `https://www.certif-scope.com/verify/?v=${verificationToken}`;

    const verificationDisplayUrl =
      normalizedLocale === "fr"
        ? "https://www.certif-scope.com/fr/verify"
        : "https://www.certif-scope.com/verify";

    const qrDataUrl = await QRCode.toDataURL(verifyUrl, {
      width: 150,
      margin: 1,
      errorCorrectionLevel: "M",
    });

    const hasRealLogo =
      CERTIF_SCOPE_LOGO_BASE64 &&
      CERTIF_SCOPE_LOGO_BASE64 !==
        "COLLE_ICI_TON_BASE64_ACTUEL_COMPLET_SANS_PREFIXE_DATA_IMAGE";

    const html = `
<!doctype html>
<html lang="${normalizedLocale}">
<head>
<meta charset="utf-8"/>
<title>${metadata.issuerName} — ${premium.title}</title>
<style>
  @page {
    size: A4;
    margin: 15mm 17mm 14mm 17mm;
  }

  * {
    box-sizing: border-box;
  }

  body {
    font-family: Arial, Helvetica, sans-serif;
    font-size: 10px;
    line-height: 1.38;
    margin: 0;
    color: #111827;
    background: #ffffff;
    -webkit-font-smoothing: antialiased;
  }

  :root {
    --blue: #0B3A63;
    --blue-dark: #082F52;
    --turquoise: #1FB6C1;
    --muted: #64748B;
    --soft: #F8FAFC;
    --soft-blue: #F3F8FB;
    --line: #D9E2EC;
    --line-dark: #B8C7D7;
    --text: #111827;
  }

  .container {
    width: 100%;
    padding: 0;
  }

  .avoid-break {
    page-break-inside: avoid;
    break-inside: avoid;
  }

  .header {
    display: flex;
    justify-content: space-between;
    align-items: flex-start;
    gap: 22px;
    padding-bottom: 10px;
    border-bottom: 2px solid var(--blue);
    margin-bottom: 12px;
  }

  .brand {
    max-width: 72%;
  }

  .issuer-logo {
    height: 58px;
    max-width: 260px;
    width: auto;
    display: block;
    object-fit: contain;
    margin-bottom: 5px;
  }

  .fallback-logo {
    font-size: 25px;
    line-height: 1;
    font-weight: 900;
    color: var(--blue);
    letter-spacing: -0.02em;
    margin-bottom: 7px;
  }

  .issuer-site {
    font-size: 8.4px;
    color: var(--muted);
    margin-bottom: 2px;
  }

  .issuer-meta {
    font-size: 8.4px;
    color: var(--muted);
  }

  .qr-box {
    width: 96px;
    border: 1px solid var(--line);
    background: #ffffff;
    border-radius: 4px;
    padding: 6px 6px 5px;
    text-align: center;
  }

  .qr-box img {
    width: 68px;
    height: 68px;
    display: block;
    margin: 0 auto 4px;
  }

  .qr-label {
    font-size: 7.2px;
    color: var(--blue);
    font-weight: 700;
    line-height: 1.15;
    letter-spacing: 0.02em;
  }

  .hero {
    text-align: center;
    margin: 12px 0 14px;
  }

  .eyebrow {
    display: inline-block;
    font-size: 8px;
    text-transform: uppercase;
    letter-spacing: 0.13em;
    color: var(--turquoise);
    font-weight: 800;
    margin-bottom: 4px;
  }

  .hero h1 {
    font-size: 25px;
    line-height: 1.05;
    margin: 0;
    color: var(--blue);
    text-transform: uppercase;
    letter-spacing: 0.035em;
    font-weight: 800;
  }

  .hero-subtitle {
    margin-top: 5px;
    font-size: 10.3px;
    color: #1f2937;
    font-weight: 700;
  }

  .hero-note {
    margin-top: 4px;
    font-size: 8.7px;
    color: var(--muted);
  }

  .result-card {
    width: 100%;
    max-width: 535px;
    margin: 0 auto 12px;
    border: 2px solid var(--blue);
    border-radius: 6px;
    padding: 14px 18px;
    background: #ffffff;
    text-align: center;
  }

  .result-label {
    font-size: 8.7px;
    color: #334155;
    text-transform: uppercase;
    letter-spacing: 0.08em;
    font-weight: 800;
    margin-bottom: 5px;
  }

  .result-value {
    font-size: 35px;
    line-height: 1;
    font-weight: 900;
    color: var(--blue);
    letter-spacing: -0.01em;
  }

  .result-unit {
    font-size: 12px;
    color: var(--turquoise);
    font-weight: 900;
    margin-left: 7px;
  }

  .result-footnote {
    margin-top: 7px;
    font-size: 8.8px;
    color: var(--muted);
  }

  .verification-band {
    display: grid;
    grid-template-columns: 1.25fr 0.85fr 0.85fr 0.85fr;
    gap: 0;
    border: 1px solid var(--line-dark);
    border-radius: 6px;
    overflow: hidden;
    margin-bottom: 11px;
    background: #ffffff;
  }

  .verification-band-title {
    grid-column: 1 / -1;
    padding: 7px 9px 6px;
    color: var(--blue);
    background: var(--soft-blue);
    border-bottom: 1px solid var(--line);
    font-size: 8.6px;
    font-weight: 900;
    text-transform: uppercase;
    letter-spacing: 0.08em;
  }

  .verification-cell {
    padding: 7px 9px;
    border-right: 1px solid var(--line);
  }

  .verification-cell:last-child {
    border-right: 0;
  }

  .cell-label {
    font-size: 7.3px;
    color: var(--muted);
    text-transform: uppercase;
    letter-spacing: 0.06em;
    margin-bottom: 2px;
  }

  .cell-value {
    font-size: 8.8px;
    color: var(--text);
    font-weight: 800;
    word-break: break-word;
  }

  .basis-card {
    border: 1px solid #CFE9ED;
    border-left: 4px solid var(--turquoise);
    background: #FBFEFF;
    border-radius: 6px;
    padding: 9px 10px;
    margin-bottom: 9px;
  }

  .usage-card {
    border: 1px solid var(--line);
    background: #ffffff;
    border-radius: 6px;
    padding: 9px 10px;
    margin-bottom: 10px;
  }

  .basis-title,
  .usage-title,
  .mini-title,
  .section-title {
    color: var(--blue);
    font-size: 9px;
    font-weight: 900;
    text-transform: uppercase;
    letter-spacing: 0.05em;
    margin-bottom: 4px;
  }

  .basis-text,
  .usage-text {
    font-size: 9.25px;
    color: #263445;
  }

  .entity-section {
    margin-bottom: 10px;
  }

  .entity-grid {
    display: grid;
    grid-template-columns: 1fr 1fr 1fr;
    gap: 7px 11px;
    border: 1px solid var(--line);
    border-radius: 6px;
    padding: 10px 11px;
    background: #ffffff;
  }

  .field-label {
    font-size: 7.6px;
    color: var(--muted);
    text-transform: uppercase;
    letter-spacing: 0.06em;
    margin-bottom: 2px;
  }

  .field-value {
    font-size: 9.2px;
    color: var(--text);
    font-weight: 700;
  }

  .page-one-bottom-grid {
    display: grid;
    grid-template-columns: 1fr 1fr;
    gap: 9px;
    margin-bottom: 10px;
  }

  .compact-card {
    border: 1px solid var(--line);
    border-radius: 6px;
    padding: 9px 10px;
    background: #ffffff;
  }

  .compact-text {
    font-size: 9px;
    color: #334155;
  }

  .mini-proof-grid {
    display: grid;
    grid-template-columns: repeat(3, 1fr);
    gap: 8px;
    margin-bottom: 10px;
  }

  .mini-card {
    border: 1px solid var(--line);
    border-radius: 6px;
    padding: 8px 9px;
    background: var(--soft);
  }

  .mini-text {
    font-size: 8.45px;
    color: #475569;
  }

  .footer-static {
    width: 100%;
    font-size: 8.2px;
    color: var(--muted);
    display: flex;
    justify-content: space-between;
    border-top: 1px solid #D9E2EC;
    padding-top: 7px;
    margin-top: 10px;
    clear: both;
  }

  .page-two-title {
    display: flex;
    justify-content: space-between;
    align-items: flex-end;
    gap: 20px;
    border-bottom: 2px solid var(--blue);
    padding-bottom: 8px;
    margin-bottom: 12px;
  }

  .page-two-title h2 {
    margin: 0;
    color: var(--blue);
    font-size: 18px;
    text-transform: uppercase;
    letter-spacing: 0.04em;
  }

  .page-two-title .ref {
    font-size: 8.5px;
    color: var(--muted);
    text-align: right;
  }

  .method-card {
    border: 1px solid var(--line);
    border-radius: 6px;
    background: #ffffff;
    padding: 10px 11px;
    margin-bottom: 10px;
  }

  .formula {
    margin: 7px 0;
    padding: 8px 10px;
    border-radius: 5px;
    background: var(--soft-blue);
    border: 1px solid #CFE9ED;
    color: var(--blue);
    font-weight: 900;
    font-size: 9.4px;
    text-align: center;
  }

  .method-meta {
    display: grid;
    grid-template-columns: 1fr 1fr;
    gap: 8px;
    margin-top: 7px;
  }

  .method-meta-item {
    border: 1px solid #E8EEF5;
    border-radius: 5px;
    background: var(--soft);
    padding: 6px 7px;
    font-size: 8.6px;
    color: #334155;
  }

  .references-card {
    border: 1px solid var(--line);
    border-radius: 6px;
    background: #ffffff;
    padding: 10px 11px;
    margin-bottom: 10px;
  }

  .references-list {
    display: grid;
    grid-template-columns: 1fr 1fr;
    gap: 3px 12px;
    margin: 7px 0 0 0;
    padding: 0;
    list-style: none;
  }

  .references-list li {
    position: relative;
    padding-left: 9px;
    font-size: 8.55px;
    color: #334155;
  }

  .references-list li::before {
    content: "•";
    position: absolute;
    left: 0;
    color: var(--turquoise);
    font-weight: 900;
  }

  .verification-grid {
    display: grid;
    grid-template-columns: 1fr 1fr;
    gap: 9px;
    margin-bottom: 10px;
  }

  .verification-simple {
    border: 1px solid #CFE9ED;
    border-left: 4px solid var(--turquoise);
    border-radius: 6px;
    background: #FBFEFF;
    padding: 9px 10px;
  }

  .verification-object {
    border: 1px solid var(--line);
    border-radius: 6px;
    background: var(--soft);
    padding: 9px 10px;
  }

  .technical-card {
    border: 1px solid var(--line);
    border-radius: 6px;
    background: #ffffff;
    padding: 9px 10px;
    margin-bottom: 10px;
  }

  .technical-head {
    display: flex;
    justify-content: space-between;
    gap: 14px;
    align-items: flex-start;
    margin-bottom: 6px;
  }

  .technical-intro {
    font-size: 8.2px;
    color: var(--muted);
    max-width: 72%;
  }

  .tech-grid {
    display: grid;
    grid-template-columns: 0.7fr 1.3fr;
    gap: 3px 8px;
    border-top: 1px solid #E8EEF5;
    padding-top: 6px;
  }

  .tech-label {
    font-size: 7.25px;
    color: var(--muted);
    text-transform: uppercase;
    letter-spacing: 0.04em;
    font-weight: 800;
  }

  .tech-value {
    font-size: 7.35px;
    color: #64748B;
    line-height: 1.18;
    word-break: break-all;
  }

  .limits-card {
    border: 1px solid #D9E2EC;
    border-left: 4px solid var(--blue);
    border-radius: 6px;
    padding: 10px 11px;
    background: #ffffff;
    margin-bottom: 10px;
  }

  .limits-text {
    font-size: 9.1px;
    color: #334155;
    line-height: 1.38;
  }

  .final-grid {
    display: grid;
    grid-template-columns: 1fr 1fr;
    gap: 8px;
    margin-top: 8px;
  }

  .final-item {
    background: #F8FAFC;
    border: 1px solid #E8EEF5;
    border-radius: 5px;
    padding: 7px 8px;
    font-size: 8.55px;
    color: #334155;
  }

  .final-item strong {
    color: var(--blue);
  }

  .muted {
    color: var(--muted);
  }

  .small {
    font-size: 8.2px;
    color: var(--muted);
    line-height: 1.25;
  }

  .section-title {
    margin: 0 0 6px;
  }
</style>
</head>
<body>
<div class="container">

  <header class="header avoid-break">
    <div class="brand">
      ${
        hasRealLogo
          ? `<img src="data:image/png;base64,${CERTIF_SCOPE_LOGO_BASE64}" class="issuer-logo" alt="Certif-Scope logo" />`
          : `<div class="fallback-logo">Certif-Scope</div>`
      }

      <div class="issuer-site">
        <a href="${metadata.issuerSite}" target="_blank" style="color:inherit; text-decoration:none;">
          ${metadata.issuerSite}
        </a>
      </div>

      <div class="issuer-meta">${i18n.headerTagline}</div>
    </div>

    <div class="qr-box">
      <img src="${qrDataUrl}" alt="QR verification" />
      <div class="qr-label">${i18n.scanToVerifyLabel}</div>
    </div>
  </header>

  <section class="hero avoid-break">
    <div class="eyebrow">${premium.eyebrow}</div>
    <h1>${premium.title}</h1>
    <div class="hero-subtitle">${premium.subtitle}</div>
    <div class="hero-note">${premium.positioningLine} · ${metadata.standardRef}</div>
  </section>

  <section class="result-card avoid-break">
    <div class="result-label">${i18n.resultLabel}</div>
    <div class="result-value">${metadata.totalCO2e}<span class="result-unit">tCO₂e</span></div>
    <div class="result-footnote">${premium.resultFootnote}</div>
  </section>

  <section class="verification-band avoid-break">
    <div class="verification-band-title">${premium.documentVerificationTitle}</div>

    <div class="verification-cell">
      <div class="cell-label">${premium.statusReference}</div>
      <div class="cell-value">${metadata.attestationId}</div>
    </div>

    <div class="verification-cell">
      <div class="cell-label">${premium.statusIssuedDate}</div>
      <div class="cell-value">${metadata.issuedDate}</div>
    </div>

    <div class="verification-cell">
      <div class="cell-label">${premium.statusValidUntil}</div>
      <div class="cell-value">${metadata.validUntil}</div>
    </div>

    <div class="verification-cell">
      <div class="cell-label">${premium.statusIssuer}</div>
      <div class="cell-value">${metadata.issuerName}</div>
    </div>
  </section>

  <section class="basis-card avoid-break">
    <div class="basis-title">${premium.methodologicalBasisTitle}</div>
    <div class="basis-text">${premium.methodologicalBasisText}</div>
  </section>

  <section class="usage-card avoid-break">
    <div class="usage-title">${premium.intendedUseTitle}</div>
    <div class="usage-text">${premium.intendedUseText}</div>
  </section>

  <section class="entity-section avoid-break" aria-labelledby="entity">
    <h2 class="section-title" id="entity">${i18n.entitySectionTitle}</h2>
    <div class="entity-grid">
      <div>
        <div class="field-label">${i18n.entityNameLabel}</div>
        <div class="field-value">${metadata.companyName}</div>
      </div>
      <div>
        <div class="field-label">${i18n.countryLabel}</div>
        <div class="field-value">${metadata.country}</div>
      </div>
      <div>
        <div class="field-label">${i18n.reportingYearLabel}</div>
        <div class="field-value">${metadata.year}</div>
      </div>
      <div>
        <div class="field-label">${i18n.activitySectorLabel}</div>
        <div class="field-value">${metadata.companySector}</div>
      </div>
      <div>
        <div class="field-label">${i18n.entityIdentifierLabel}</div>
        <div class="field-value">${metadata.entityIdentifier}</div>
      </div>
      <div>
        <div class="field-label">${i18n.uniqueIdentifierLabel}</div>
        <div class="field-value">${metadata.attestationId}</div>
      </div>
    </div>
  </section>

  <section class="page-one-bottom-grid avoid-break">
    <div class="compact-card">
      <div class="section-title">${i18n.documentNatureSectionTitle}</div>
      <div class="compact-text"><strong>${i18n.documentNatureText}</strong></div>
    </div>

    <div class="compact-card">
      <div class="section-title">${i18n.scopeSectionTitle}</div>
      <div class="compact-text"><strong>${i18n.scopeDescriptionLabel} :</strong> ${i18n.scopeText}</div>
    </div>
  </section>

  <section class="mini-proof-grid avoid-break">
    <div class="mini-card">
      <div class="mini-title">${premium.privacyTitle}</div>
      <div class="mini-text">
        ${premium.privacyBullets.join("<br/>")}
      </div>
    </div>

    <div class="mini-card">
      <div class="mini-title">${premium.authenticityTitle}</div>
      <div class="mini-text">${premium.authenticityText}</div>
    </div>

    <div class="mini-card">
      <div class="mini-title">${premium.validityTitle}</div>
      <div class="mini-text">${premium.validityText}</div>
    </div>
  </section>

  <div class="footer-static">
    <div>${i18n.footerText}</div>
    <div>${i18n.pageLabel} 1 / 2</div>
  </div>

  <div style="page-break-after: always;"></div>

  <section class="page-two-title avoid-break">
    <h2>${i18n.methodologySectionTitle}</h2>
    <div class="ref">
      ${premium.statusReference} : <strong>${metadata.attestationId}</strong><br/>
      ${premium.statusIssuer} : ${metadata.issuerName}
    </div>
  </section>

  <section class="method-card avoid-break">
    <div class="section-title">${i18n.methodologyLabel}</div>
    <div><strong>${metadata.methodology}</strong></div>
    <div class="formula">${premium.methodFormula}</div>

    <div class="method-meta">
      <div class="method-meta-item">
        <strong>${premium.factorVersionLabel} :</strong><br/>
        ${metadata.factorVersion}
      </div>

      <div class="method-meta-item">
        <strong>${i18n.transferabilityLabel} :</strong><br/>
        ${i18n.transferabilityText}
      </div>
    </div>
  </section>

  <section class="references-card avoid-break">
    <div class="section-title">${premium.referencesTitle}</div>
    <div class="small">${premium.referencesIntro}</div>

    <ul class="references-list">
      ${premium.referencesItems.map((ref) => `<li>${ref}</li>`).join("")}
    </ul>

    <div class="small" style="margin-top:7px;">
      ${premium.referencesContext}
    </div>
  </section>

  <section class="verification-grid avoid-break">
    <div class="verification-simple">
      <div class="section-title">${premium.simpleVerificationTitle}</div>
      <div>${premium.simpleVerificationText}</div>
      <div style="margin-top:6px;">
        <strong>${i18n.verificationPageLabel} :</strong><br/>
        <a href="${verificationDisplayUrl}" style="color:var(--blue); text-decoration:none;">
          ${verificationDisplayUrl}
        </a>
      </div>
      <div class="small" style="margin-top:5px;">
        ${premium.statusReference} : <strong>${metadata.attestationId}</strong>
      </div>
    </div>

    <div class="verification-object">
      <div class="section-title">${premium.verifiableObjectTitle}</div>
      <div>${premium.verifiableObjectText}</div>
      <div class="small" style="margin-top:6px;">
        ${i18n.pdfObjectText}
      </div>
    </div>
  </section>

  <section class="technical-card avoid-break">
    <div class="technical-head">
      <div>
        <div class="section-title">${premium.technicalElementsTitle}</div>
        <div class="technical-intro">${premium.technicalVerificationIntro}</div>
      </div>
    </div>

    <div class="tech-grid">
      <div class="tech-label">${i18n.algorithmLabel}</div>
      <div class="tech-value">${metadata.algorithm}</div>

      <div class="tech-label">${i18n.hashLabel}</div>
      <div class="tech-value">${metadata.hash}</div>

      <div class="tech-label">${i18n.signatureLabel}</div>
      <div class="tech-value">${metadata.signature}</div>

      <div class="tech-label">${i18n.publicKeyLabel}</div>
      <div class="tech-value">MCowBQYDK2VwAyEAbKp2pg4wmzE5Kqo9tEwv7JJjxQyT2cBmwiLLHp4cSac=</div>
    </div>
  </section>

  <section class="limits-card avoid-break" aria-labelledby="limits">
    <h2 class="section-title" id="limits">${premium.scopeAndLimitsTitle}</h2>
    <div class="limits-text">${premium.scopeAndLimitsText}</div>

    <div class="final-grid">
      <div class="final-item">
        <strong>${premium.finalResponsibilityTitle}</strong><br/>
        ${premium.finalResponsibilityText}
      </div>

      <div class="final-item">
        <strong>${premium.finalValidityTitle}</strong><br/>
        ${premium.finalValidityText}
      </div>
    </div>

    <div class="small" style="margin-top:7px; font-weight:700;">
      ${premium.finalLanguageNoticeTitle} : ${i18n.englishPrevailsNotice}
    </div>

    <div class="small" style="margin-top:4px;">
      ${i18n.methodologyNote}
    </div>
  </section>

  <div class="footer-static">
    <div>${i18n.footerText}</div>
    <div>${i18n.pageLabel} 2 / 2</div>
  </div>
</div>
</body>
</html>
`;

    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 15000);

    const pdfResponse = await fetch("https://api.pdfshift.io/v3/convert/pdf", {
      method: "POST",
      headers: {
        "X-API-Key": process.env.PDFSHIFT_API_KEY!,
        "Content-Type": "application/json",
      },
      body: JSON.stringify({
        source: html,
        format: "A4",
        use_print: true,
      }),
      signal: controller.signal,
    });

    clearTimeout(timeoutId);

    if (!pdfResponse.ok) {
      const error = await pdfResponse.text();
      return new Response(error, { status: pdfResponse.status });
    }

    const pdfBuffer = Buffer.from(await pdfResponse.arrayBuffer());

    const safeIssuerName = metadata.issuerName
      .toLowerCase()
      .replace(/[^a-z0-9\-]/g, "");

    return new Response(pdfBuffer, {
      headers: {
        "Content-Type": "application/pdf",
        "Content-Disposition": `attachment; filename="${safeIssuerName}-${metadata.attestationId}.pdf"`,
        "Cache-Control": "no-store",
        "X-Content-Type-Options": "nosniff",
      },
    });
  } catch (err) {
    if (process.env.NODE_ENV !== "production") {
      console.error(err);
    }

    return new Response("Internal Server Error", { status: 500 });
  }
      }
