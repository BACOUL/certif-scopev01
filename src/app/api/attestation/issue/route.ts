export const runtime = "nodejs";

import Stripe from "stripe";
import QRCode from "qrcode";
import { signCanonicalPayload, makeAttestationId } from "@/lib/sign";
import {
  ATTESTATION_I18N,
  AttestationLocale,
  DEFAULT_ATTESTATION_LOCALE,
} from "@/lib/attestation-i18n/index";

const stripe = new Stripe(process.env.STRIPE_SECRET_KEY!);

/**
 * Logo Certif-Scope en base64.
 *
 * IMPORTANT :
 * - Tu peux coller ton base64 complet ici.
 * - Tu peux le coller sur plusieurs lignes.
 * - Ne mets PAS "data:image/png;base64,".
 * - Le .replace(/\s+/g, "") supprime automatiquement les retours à la ligne.
 */
const RAW_CERTIF_SCOPE_LOGO_BASE64 = `

`.replace(/\s+/g, "");

const CERTIF_SCOPE_LOGO_BASE64 = RAW_CERTIF_SCOPE_LOGO_BASE64;

function escapeHtml(input: string) {
  return input
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
    .replace(/"/g, "&quot;")
    .replace(/'/g, "&#39;");
}

function toBase64Url(input: string) {
  return Buffer.from(input, "utf8")
    .toString("base64")
    .replace(/\+/g, "-")
    .replace(/\//g, "_")
    .replace(/=+$/g, "");
}

function addMonthsToISODate(date: string, months: number) {
  const parsed = new Date(`${date}T00:00:00.000Z`);

  if (Number.isNaN(parsed.getTime())) {
    return "";
  }

  parsed.setUTCMonth(parsed.getUTCMonth() + months);

  return parsed.toISOString().slice(0, 10);
}

export async function GET(req: Request) {
  try {
    if (!process.env.PDFSHIFT_API_KEY) {
      return new Response("PDFSHIFT_API_KEY missing", { status: 500 });
    }

    const { searchParams } = new URL(req.url);
    const sessionId = searchParams.get("session_id");

    if (!sessionId) {
      return new Response("Missing session_id", { status: 400 });
    }

    let metadataRaw: Record<string, any> = {};

    if (sessionId.startsWith("key_")) {
      metadataRaw = Object.fromEntries(searchParams.entries());
    } else {
      const session = await stripe.checkout.sessions.retrieve(sessionId);

      if (session.payment_status !== "paid") {
        return new Response("Payment not completed", { status: 403 });
      }

      metadataRaw = session.metadata || {};
    }

    const locale =
      (metadataRaw.attestationLocale as AttestationLocale) ||
      DEFAULT_ATTESTATION_LOCALE;

    const i18n = ATTESTATION_I18N[locale] || ATTESTATION_I18N.en;

    const required = ["companyName", "totalCO2e", "year"];
    const missing = required.filter((k) => {
      const v = metadataRaw[k];
      return v === undefined || v === null || String(v).trim() === "";
    });

    if (missing.length) {
      return new Response(`Missing metadata: ${missing.join(", ")}`, {
        status: 400,
      });
    }

    const totalCO2eNum = Number(String(metadataRaw.totalCO2e).replace(",", "."));

    if (Number.isNaN(totalCO2eNum)) {
      return new Response("Invalid metadata: totalCO2e must be a number", {
        status: 400,
      });
    }

    const issuedDate = new Date().toISOString();

    const canonicalPayload = {
      issuer: "Certif-Scope" as const,
      standard: "CS-SB-v1" as const,
      attestationId: "",
      companyName: String(metadataRaw.companyName),
      country: String(metadataRaw.country || "—"),
      year: String(metadataRaw.year),
      totalCO2e: String(totalCO2eNum),
      issuedDate,
    };

    const tempSignature = signCanonicalPayload({
      ...canonicalPayload,
      attestationId: "TEMP",
    });

    const attestationId = makeAttestationId(
      canonicalPayload.year,
      tempSignature.hashHex
    );

    const signatureResult = signCanonicalPayload({
      ...canonicalPayload,
      attestationId,
    });

    const issuedDateForDisplay = issuedDate.slice(0, 10);
    const validityMonths = String(metadataRaw.validityMonths || "12");
    const validUntil =
      String(metadataRaw.validUntil || "").trim() ||
      addMonthsToISODate(issuedDateForDisplay, Number(validityMonths) || 12);

    const metadata = {
      attestationId,
      issuerName: escapeHtml(String(metadataRaw.issuerName || "Certif-Scope")),
      issuerSite: escapeHtml(
        String(metadataRaw.issuerSite || "https://www.certif-scope.com")
      ),
      companyName: escapeHtml(String(metadataRaw.companyName)),
      companySector: escapeHtml(String(metadataRaw.companySector || "—")),
      entityIdentifier: escapeHtml(String(metadataRaw.entityIdentifier || "—")),
      country: escapeHtml(String(metadataRaw.country || "—")),
      year: escapeHtml(String(metadataRaw.year)),
      totalCO2e: escapeHtml(String(totalCO2eNum)),
      methodology: escapeHtml(
        String(
          metadataRaw.methodology ||
            "Certif-Scope deterministic spend-based methodology v1.0"
        )
      ),
      issuedDate: escapeHtml(issuedDateForDisplay),
      validUntil: escapeHtml(validUntil),
      validityMonths: escapeHtml(validityMonths),
      standardRef: escapeHtml(
        String(metadataRaw.standardRef || "Certif-Scope CS-SB-v1")
      ),
      signature: signatureResult.signatureBase64,
      hash: signatureResult.hashHex,
      algorithm: signatureResult.algorithm,
      factorVersion: escapeHtml(
        String(
          metadataRaw.factorVersion ||
            metadataRaw.emissionFactorVersion ||
            "Certif-Scope factors v1"
        )
      ),
    };

    const verificationPayload = {
      certificateId: metadata.attestationId,
      issuer: "Certif-Scope",
      issuedAt: metadata.issuedDate,
      validUntil: metadata.validUntil,
      methodVersion: "CS-SB-v1",
      factorVersion: metadata.factorVersion,
      algorithm: metadata.algorithm,
      hash: metadata.hash,
      signature: metadata.signature,
    };

    const verificationToken = toBase64Url(JSON.stringify(verificationPayload));

    const verifyUrl =
      locale === "fr"
        ? `https://www.certif-scope.com/fr/verify/?v=${verificationToken}`
        : `https://www.certif-scope.com/verify/?v=${verificationToken}`;

    const verificationDisplayUrl =
      locale === "fr"
        ? "https://www.certif-scope.com/fr/verify"
        : "https://www.certif-scope.com/verify";

    const qrDataUrl = await QRCode.toDataURL(verifyUrl, {
      width: 140,
      margin: 1,
    });

    const referenceLabel = locale === "fr" ? "Référence" : "Reference";
    const issuedLabel = locale === "fr" ? "Date d’émission" : "Issue date";
    const validUntilLabel =
      locale === "fr" ? "Valable jusqu’au" : "Valid until";
    const issuerLabelShort = locale === "fr" ? "Émetteur" : "Issuer";

    const methodologicalBasisTitle =
      locale === "fr" ? "Fondement méthodologique" : "Methodological basis";
    const methodologicalBasisText =
      locale === "fr"
        ? "Cette attestation fournit une estimation CO₂e indicative, standardisée et vérifiable, fondée sur une méthodologie spend-based utilisant des données de dépenses agrégées déclarées et des facteurs d’émission monétaires."
        : "This attestation provides an indicative, standardized and verifiable CO₂e estimate based on a spend-based methodology using declared aggregated expenditure data and monetary emission factors.";

    const intendedUseTitle = locale === "fr" ? "Usage prévu" : "Intended use";
    const intendedUseText =
      locale === "fr"
        ? "Cette attestation peut être utilisée comme document indicatif dans un dossier fournisseur, une demande client, un appel d’offres, une demande bancaire, une demande d’assurance ou une démarche interne de première estimation carbone, lorsque le besoin porte sur un indicateur CO₂e agrégé et documenté."
        : "This attestation may be used as an indicative document in supplier files, client requests, tenders, banking requests, insurance requests or an internal first carbon estimation process when the need concerns an aggregated and documented CO₂e indicator.";

    const confidentialityTitle =
      locale === "fr" ? "Confidentialité" : "Confidentiality";
    const authenticityTitle = locale === "fr" ? "Authenticité" : "Authenticity";
    const validityTitle = locale === "fr" ? "Validité" : "Validity";

    const confidentialityLines =
      locale === "fr"
        ? [
            "Aucune conservation des données financières détaillées.",
            "Résultat CO₂e agrégé uniquement.",
            "Attestation vérifiable indépendamment.",
          ]
        : [
            "No storage of detailed financial data.",
            "Aggregated CO₂e result only.",
            "Independently verifiable attestation.",
          ];

    const authenticityText =
      locale === "fr"
        ? "L’attestation comporte une référence unique, un QR code et des éléments de contrôle documentaire."
        : "The attestation includes a unique reference, a QR code and documentary control elements.";

    const validityText =
      locale === "fr"
        ? "La validité documentaire est limitée dans le temps et indiquée sur l’attestation."
        : "Documentary validity is time-limited and stated on the attestation.";

    const simpleVerificationTitle =
      locale === "fr" ? "Vérification simple" : "Simple verification";
    const simpleVerificationText =
      locale === "fr"
        ? "Scanner le QR code ou utiliser la référence d’attestation sur la page officielle de vérification."
        : "Scan the QR code or use the attestation reference on the official verification page.";

    const documentaryVerificationLabel =
      locale === "fr"
        ? "Page de vérification documentaire"
        : "Document verification page";

    const verificationReferenceHint =
      locale === "fr"
        ? `Référence : ${metadata.attestationId}`
        : `Reference: ${metadata.attestationId}`;

    const verifiableObjectTitle =
      locale === "fr" ? "Objet vérifiable" : "Verifiable object";
    const verifiableObjectText =
      locale === "fr"
        ? "Le PDF signé et son QR code constituent les éléments de contrôle documentaire."
        : "The signed PDF and its QR code constitute the documentary control elements.";

    const technicalElementsTitle =
      locale === "fr"
        ? "Éléments techniques de vérification"
        : "Technical verification elements";

    const technicalElementsIntro =
      locale === "fr"
        ? "Les éléments ci-dessous permettent une vérification documentaire avancée. Aucune action n’est requise pour un lecteur standard du document."
        : "The elements below allow advanced documentary verification. No action is required for a standard document reader.";

    const perimeterLimitsTitle =
      locale === "fr" ? "Périmètre et limites" : "Scope and limitations";
    const perimeterLimitsText =
      locale === "fr"
        ? "Cette attestation est indicative. Elle repose sur les données déclarées par l’entité et sur une méthodologie spend-based. Elle ne remplace pas un audit carbone, un inventaire GES complet, une vérification par tiers indépendant, un Bilan Carbone® ou un reporting réglementaire lorsque ceux-ci sont explicitement exigés."
        : "This attestation is indicative. It is based on data declared by the entity and on a spend-based methodology. It does not replace a carbon audit, a full GHG inventory, independent third-party verification, a full carbon assessment or regulatory reporting when these are explicitly required.";

    const responsibilityTitle =
      locale === "fr" ? "Responsabilité des données" : "Data responsibility";
    const responsibilityText =
      locale === "fr"
        ? "Les résultats dépendent de l’exactitude, de la complétude et de la cohérence des données déclarées par l’entité."
        : "Results depend on the accuracy, completeness and consistency of the data declared by the entity.";

    const documentaryValidityTitle =
      locale === "fr" ? "Validité documentaire" : "Documentary validity";
    const documentaryValidityText =
      locale === "fr"
        ? "La période de validité reflète la pertinence temporelle des données déclarées, des facteurs d’émission utilisés et de la méthodologie appliquée."
        : "The validity period reflects the temporal relevance of the declared data, the emission factors used and the methodology applied.";

    const languageNotice =
      locale === "fr"
        ? "Langue du document : En cas de divergence entre les versions linguistiques, la version anglaise prévaut."
        : "Document language: In case of divergence between language versions, the English version shall prevail.";

    const internalStandardNotice =
      locale === "fr"
        ? "CS-SB-v1 est une méthodologie standardisée interne maintenue par Certif-Scope."
        : "CS-SB-v1 is an internal standardized methodology maintained by Certif-Scope.";

    const html = `
<!doctype html>
<html lang="${locale}">
<head>
<meta charset="utf-8"/>
<title>${metadata.issuerName} — Attestation</title>
<style>
  @page {
    size: A4;
    margin: 13mm 13mm 12mm 13mm;
  }

  * {
    box-sizing: border-box;
  }

  body {
    font-family: Inter, "Helvetica Neue", Arial, Helvetica, sans-serif;
    font-size: 9.8px;
    line-height: 1.36;
    margin: 0;
    color: #12202f;
    -webkit-font-smoothing: antialiased;
    text-rendering: geometricPrecision;
  }

  :root {
    --serif: "Times New Roman", Georgia, "Garamond", serif;
    --accent: #0b3a63;
    --accent-soft: #1fb6c1;
    --border: #d9e2ec;
    --border-strong: #b7c8d8;
    --muted: #6a7785;
    --text-soft: #415161;
    --bg-soft: #f7fafc;
    --bg-soft-2: #eef7f8;
  }

  .container {
    width: 100%;
  }

  .page-break {
    page-break-after: always;
    break-after: page;
    height: 0;
  }

  .header {
    display: flex;
    align-items: flex-start;
    justify-content: space-between;
    gap: 20px;
    padding-bottom: 10px;
    margin-bottom: 14px;
    border-bottom: 3px solid var(--accent);
  }

  .issuer {
    flex: 1 1 auto;
    min-width: 0;
  }

  .issuer-logo {
    height: 68px;
    max-width: 290px;
    width: auto;
    object-fit: contain;
    display: block;
    margin-bottom: 6px;
  }

  .issuer-site {
    font-size: 8.2px;
    color: var(--muted);
    margin-bottom: 3px;
  }

  .issuer-meta {
    font-size: 8.2px;
    color: var(--muted);
  }

  .qr-box {
    width: 92px;
    flex: 0 0 92px;
    text-align: center;
    border: 1px solid var(--border);
    border-radius: 4px;
    padding: 6px 5px 5px;
    background: #fff;
  }

  .qr-box img {
    width: 64px;
    height: 64px;
    display: block;
    margin: 0 auto 4px;
  }

  .qr-box .small {
    font-size: 7.4px;
    line-height: 1.18;
    color: #24384c;
  }

  .hero {
    text-align: center;
    margin: 6px 0 16px;
  }

  .eyebrow {
    color: var(--accent-soft);
    font-size: 7.7px;
    font-weight: 800;
    letter-spacing: 1.2px;
    text-transform: uppercase;
    margin-bottom: 5px;
  }

  .hero h1 {
    margin: 0;
    color: var(--accent);
    font-size: 22px;
    line-height: 1.05;
    letter-spacing: 0.4px;
    text-transform: uppercase;
    font-weight: 800;
  }

  .hero .sub-1 {
    margin-top: 4px;
    font-size: 9.5px;
    color: #24384c;
    font-weight: 700;
  }

  .hero .sub-2 {
    margin-top: 3px;
    font-size: 8px;
    color: var(--muted);
  }

  .result-wrap {
    margin-bottom: 12px;
  }

  .result-card {
    max-width: 540px;
    margin: 0 auto;
    border: 2px solid var(--accent);
    border-radius: 6px;
    background: linear-gradient(180deg, #ffffff 0%, #fbfdff 100%);
    padding: 12px 16px 11px;
    text-align: center;
  }

  .result-label {
    font-size: 8px;
    color: #4a5a6a;
    text-transform: uppercase;
    letter-spacing: 0.8px;
    font-weight: 800;
    margin-bottom: 5px;
  }

  .result-value {
    font-family: var(--serif);
    color: var(--accent);
    font-weight: 800;
    font-size: 27px;
    line-height: 1;
    letter-spacing: 0.2px;
    margin-bottom: 4px;
  }

  .result-note {
    font-size: 7.7px;
    color: var(--muted);
  }

  .summary-table {
    width: 100%;
    border-collapse: separate;
    border-spacing: 0;
    border: 1px solid var(--border-strong);
    border-radius: 5px;
    overflow: hidden;
    margin-bottom: 10px;
    table-layout: fixed;
  }

  .summary-table thead th {
    background: #eef5f7;
    color: var(--accent);
    font-size: 8.1px;
    text-transform: uppercase;
    letter-spacing: 0.5px;
    font-weight: 800;
    text-align: left;
    padding: 6px 8px;
    border-bottom: 1px solid var(--border-strong);
  }

  .summary-table tbody td {
    padding: 7px 8px;
    border-right: 1px solid var(--border);
    vertical-align: top;
  }

  .summary-table tbody td:last-child {
    border-right: none;
  }

  .summary-label {
    font-size: 7px;
    color: #7b8794;
    text-transform: uppercase;
    letter-spacing: 0.4px;
    margin-bottom: 3px;
  }

  .summary-value {
    font-size: 9.2px;
    color: #162433;
    font-weight: 700;
    word-break: break-word;
  }

  .accent-card,
  .soft-card,
  .section-card,
  .mini-card,
  .verification-card,
  .tech-card,
  .bottom-card {
    border: 1px solid var(--border);
    border-radius: 5px;
    background: #fff;
    overflow: hidden;
  }

  .accent-card {
    margin-bottom: 8px;
    border-left: 4px solid var(--accent-soft);
    background: var(--bg-soft-2);
    padding: 9px 10px;
  }

  .soft-card {
    margin-bottom: 10px;
    padding: 9px 10px;
    background: var(--bg-soft);
  }

  .card-title {
    color: var(--accent);
    font-size: 8.4px;
    font-weight: 800;
    text-transform: uppercase;
    letter-spacing: 0.5px;
    margin-bottom: 4px;
  }

  .card-text {
    color: #203140;
    font-size: 9px;
    line-height: 1.35;
  }

  .section-title {
    color: var(--accent);
    font-size: 9.7px;
    line-height: 1.1;
    font-weight: 800;
    text-transform: uppercase;
    margin: 0 0 8px;
    letter-spacing: 0.3px;
  }

  .section-card {
    padding: 8px 9px 9px;
    margin-bottom: 10px;
  }

  .entity-grid {
    display: grid;
    grid-template-columns: 1fr 1fr;
    gap: 10px 16px;
  }

  .entity-item .entity-label {
    font-size: 7px;
    color: #7b8794;
    text-transform: uppercase;
    letter-spacing: 0.4px;
    margin-bottom: 3px;
  }

  .entity-item .entity-value {
    font-size: 9.2px;
    color: #12202f;
    font-weight: 700;
    word-break: break-word;
  }

  .two-col {
    display: grid;
    grid-template-columns: 1fr 1fr;
    gap: 10px;
    margin-bottom: 10px;
  }

  .mini-card {
    padding: 8px 9px 9px;
    min-height: 78px;
  }

  .three-col {
    display: grid;
    grid-template-columns: 1fr 1fr 1fr;
    gap: 8px;
    margin-bottom: 8px;
  }

  .plain-list {
    margin: 0;
    padding-left: 14px;
  }

  .plain-list li {
    margin: 0 0 2px;
  }

  .page-2-header {
    display: flex;
    align-items: flex-start;
    justify-content: space-between;
    gap: 12px;
    margin-bottom: 10px;
  }

  .page-2-meta {
    text-align: right;
    font-size: 7.6px;
    color: var(--muted);
    line-height: 1.3;
    padding-top: 2px;
  }

  .section-heading {
    color: var(--accent);
    font-size: 11px;
    font-weight: 800;
    text-transform: uppercase;
    letter-spacing: 0.4px;
    margin: 0 0 10px;
    padding-bottom: 6px;
    border-bottom: 3px solid var(--accent);
  }

  .page-2-block {
    margin-bottom: 10px;
  }

  .methodology-card {
    padding: 8px 9px 9px;
  }

  .formula-box {
    margin: 8px 0;
    background: #edf6f8;
    border: 1px solid var(--border);
    border-radius: 4px;
    text-align: center;
    padding: 7px 8px;
    color: var(--accent);
    font-size: 8.3px;
    font-weight: 800;
  }

  .inline-grid-2 {
    display: grid;
    grid-template-columns: 1fr 1fr;
    gap: 8px;
  }

  .inline-box {
    border: 1px solid var(--border);
    border-radius: 4px;
    background: #fafcfe;
    padding: 6px 7px;
  }

  .inline-box .box-label {
    font-size: 7px;
    color: #7b8794;
    text-transform: uppercase;
    letter-spacing: 0.35px;
    margin-bottom: 2px;
  }

  .inline-box .box-value {
    font-size: 8.5px;
    color: #162433;
    font-weight: 700;
    word-break: break-word;
  }

  .references-card {
    padding: 8px 9px 9px;
  }

  .references-grid {
    display: grid;
    grid-template-columns: 1fr 1fr;
    gap: 8px 16px;
    margin: 6px 0 7px;
  }

  .references-grid ul {
    margin: 0;
    padding-left: 14px;
  }

  .references-grid li {
    margin-bottom: 2px;
  }

  .verification-grid {
    display: grid;
    grid-template-columns: 1fr 1fr;
    gap: 10px;
    margin-bottom: 10px;
  }

  .verification-card {
    padding: 8px 9px 9px;
    min-height: 100px;
  }

  .verification-card.accent {
    border-left: 4px solid var(--accent-soft);
    background: #f4fbfb;
  }

  .verification-link {
    margin-top: 7px;
    font-size: 8.8px;
    color: var(--accent);
    font-weight: 700;
    word-break: break-all;
    text-decoration: none;
  }

  .hint {
    font-size: 7.7px;
    color: var(--muted);
    margin-top: 4px;
  }

  .tech-card {
    padding: 8px 9px 9px;
    margin-bottom: 10px;
  }

  .divider {
    height: 1px;
    background: var(--border);
    margin: 8px 0 7px;
  }

  .tech-grid {
    display: grid;
    grid-template-columns: 148px 1fr;
    gap: 3px 10px;
    align-items: start;
  }

  .tech-key {
    font-size: 7px;
    color: #7b8794;
    text-transform: uppercase;
    letter-spacing: 0.35px;
  }

  .tech-value {
    font-size: 7.7px;
    color: #152535;
    word-break: break-all;
    line-height: 1.25;
  }

  .bottom-card {
    padding: 9px 10px 10px;
    border-left: 4px solid var(--accent);
    background: #fbfdff;
  }

  .bottom-intro {
    font-size: 8.8px;
    color: #24384c;
    line-height: 1.35;
    margin-bottom: 8px;
  }

  .bottom-grid {
    display: grid;
    grid-template-columns: 1fr 1fr;
    gap: 10px;
    margin-bottom: 8px;
  }

  .small-title {
    color: var(--accent);
    font-size: 8px;
    font-weight: 800;
    text-transform: uppercase;
    margin-bottom: 3px;
    letter-spacing: 0.35px;
  }

  .small-text {
    font-size: 8.3px;
    color: #24384c;
    line-height: 1.3;
  }

  .muted-line {
    color: var(--muted);
    font-size: 7.8px;
    margin-top: 5px;
  }

  .footer-static {
    display: flex;
    justify-content: space-between;
    gap: 12px;
    border-top: 1px solid var(--border);
    padding-top: 6px;
    margin-top: 10px;
    color: var(--muted);
    font-size: 7.6px;
  }

  .small {
    color: var(--muted);
  }
</style>
</head>
<body>
<div class="container">

  <div class="header">
    <div class="issuer">
      ${
        CERTIF_SCOPE_LOGO_BASE64
          ? `<img src="data:image/png;base64,${CERTIF_SCOPE_LOGO_BASE64}" class="issuer-logo" alt="Issuer logo" />`
          : ""
      }
      <div class="issuer-site">
        <a href="${metadata.issuerSite}" target="_blank" style="color:inherit; text-decoration:none;">
          ${metadata.issuerSite}
        </a>
      </div>
      <div class="issuer-meta">${i18n.headerTagline}</div>
    </div>

    <div class="qr-box">
      <img src="${qrDataUrl}" alt="QR verification" />
      <div class="small">${i18n.scanToVerifyLabel}</div>
    </div>
  </div>

  <div class="hero">
    <div class="eyebrow">Document CO₂e indicatif · standardisé · vérifiable</div>
    <h1>${i18n.title}</h1>
    <div class="sub-1">${i18n.standardReference}</div>
    <div class="sub-2">${i18n.subtitle} · ${metadata.standardRef}</div>
  </div>

  <div class="result-wrap">
    <div class="result-card">
      <div class="result-label">${i18n.resultLabel}</div>
      <div class="result-value">${metadata.totalCO2e} tCO₂e</div>
      <div class="result-note">
        ${locale === "fr" ? "Estimation indicative issue des données déclarées par l’entité." : "Indicative estimate derived from data declared by the entity."}
      </div>
    </div>
  </div>

  <table class="summary-table" role="presentation">
    <thead>
      <tr>
        <th colspan="4">${locale === "fr" ? "Document vérifiable" : "Verifiable document"}</th>
      </tr>
    </thead>
    <tbody>
      <tr>
        <td>
          <div class="summary-label">${referenceLabel}</div>
          <div class="summary-value">${metadata.attestationId}</div>
        </td>
        <td>
          <div class="summary-label">${issuedLabel}</div>
          <div class="summary-value">${metadata.issuedDate}</div>
        </td>
        <td>
          <div class="summary-label">${validUntilLabel}</div>
          <div class="summary-value">${metadata.validUntil}</div>
        </td>
        <td>
          <div class="summary-label">${issuerLabelShort}</div>
          <div class="summary-value">${metadata.issuerName}</div>
        </td>
      </tr>
    </tbody>
  </table>

  <div class="accent-card">
    <div class="card-title">${methodologicalBasisTitle}</div>
    <div class="card-text">${methodologicalBasisText}</div>
  </div>

  <div class="soft-card">
    <div class="card-title">${intendedUseTitle}</div>
    <div class="card-text">${intendedUseText}</div>
  </div>

  <div class="section-card">
    <div class="section-title">3. ${i18n.entitySectionTitle}</div>
    <div class="entity-grid">
      <div class="entity-item">
        <div class="entity-label">${i18n.entityNameLabel}</div>
        <div class="entity-value">${metadata.companyName}</div>
      </div>
      <div class="entity-item">
        <div class="entity-label">${i18n.countryLabel}</div>
        <div class="entity-value">${metadata.country}</div>
      </div>
      <div class="entity-item">
        <div class="entity-label">${i18n.activitySectorLabel}</div>
        <div class="entity-value">${metadata.companySector}</div>
      </div>
      <div class="entity-item">
        <div class="entity-label">${i18n.reportingYearLabel}</div>
        <div class="entity-value">${metadata.year}</div>
      </div>
      <div class="entity-item">
        <div class="entity-label">${i18n.entityIdentifierLabel}</div>
        <div class="entity-value">${metadata.entityIdentifier}</div>
      </div>
      <div class="entity-item">
        <div class="entity-label">${locale === "fr" ? "Identifiant unique du document" : "Unique document identifier"}</div>
        <div class="entity-value">${metadata.attestationId}</div>
      </div>
    </div>
  </div>

  <div class="two-col">
    <div class="mini-card">
      <div class="card-title">2. ${i18n.documentNatureSectionTitle}</div>
      <div class="card-text"><strong>${i18n.documentNatureText}</strong></div>
    </div>
    <div class="mini-card">
      <div class="card-title">4. ${i18n.scopeSectionTitle}</div>
      <div class="card-text">
        <strong>${i18n.scopeDescriptionLabel} :</strong> ${i18n.scopeText}
      </div>
    </div>
  </div>

  <div class="three-col">
    <div class="mini-card">
      <div class="card-title">${confidentialityTitle}</div>
      <div class="card-text">
        ${confidentialityLines.map((line) => `<div>• ${line}</div>`).join("")}
      </div>
    </div>
    <div class="mini-card">
      <div class="card-title">${authenticityTitle}</div>
      <div class="card-text">${authenticityText}</div>
    </div>
    <div class="mini-card">
      <div class="card-title">${validityTitle}</div>
      <div class="card-text">${validityText}</div>
    </div>
  </div>

  <div class="footer-static">
    <div>${i18n.footerText}</div>
    <div>${i18n.pageLabel} 1 / 2</div>
  </div>

  <div class="page-break"></div>

  <div class="page-2-header">
    <div class="section-heading">7. ${i18n.methodologySectionTitle}</div>
    <div class="page-2-meta">
      <div>${referenceLabel} : ${metadata.attestationId}</div>
      <div>${issuerLabelShort} : ${metadata.issuerName}</div>
    </div>
  </div>

  <div class="page-2-block methodology-card section-card">
    <div class="card-title">${locale === "fr" ? "Méthodologie" : "Methodology"}</div>
    <div class="card-text"><strong>${metadata.methodology}</strong></div>

    <div class="formula-box">
      ${locale === "fr"
        ? "Dépenses agrégées déclarées × facteurs d’émission monétaires = estimation CO₂e indicative"
        : "Declared aggregated expenditures × monetary emission factors = indicative CO₂e estimate"}
    </div>

    <div class="inline-grid-2">
      <div class="inline-box">
        <div class="box-label">${locale === "fr" ? "Version des facteurs" : "Factor version"}</div>
        <div class="box-value">${metadata.factorVersion}</div>
      </div>
      <div class="inline-box">
        <div class="box-label">${i18n.transferabilityLabel}</div>
        <div class="box-value">${i18n.transferabilityText}</div>
      </div>
    </div>
  </div>

  <div class="page-2-block references-card section-card">
    <div class="card-title">${locale === "fr" ? "Références de cohérence" : "Reference frameworks"}</div>
    <div class="small-text">
      ${locale === "fr"
        ? "La méthodologie Certif-Scope s’inscrit dans une logique de cohérence avec les cadres de référence carbone et de transparence environnementale suivants :"
        : "The Certif-Scope methodology follows a coherence logic aligned with the following carbon and environmental transparency reference frameworks:"}
    </div>

    <div class="references-grid">
      <ul>
        <li>GHG Protocol — Scope 3, méthode spend-based</li>
        <li>ISO 14083 — émissions liées aux opérations de transport</li>
        <li>Taxonomie européenne — contexte durabilité et reporting extra-financier</li>
      </ul>
      <ul>
        <li>ISO 14064-1 — quantification et reporting GES</li>
        <li>CSRD / ESRS — transparence environnementale européenne</li>
      </ul>
    </div>

    <div class="muted-line">
      ${locale === "fr"
        ? "Ces références sont citées comme cadre de cohérence méthodologique et contextuel. Elles ne transforment pas cette attestation en rapport réglementaire, audit certifié ou vérification par tiers indépendant."
        : "These references are cited as a framework for methodological and contextual coherence. They do not transform this attestation into a regulatory report, certified audit or independent third-party verification."}
    </div>
  </div>

  <div class="verification-grid">
    <div class="verification-card accent">
      <div class="card-title">${simpleVerificationTitle}</div>
      <div class="card-text">${simpleVerificationText}</div>

      <div class="card-title" style="margin-top:8px;">${documentaryVerificationLabel}</div>
      <a href="${verificationDisplayUrl}" class="verification-link">
        ${verificationDisplayUrl}
      </a>
      <div class="hint">${verificationReferenceHint}</div>
    </div>

    <div class="verification-card">
      <div class="card-title">${verifiableObjectTitle}</div>
      <div class="card-text">${verifiableObjectText}</div>
      <div class="hint" style="margin-top:8px;">
        ${locale === "fr"
          ? "Le document PDF signé et son QR code de vérification constituent les éléments de contrôle documentaire."
          : "The signed PDF document and its verification QR code constitute the documentary control elements."}
      </div>
    </div>
  </div>

  <div class="tech-card">
    <div class="card-title">${technicalElementsTitle}</div>
    <div class="small-text">${technicalElementsIntro}</div>

    <div class="divider"></div>

    <div class="tech-grid">
      <div class="tech-key">${locale === "fr" ? "Algorithme" : "Algorithm"}</div>
      <div class="tech-value">${metadata.algorithm}</div>

      <div class="tech-key">${locale === "fr" ? "Empreinte du contenu signé (SHA-256)" : "Signed content fingerprint (SHA-256)"}</div>
      <div class="tech-value">${metadata.hash}</div>

      <div class="tech-key">${locale === "fr" ? "Signature (Base64)" : "Signature (Base64)"}</div>
      <div class="tech-value">${metadata.signature}</div>

      <div class="tech-key">${locale === "fr" ? "Clé publique de vérification de l’émetteur" : "Issuer verification public key"}</div>
      <div class="tech-value">MCowBQYDK2VwAyEAbKp2pg4wmzE5Kqo9tEwv7JJjxQyT2cBmwiLLHp4cSac=</div>
    </div>
  </div>

  <div class="bottom-card">
    <div class="card-title">${perimeterLimitsTitle}</div>
    <div class="bottom-intro">${perimeterLimitsText}</div>

    <div class="bottom-grid">
      <div>
        <div class="small-title">${responsibilityTitle}</div>
        <div class="small-text">${responsibilityText}</div>
      </div>
      <div>
        <div class="small-title">${documentaryValidityTitle}</div>
        <div class="small-text">${documentaryValidityText}</div>
      </div>
    </div>

    <div class="small-text"><strong>${languageNotice}</strong></div>
    <div class="muted-line">${internalStandardNotice}</div>
  </div>

  <div class="footer-static">
    <div>${i18n.footerText}</div>
    <div>${i18n.pageLabel} 2 / 2</div>
  </div>

</div>
</body>
</html>
`;

    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 15000);

    const pdfResponse = await fetch("https://api.pdfshift.io/v3/convert/pdf", {
      method: "POST",
      headers: {
        "X-API-Key": process.env.PDFSHIFT_API_KEY!,
        "Content-Type": "application/json",
      },
      body: JSON.stringify({
        source: html,
        format: "A4",
        use_print: true,
      }),
      signal: controller.signal,
    });

    clearTimeout(timeoutId);

    if (!pdfResponse.ok) {
      const error = await pdfResponse.text();
      return new Response(error, { status: pdfResponse.status });
    }

    const pdfBuffer = Buffer.from(await pdfResponse.arrayBuffer());

    const safeIssuerName = metadata.issuerName
      .toLowerCase()
      .replace(/[^a-z0-9\-]/g, "");

    return new Response(pdfBuffer, {
      headers: {
        "Content-Type": "application/pdf",
        "Content-Disposition": `attachment; filename="${safeIssuerName}-${metadata.attestationId}.pdf"`,
        "Cache-Control": "no-store",
        "X-Content-Type-Options": "nosniff",
      },
    });
  } catch (err) {
    if (process.env.NODE_ENV !== "production") {
      console.error(err);
    }

    return new Response("Internal Server Error", { status: 500 });
  }
                         }
